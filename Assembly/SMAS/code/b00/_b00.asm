...
; Disable NMI, IRQ, and joypad read
STZ $4200               ; $00:8001: 9C 00 42

; HDMA Disabled
STZ $420C               ; $00:8004: 9C 0C 42

; DMA Disabled
STZ $420B               ; $00:8007: 9C 0B 42
STZ $2140               ; $00:800A: 9C 40 21
STZ $2141               ; $00:800D: 9C 41 21
STZ $2142               ; $00:8010: 9C 42 21

; SPC ports disabled.
STZ $2143               ; $00:8013: 9C 43 21
LDA #$80                ; $00:8016: A9 80
...
; 16-bit AXY, not decimal mode
REP #$38                ; $00:801D: C2 38
LDA #$0000              ; $00:801F: A9 00 00

; Direct page register is now $0000
TCD                     ; $00:8022: 5B
LDA #$01FF              ; $00:8023: A9 FF 01

; The stack pointer is now $00:01FF
TCS                     ; $00:8026: 1B

; 8-bit AXY
SEP #$30                ; $00:8027: E2 30
LDA #$00                ; $00:8029: A9 00
PHA                     ; $00:802B: 48

; Set program bank: Bank $00
PLB                     ; $00:802C: AB
LDA $213F               ; $00:802D: AD 3F 21
BIT #$10                ; $00:8030: 89 10

; This ROM is NTSC
; If the SNES is a NTSC SNES
; Continue
BEQ CODE_00803B         ; $00:8032: F0 07

; Otherwise, show error message (incompatible SNES)
JMP CODE_0093C7         ; $00:8034: 4C C7 93

; Show error message (Pirated game)

CODE_008037:
JML CODE_009641         ; $00:8037: 5C 41 96 00

CODE_00803B:
JSR CODE_0083B6         ; $00:803B: 20 B6 83
LDA #$00                ; $00:803E: A9 00
STA $701FF0             ; $00:8040: 8F F0 1F 70
STA $701FF2             ; $00:8044: 8F F2 1F 70

; 16-bit A
REP #$20                ; $00:8048: C2 20
...
; 8-bit A
SEP #$20                ; $00:8056: E2 20
LDA #$81                ; $00:8058: A9 81

; Enable NMI and joypad.
STA $4200               ; $00:805A: 8D 00 42

; Store timer of Mario logo (#$81 too)
STA $001C               ; $00:805D: 8D 1C 00
JSR CODE_008C1B         ; $00:8060: 20 1B 8C
NOP                     ; $00:8063: EA

; No operation. Likely overwriting a
; deprecated program operation
NOP                     ; $00:8064: EA
LDA #$AA                ; $00:8065: A9 AA
STA $702000             ; $00:8067: 8F 00 20 70
CMP $700000             ; $00:806B: CF 00 00 70

; Copier check
;
; If $700000 is not $AA
; Then the game is pirated, show error msg.
BNE CODE_008037         ; $00:806F: D0 C6
LDA #$55                ; $00:8071: A9 55
STA $702000             ; $00:8073: 8F 00 20 70
CMP $700000             ; $00:8077: CF 00 00 70

; Another copier check
;
; If $700000 is not $55
; Then the game is pirated, show error msg.
BNE CODE_008037         ; $00:807B: D0 BA
...
LDX $E0                 ; $00:8088: A6 E0
LDA DATA_00883D,x       ; $00:808A: BD 3D 88
BEQ CODE_008092         ; $00:808D: F0 03

; Game mode as index
; If flag to clear OAM is set, indexed by
; game mode
; <Then clear the OAM.
JSR CODE_008719         ; $00:808F: 20 19 87

; Update the game

CODE_008092:
JSL GameIndex           ; $00:8092: 22 C7 80 00
LDX $E0                 ; $00:8096: A6 E0
LDA DATA_00883D,x       ; $00:8098: BD 3D 88
BEQ CODE_0080A0         ; $00:809B: F0 03

; If flag to update OAM sizes is set
;
; Update OAM sizes
JSR CODE_008773         ; $00:809D: 20 73 87

CODE_0080A0:
STZ $0122               ; $00:80A0: 9C 22 01

; Game loop
JMP CODE_00807D         ; $00:80A3: 4C 7D 80

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Pointers below are specified by $E0's values
;;

; Nintendo Presents Screen + Title screen
...
; Close erase file box
db CODE_00B2A5

; Fade out of game select screen
db CODE_00A2C5
...
STA $03                 ; $00:80CD: 85 03
LDA.l DATA_0080B1,x     ; $00:80CF: BF B1 80 00
STA $04                 ; $00:80D3: 85 04
LDA.l DATA_0080BC,x     ; $00:80D5: BF BC 80 00

; Store indirect pointers.
STA $05                 ; $00:80D9: 85 05

; Jump to the indirect game pointers.
JML [$0003]             ; $00:80DB: DC 03 00

; Forced blank + HDMA-Disable

CODE_0080DE:
JSL ForceBlank          ; $00:80DE: 22 2E 88 00

; Disable interrupts
STZ $4200               ; $00:80E2: 9C 00 42
LDA #$00                ; $00:80E5: A9 00
PHA                     ; $00:80E7: 48

; Set program bank to $00
PLB                     ; $00:80E8: AB

; Set interrupt flag
SEI                     ; $00:80E9: 78
STZ $4200               ; $00:80EA: 9C 00 42

; No interrupts and HDMA
STZ $420C               ; $00:80ED: 9C 0C 42
REP #$20                ; $00:80F0: C2 20
LDA #$01FF              ; $00:80F2: A9 FF 01
TCS                     ; $00:80F5: 1B
...
CODE_008139:
JSL ForceBlank          ; $00:8139: 22 2E 88 00

; Disable interrupts
STZ $4200               ; $00:813D: 9C 00 42
...
CODE_0081A0:
LDX #$0F                ; $00:81A0: A2 0F

CODE_0081A2:
LDA $70,x               ; $00:81A2: B5 70
...
; Load savegame into RAM $7FFB00 area.
JSL CODE_00A849         ; $00:8207: 22 49 A8 00
...
CODE_00822B:
LDA $701FF0             ; $00:822B: AF F0 1F 70

; Get game to show
PHA                     ; $00:822F: 48
INC A                   ; $00:8230: 1A
AND #$03                ; $00:8231: 29 03

; Do  different game next time
STA $701FF0             ; $00:8233: 8F F0 1F 70
STZ $D1                 ; $00:8237: 64 D1
LDA #$01                ; $00:8239: A9 01

; We are doing the game-play exmaple video
STA $701FF2             ; $00:823B: 8F F2 1F 70
LDX #$0F                ; $00:823F: A2 0F

CODE_008241:
LDA $70,x               ; $00:8241: B5 70
STA $700490,x           ; $00:8243: 9F 90 04 70
DEX                     ; $00:8247: CA

; ?? Seems to write $00 to SRAM
BPL CODE_008241         ; $00:8248: 10 F7
REP #$20                ; $00:824A: C2 20
LDA #$0000              ; $00:824C: A9 00 00
STA $00                 ; $00:824F: 85 00
LDA #$0000              ; $00:8251: A9 00 00
...
; Store here
STA $7FFF00             ; $00:8262: 8F 00 FF 7F
LDA.l DATA_008194,x     ; $00:8266: BF 94 81 00
STA $03                 ; $00:826A: 85 03
LDA.l DATA_008198,x     ; $00:826C: BF 98 81 00
...
CODE_0082B1:
LDA DATA_008DE3,y       ; $00:82B1: B9 E3 8D
...
PHK                     ; $00:82E4: 4B
PLB                     ; $00:82E5: AB
LDA $00                 ; $00:82E6: A5 00
PHA                     ; $00:82E8: 48

; Preserve 'everything', set direct page reg to $0000
;
...
;
;
; PEI ($00) would've worked too.
; Preserve RAM address $00-$01 (scratch ram)
SEP #$30                ; $00:82E9: E2 30
LDA $7FFF00             ; $00:82EB: AF 00 FF 7F
TAX                     ; $00:82EF: AA

; Jump to NMI routines of the current game
...
CODE_0082FF:
LDA #$01                ; $00:82FF: A9 01
XBA                     ; $00:8301: EB
LDA #$00                ; $00:8302: A9 00

; Set direct page to $0100
TCD                     ; $00:8304: 5B

; vital read to enable NMI
LDA $4210               ; $00:8305: AD 10 42
LDA $00                 ; $00:8308: A5 00

; Copy of reg $2100 to $0100 (Brightness)
STA $2100               ; $00:830A: 8D 00 21

; Disable HDMA
STZ $420C               ; $00:830D: 9C 0C 42
LDA $22                 ; $00:8310: A5 22
BNE CODE_00831D         ; $00:8312: D0 09
INC $22                 ; $00:8314: E6 22
PHD                     ; $00:8316: 0B
JSR CODE_0088C3         ; $00:8317: 20 C3 88
SEP #$30                ; $00:831A: E2 30

; Flag to do some general purpose DMA?
; When $0122 is set, do some DMA transfers I guess
PLD                     ; $00:831C: 2B

; Set up hardware addresses

CODE_00831D:
JSR CODE_00841A         ; $00:831D: 20 1A 84

; Do something SFX panels and music related
JSR CODE_0087CC         ; $00:8320: 20 CC 87
LDX #$81                ; $00:8323: A2 81
LDA $24                 ; $00:8325: A5 24
BEQ CODE_008340         ; $00:8327: F0 17
AND #$FE                ; $00:8329: 29 FE
...
; If $7E0124 is set, let the IRQ cover the whole screen
; by disabling NMI or something.

CODE_008340:
STX $4200               ; $00:8340: 8E 00 42
LDA $00                 ; $00:8343: A5 00

; $7E0100 = brightness
STA $2100               ; $00:8345: 8D 00 21
LDA $20                 ; $00:8348: A5 20
...
CODE_00834E:
JSL CODE_038003         ; $00:834E: 22 03 80 03

; NMI for SMB1
RTS                     ; $00:8352: 60

CODE_008353:
JSL CODE_0D8003         ; $00:8353: 22 03 80 0D

; NMI for SMB:TLL
RTS                     ; $00:8357: 60

CODE_008358:
JSL CODE_118003         ; $00:8358: 22 03 80 11

; NMI for SMB2
RTS                     ; $00:835C: 60

CODE_00835D:
JSL CODE_208003         ; $00:835D: 22 03 80 20
...
; IRQ Pointers, based on current game.
dw CODE_0083B1
...
; Preserve "everything", set direct page to $0000
SEP #$30                ; $00:8379: E2 30
LDA $7FFF00             ; $00:837B: AF 00 FF 7F
TAX                     ; $00:837F: AA

; Execute IRQ based on current game
...
; Recover "everything" and return
RTI                     ; $00:838A: 40

CODE_00838B:
LDA $4211               ; $00:838B: AD 11 42

; Menu IRQ routine. Wait until IRQ occurs
BPL CODE_0083A1         ; $00:838E: 10 11

CODE_008390:
BIT $4212               ; $00:8390: 2C 12 42

; Wait for H-blank
BVC CODE_008390         ; $00:8393: 50 FB
LDA $0214               ; $00:8395: AD 14 02
STA $210D               ; $00:8398: 8D 0D 21
LDA $0215               ; $00:839B: AD 15 02
STA $210D               ; $00:839E: 8D 0D 21

; Set BG1 H-scroll to normal

CODE_0083A1:
RTS                     ; $00:83A1: 60

CODE_0083A2:
JSL CODE_038006         ; $00:83A2: 22 06 80 03

; IRQ for SMB1
RTS                     ; $00:83A6: 60

CODE_0083A7:
JSL CODE_0D8006         ; $00:83A7: 22 06 80 0D

; IRQ for SMB:TLL
RTS                     ; $00:83AB: 60

CODE_0083AC:
JSL CODE_118006         ; $00:83AC: 22 06 80 11

; IRQ for SMB2
RTS                     ; $00:83B0: 60

CODE_0083B1:
JSL CODE_208006         ; $00:83B1: 22 06 80 20

; IRQ for SMB3
RTS                     ; $00:83B5: 60

; Upload SPC engine + audio data of hall and menu screen

CODE_0083B6:
JSR CODE_008A4F         ; $00:83B6: 20 4F 8A

; 16-bit A. 8-bit XY.

CODE_0083B9:
REP #$20                ; $00:83B9: C2 20
LDA #$0150              ; $00:83BB: A9 50 01

; Size: $0150
STA $00                 ; $00:83BE: 85 00
LDA #$0000              ; $00:83C0: A9 00 00

; Destination: $00:0000
TAY                     ; $00:83C3: A8

; Clear RAM
JSL CODE_00868E         ; $00:83C4: 22 8E 86 00
LDA #$1E00              ; $00:83C8: A9 00 1E

; Size: $1E00
STA $00                 ; $00:83CB: 85 00
LDA #$0200              ; $00:83CD: A9 00 02

; Destination: $00:0200
TAY                     ; $00:83D0: A8

; Clear RAM
JSL CODE_00868E         ; $00:83D1: 22 8E 86 00
LDA #$E000              ; $00:83D5: A9 00 E0

; Size: $E000
STA $00                 ; $00:83D8: 85 00
LDA #$2000              ; $00:83DA: A9 00 20

; Destination: $7E:2000
LDY #$7E                ; $00:83DD: A0 7E

; Clear RAM
JSL CODE_00868E         ; $00:83DF: 22 8E 86 00
LDA #$0000              ; $00:83E3: A9 00 00

; Size: $10000
STA $00                 ; $00:83E6: 85 00
LDA #$0000              ; $00:83E8: A9 00 00

; Destination: $7F:0000
LDY #$7F                ; $00:83EB: A0 7F

; Clear RAM
JSL CODE_00868E         ; $00:83ED: 22 8E 86 00
...
TCD                     ; $00:840B: 5B
STZ $2133               ; $00:840C: 9C 33 21
JSR CODE_00841A         ; $00:840F: 20 1A 84

; Direct page: $0100
;
;
; No interlace, overscan, p-hires etc
; Setup hardware RAM addresses of menu screens
; Recover direct page
PLD                     ; $00:8412: 2B
LDA #$01                ; $00:8413: A9 01

; Upload full palette data from $7F:9200 for 1 frame.
STA $000AC0             ; $00:8415: 8F C0 0A 00
RTS                     ; $00:8419: 60

; 16-bit XY. This routine sets up hardware regs

CODE_00841A:
REP #$10                ; $00:841A: C2 10
LDA $01                 ; $00:841C: A5 01

; DP is $0100 in this case.
; Object size + Chr address
STA $2101               ; $00:841E: 8D 01 21
LDX $04                 ; $00:8421: A6 04

; BG1 Tilemap Address and Size
; BG2 Tilemap Address and Size
STX $2107               ; $00:8423: 8E 07 21
LDA $06                 ; $00:8426: A5 06

; BG3 Tilemap Address and Size
STA $2109               ; $00:8428: 8D 09 21
LDA $07                 ; $00:842B: A5 07

; BG1 and 2 Character address
STA $210B               ; $00:842D: 8D 0B 21
LDA $08                 ; $00:8430: A5 08
STA $210D               ; $00:8432: 8D 0D 21
LDA $09                 ; $00:8435: A5 09

; BG1 Horizontal Scroll
STA $210D               ; $00:8437: 8D 0D 21
LDA $0A                 ; $00:843A: A5 0A
STA $210E               ; $00:843C: 8D 0E 21
LDA $0B                 ; $00:843F: A5 0B

; BG1 Vertical Scroll
STA $210E               ; $00:8441: 8D 0E 21
LDA $0C                 ; $00:8444: A5 0C
STA $210F               ; $00:8446: 8D 0F 21
LDA $0D                 ; $00:8449: A5 0D

; BG2 Horizontal Scroll
STA $210F               ; $00:844B: 8D 0F 21
LDA $0E                 ; $00:844E: A5 0E
STA $2110               ; $00:8450: 8D 10 21
LDA $0F                 ; $00:8453: A5 0F

; BG2 Vertical Scroll
STA $2110               ; $00:8455: 8D 10 21
LDA $10                 ; $00:8458: A5 10
STA $2111               ; $00:845A: 8D 11 21
LDA $11                 ; $00:845D: A5 11

; BG3 Horizontal Scroll
STA $2111               ; $00:845F: 8D 11 21
LDA $12                 ; $00:8462: A5 12
STA $2112               ; $00:8464: 8D 12 21
...
LDA $1E                 ; $00:8471: A5 1E
STA $2132               ; $00:8473: 8D 32 21
LDA $1F                 ; $00:8476: A5 1F

; Fixed Color data
STA $2132               ; $00:8478: 8D 32 21
LDX $17                 ; $00:847B: A6 17

; Main and Subscreen designation
STX $212C               ; $00:847D: 8E 2C 21
LDX $19                 ; $00:8480: A6 19

; Window mask designation for main
; and subscreen
STX $212E               ; $00:8482: 8E 2E 21
LDX $14                 ; $00:8485: A6 14

; Window Mask Settings for BG1/2/3/4
STX $2123               ; $00:8487: 8E 23 21
LDA $16                 ; $00:848A: A5 16

; Window Mask Settings for OBJ and Color Window
STA $2125               ; $00:848C: 8D 25 21
LDX $1B                 ; $00:848F: A6 1B

; Color addition select & CGADSUB
STX $2130               ; $00:8491: 8E 30 21
LDX $02                 ; $00:8494: A6 02

; Screen mode
STX $2105               ; $00:8496: 8E 05 21

; 8-bit XY
SEP #$10                ; $00:8499: E2 10
...
CODE_00849C:
JSL ForceBlank          ; $00:849C: 22 2E 88 00
PHD                     ; $00:84A0: 0B
...
; If it is not time to upload the game select screen tilemap, branch
BEQ CODE_0084D7         ; $00:84A8: F0 2D
LDA #$01                ; $00:84AA: A9 01
STA $0124               ; $00:84AC: 8D 24 01
LDA #$37                ; $00:84AF: A9 37

; Set vertical IRQ timer
STA $0125               ; $00:84B1: 8D 25 01
PHB                     ; $00:84B4: 8B
PHX                     ; $00:84B5: DA
REP #$30                ; $00:84B6: C2 30
LDA $0000D0             ; $00:84B8: AF D0 00 00
AND #$0003              ; $00:84BC: 29 03 00
ASL A                   ; $00:84BF: 0A
TAX                     ; $00:84C0: AA
...
LDA #$0FFF              ; $00:84CD: A9 FF 0F
MVN $7F,DATA_02F000>>16 ; $00:84D0: 54 7F 02
SEP #$30                ; $00:84D3: E2 30
PLX                     ; $00:84D5: FA

; Upload the game select tilemap to RAM after
; closing the file select menu. Upload the tiles
; according to $D0's index so we won't upload the
; wrong game's portrait tiles.
;
;
;
;
; Source: $02:F000. Dest: $7F:1000
; Used when collapsing a file select menu
; after selecting a game. 8-bit AXY
PLB                     ; $00:84D6: AB

CODE_0084D7:
LDA DATA_008873,x       ; $00:84D7: BD 73 88
...
LDA #$4300              ; $00:8531: A9 00 43
TCD                     ; $00:8534: 5B
JSR CODE_008565         ; $00:8535: 20 65 85

; Set direct page to $4300
;
; DMA game select/menu tiles to VRAM
PLD                     ; $00:8538: 2B
TXA                     ; $00:8539: 8A
AND #$00FF              ; $00:853A: 29 FF 00
ASL A                   ; $00:853D: 0A

; Index correctly
TAX                     ; $00:853E: AA
LDA #$0200              ; $00:853F: A9 00 02

; Size: $0200
STA $00                 ; $00:8542: 85 00
LDA DATA_0088BF,x       ; $00:8544: BD BF 88
LDY.b #DATA_3C8E00>>16  ; $00:8547: A0 3C
STA $02                 ; $00:8549: 85 02

; Upload palette
STY $04                 ; $00:854B: 84 04
LDA #$9200              ; $00:854D: A9 00 92

; $7F:9200
LDY #$7F                ; $00:8550: A0 7F

; Upload to RAM
JSL CODE_00866D         ; $00:8552: 22 6D 86 00
...
; Upload Hall Screen/Game select tiles

CODE_008565:
PHX                     ; $00:8565: DA
LDY #$80                ; $00:8566: A0 80
...
; DMA mode 01: 2 regs write once
STA $00                 ; $00:856E: 85 00

; <DMA Channel #0
LDY #$01                ; $00:8570: A0 01
TXA                     ; $00:8572: 8A
ASL A                   ; $00:8573: 0A
TAX                     ; $00:8574: AA
...
CODE_00857A:
LDA #$0000              ; $00:857A: A9 00 00
STA $2116               ; $00:857D: 8D 16 21
LDA.w #DATA_018000      ; $00:8580: A9 00 80
STA $02                 ; $00:8583: 85 02
LDX.b #DATA_018000>>16  ; $00:8585: A2 01
STX $04                 ; $00:8587: 86 04
LDA #$8000              ; $00:8589: A9 00 80
STA $05                 ; $00:858C: 85 05

; DMA 'hall screen' graphics to VRAM
; DMA to: VRAM $0000-$7FFF
;
;
; DMA source: $01:8000
; Size: $8000 bytes/32kB
STY $420B               ; $00:858E: 8C 0B 42
LDA.w #DATA_3DCC00      ; $00:8591: A9 00 CC
STA $02                 ; $00:8594: 85 02
LDX.b #DATA_3DCC00>>16  ; $00:8596: A2 3D
STX $04                 ; $00:8598: 86 04
LDA #$1400              ; $00:859A: A9 00 14
STA $05                 ; $00:859D: 85 05

; Upload animated hall screen tiles of Bowser, princess, toad, luigi's ear to VRAM
; DMA to: VRAM $8000-$93FF
;
;
; DMA Source: $3D:CC00
; Size: $1400 bytes
STY $420B               ; $00:859F: 8C 0B 42
LDA #$5000              ; $00:85A2: A9 00 50
STA $2116               ; $00:85A5: 8D 16 21
LDA.w #DATA_029000      ; $00:85A8: A9 00 90
STA $02                 ; $00:85AB: 85 02
LDX.b #DATA_029000>>16  ; $00:85AD: A2 02
STX $04                 ; $00:85AF: 86 04
LDA #$0800              ; $00:85B1: A9 00 08
STA $05                 ; $00:85B4: 85 05

; Upload triangle fading GFX to VRAM
;
; DMA to: VRAM $A000-$A7FF
;
; DMA Source: $02:9000
; Size: $0800 bytes
STY $420B               ; $00:85B6: 8C 0B 42
LDA #$6000              ; $00:85B9: A9 00 60
STA $2116               ; $00:85BC: 8D 16 21
LDA.w #DATA_028000      ; $00:85BF: A9 00 80
STA $02                 ; $00:85C2: 85 02
LDX.b #DATA_028000>>16  ; $00:85C4: A2 02
STX $04                 ; $00:85C6: 86 04
LDA #$1000              ; $00:85C8: A9 00 10
STA $05                 ; $00:85CB: 85 05

; Upload Nintendo Presents graphics to VRAM
;
;
; DMA to: VRAM $C000-$CFFF
;
; DMA Source: $02:8000
; Size: $1000 bytes
STY $420B               ; $00:85CD: 8C 0B 42
LDA #$7000              ; $00:85D0: A9 00 70
STA $2116               ; $00:85D3: 8D 16 21
LDA.w #DATA_02C000      ; $00:85D6: A9 00 C0
STA $02                 ; $00:85D9: 85 02
LDX.b #DATA_02C000>>16  ; $00:85DB: A2 02
STX $04                 ; $00:85DD: 86 04
LDA #$0800              ; $00:85DF: A9 00 08
STA $05                 ; $00:85E2: 85 05

; Upload default BG2 hall screen tilemap?
;
; DMA to: VRAM $E000-$E7FF
;
; DMA Source: $02:C000
; DMA Size: $0800 bytes
STY $420B               ; $00:85E4: 8C 0B 42
LDA #$7800              ; $00:85E7: A9 00 78
STA $2116               ; $00:85EA: 8D 16 21
LDA.w #DATA_3C8000      ; $00:85ED: A9 00 80
STA $02                 ; $00:85F0: 85 02
LDX.b #DATA_3C8000>>16  ; $00:85F2: A2 3C
STX $04                 ; $00:85F4: 86 04
LDA #$0800              ; $00:85F6: A9 00 08
STA $05                 ; $00:85F9: 85 05

; upload default BG1 hall screen tilemap
;
;
; DMA to: VRAM $F000-$F7FF
;
; DMA Source: $3C:8000
; DMA Size: $0800 bytes
...
CODE_0085FF:
LDA #$0000              ; $00:85FF: A9 00 00
STA $2116               ; $00:8602: 8D 16 21
LDA.w #DATA_2B8000      ; $00:8605: A9 00 80
STA $02                 ; $00:8608: 85 02
LDX.b #DATA_2B8000>>16  ; $00:860A: A2 2B
STX $04                 ; $00:860C: 86 04
...
; Upload game select menu tiles
; and its components to VRAM $0000-$D7FF
;
; Source: $2B8000-$2BFFFF
; $2C8000-$2CFFFF
STY $420B               ; $00:8624: 8C 0B 42
LDA #$6C00              ; $00:8627: A9 00 6C
STA $2116               ; $00:862A: 8D 16 21
LDA.w #DATA_02C000      ; $00:862D: A9 00 C0
STA $02                 ; $00:8630: 85 02
LDX.b #DATA_02C000>>16  ; $00:8632: A2 02
STX $04                 ; $00:8634: 86 04
LDA #$0800              ; $00:8636: A9 00 08
STA $05                 ; $00:8639: 85 05

; Upload default BG2 hall screen tilemap?
; DMA to VRAM $D800-$DFFF
;
; Source: $02:C000
;
; Size: $0800 bytes
...
LDA.w #DATA_3DA800      ; $00:8644: A9 00 A8
STA $02                 ; $00:8647: 85 02
LDX.b #DATA_3DA800>>16  ; $00:8649: A2 3D
STX $04                 ; $00:864B: 86 04
LDA #$0800              ; $00:864D: A9 00 08
STA $05                 ; $00:8650: 85 05

; Upload to VRAM $E000-$E7FF
;
; Upload absolute nothingness?
;
; Source: $3D:A800
STY $420B               ; $00:8652: 8C 0B 42
LDA #$7800              ; $00:8655: A9 00 78
STA $2116               ; $00:8658: 8D 16 21
LDA.w #DATA_02F000      ; $00:865B: A9 00 F0
STA $02                 ; $00:865E: 85 02
LDX.b #DATA_02F000>>16  ; $00:8660: A2 02
STX $04                 ; $00:8662: 86 04
LDA #$1000              ; $00:8664: A9 00 10
STA $05                 ; $00:8667: 85 05

; Upload game select menu tilemap
; to VRAM $F000-$FFFF
;
; Source: $02:F000-$02:FFFF
STY $420B               ; $00:8669: 8C 0B 42
RTS                     ; $00:866C: 60

CODE_00866D:
STA $2181               ; $00:866D: 8D 81 21
STY $2183               ; $00:8670: 8C 83 21
LDA #$8000              ; $00:8673: A9 00 80
STA $4300               ; $00:8676: 8D 00 43
LDA $02                 ; $00:8679: A5 02
STA $4302               ; $00:867B: 8D 02 43
LDY $04                 ; $00:867E: A4 04
STY $4304               ; $00:8680: 8C 04 43
LDA $00                 ; $00:8683: A5 00
STA $4305               ; $00:8685: 8D 05 43
LDY #$01                ; $00:8688: A0 01

; DMA stuff to RAM routine.
; Input: $7E0002 - 3 bytes - source address
; $7E0000 - 2 bytes - size
; A = destination RAM
; Y = flag for RAM $7F/$7E
; A = 16-bit. XY = 8-bit.
STY $420B               ; $00:868A: 8C 0B 42
RTL                     ; $00:868D: 6B

CODE_00868E:
STZ $02                 ; $00:868E: 64 02

CODE_008690:
STA $4302               ; $00:8690: 8D 02 43
STY $4304               ; $00:8693: 8C 04 43
LDA #$3480              ; $00:8696: A9 80 34
STA $4300               ; $00:8699: 8D 00 43
LDA $00                 ; $00:869C: A5 00
STA $4305               ; $00:869E: 8D 05 43
LDY #$01                ; $00:86A1: A0 01
...
; Y: Dest bank
; $00: DMA Size
; $02: 16-bit value to transfer (usually $0000 or $FFFF)
RTL                     ; $00:86B4: 6B

CODE_0086B5:
LDA $4016               ; $00:86B5: AD 16 40
AND #$01                ; $00:86B8: 29 01
EOR #$01                ; $00:86BA: 49 01
ASL A                   ; $00:86BC: 0A

; NES-styled Joypad data 1
; Read if the latch is open
; Reverse the latch status
; Multiply by 2
; Store into SRAM
STA $701FF4             ; $00:86BD: 8F F4 1F 70
...
CODE_0086CC:
STZ $4016               ; $00:86CC: 9C 16 40

; Something related to controllers
LDA $4218               ; $00:86CF: AD 18 42
...
CODE_008719:
PHD                     ; $00:8719: 0B
REP #$30                ; $00:871A: C2 30
LDA #$0800              ; $00:871C: A9 00 08

; Prepare OAM for the logo, menu, hall screen etc.
; Direct page: $0800
TCD                     ; $00:871F: 5B
...
STA $14,x               ; $00:8731: 95 14
STA $18,x               ; $00:8733: 95 18
...
; Basically this sticks 2 bits of 4 bytes into 1 byte
; And stores it to OAM $0A00-$0A1F for the tilesize
PLD                     ; $00:87CA: 2B
RTS                     ; $00:87CB: 60

CODE_0087CC:
LDA $0062               ; $00:87CC: AD 62 00
...
CODE_0087E8:
STA $2140               ; $00:87E8: 8D 40 21
STA $0064               ; $00:87EB: 8D 64 00

CODE_0087EE:
LDA $0061               ; $00:87EE: AD 61 00
...
CODE_0087FB:
STA $2141               ; $00:87FB: 8D 41 21
STA $0065               ; $00:87FE: 8D 65 00

CODE_008801:
LDA $0063               ; $00:8801: AD 63 00
...
CODE_00880E:
STA $2143               ; $00:880E: 8D 43 21
STA $0067               ; $00:8811: 8D 67 00

CODE_008814:
STZ $0060               ; $00:8814: 9C 60 00
...
ForceBlank:
LDA #$80                ; $00:882E: A9 80
STA $2100               ; $00:8830: 8D 00 21
STA $0100               ; $00:8833: 8D 00 01
STZ $420C               ; $00:8836: 9C 0C 42
STZ $0120               ; $00:8839: 9C 20 01

; Enable forced blank & Disable HDMA
; Forced blank
; Forced blank on mirror too
; HDMA disable
; HDMA disable on mirror too
RTL                     ; $00:883C: 6B
...
; 16-bit XY
REP #$10                ; $00:88C9: C2 10
LDX #$1801              ; $00:88CB: A2 01 18

; Base register: $2118. 2 regs write once.
STX $00                 ; $00:88CE: 86 00
LDA #$80                ; $00:88D0: A9 80

; VRAM:
; increment after writing $2119/reading $213A
STA $2115               ; $00:88D2: 8D 15 21
LDA $0219               ; $00:88D5: AD 19 02
BEQ CODE_0088FD         ; $00:88D8: F0 23
LDX #$6C00              ; $00:88DA: A2 00 6C
STX $2116               ; $00:88DD: 8E 16 21
LDX.w #DATA_2CD800      ; $00:88E0: A2 00 D8
STX $02                 ; $00:88E3: 86 02
LDA.b #DATA_2CD800>>16  ; $00:88E5: A9 2C
STA $04                 ; $00:88E7: 85 04
LDX #$1000              ; $00:88E9: A2 00 10
STX $05                 ; $00:88EC: 86 05
LDA #$01                ; $00:88EE: A9 01
STA $420B               ; $00:88F0: 8D 0B 42

; Flag to do file select images DMA. If it is set:
; Do file select images DMA.
; VRAM Destination: $D800
;
; DMA Source: $2C:D800
;
; Size: 4kB
...
;
;
; Reset "file select images DMA" flag
STZ $0219               ; $00:88F3: 9C 19 02
STZ $00E5               ; $00:88F6: 9C E5 00

; Clear offset of fading out into game select triangle
; graphics.
STZ $00E6               ; $00:88F9: 9C E6 00
RTS                     ; $00:88FC: 60

CODE_0088FD:
LDX $00E5               ; $00:88FD: AE E5 00
BEQ CODE_008926         ; $00:8900: F0 24
STX $02                 ; $00:8902: 86 02
LDA #$02                ; $00:8904: A9 02
STA $04                 ; $00:8906: 85 04

; If we're fading out into the game select room
;
; DMA source: bank #$02, offset that $00E5 has
LDX #$5000              ; $00:8908: A2 00 50
LDA $00E0               ; $00:890B: AD E0 00

; If fading out, VRAM address is $A000, else, $E000
...
CODE_008913:
STX $2116               ; $00:8913: 8E 16 21

; |DMA size: $800 bytes
LDY #$0800              ; $00:8916: A0 00 08
...
CODE_008926:
LDA $00DE               ; $00:8926: AD DE 00
BEQ CODE_008947         ; $00:8929: F0 1C
LDX #$7800              ; $00:892B: A2 00 78
STX $2116               ; $00:892E: 8E 16 21
LDX #$0000              ; $00:8931: A2 00 00
STX $02                 ; $00:8934: 86 02
LDA #$7F                ; $00:8936: A9 7F
STA $04                 ; $00:8938: 85 04
LDY #$0800              ; $00:893A: A0 00 08
STY $05                 ; $00:893D: 84 05
LDA #$01                ; $00:893F: A9 01
STA $420B               ; $00:8941: 8D 0B 42

; If character animation on hall screen flag is set
;
; VRAM address: $F000
;
; Update screen's tilemap using ram $7F0000 AKA animate stuff
; Source: $7F0000
;
;
;
...
;
; Enable DMA transfer
; Reset character animation on hall screen flag
STZ $00DE               ; $00:8944: 9C DE 00

CODE_008947:
LDA $0AC0               ; $00:8947: AD C0 0A
BEQ CODE_00896A         ; $00:894A: F0 1E
STZ $2121               ; $00:894C: 9C 21 21
LDY #$2200              ; $00:894F: A0 00 22
STY $00                 ; $00:8952: 84 00
LDY #$9200              ; $00:8954: A0 00 92
STY $02                 ; $00:8957: 84 02
LDA #$7F                ; $00:8959: A9 7F
STA $04                 ; $00:895B: 85 04
LDY #$0200              ; $00:895D: A0 00 02
STY $05                 ; $00:8960: 84 05
LDA #$01                ; $00:8962: A9 01
STA $420B               ; $00:8964: 8D 0B 42

; Dynamically upload palette
; If $0AC0 is set, upload full palette to CGRAM
; located at Source: $7F:9200.
; One reg write once. Reg = $2122
;
; Source: $7F:9200
;
;
...
CODE_00896A:
REP #$20                ; $00:896A: C2 20
SEP #$10                ; $00:896C: E2 10
STZ $2102               ; $00:896E: 9C 02 21
LDA #$0400              ; $00:8971: A9 00 04
STA $00                 ; $00:8974: 85 00
LDA #$0800              ; $00:8976: A9 00 08
STA $02                 ; $00:8979: 85 02
STZ $04                 ; $00:897B: 64 04
LDA #$0220              ; $00:897D: A9 20 02
STA $05                 ; $00:8980: 85 05
LDY #$01                ; $00:8982: A0 01

; Update OAM using DMA
;
; OAM address: $0000?
; \1 reg write once
; /OAM data DMA
...
;
;
; Indirect pointer $7F8002 at $E8-$EA
RTS                     ; $00:89B9: 60

CODE_0089BA:
STA $2117               ; $00:89BA: 8D 17 21
INY                     ; $00:89BD: C8
...
CODE_0089F9:
SEI                     ; $00:89F9: 78
STZ $4200               ; $00:89FA: 9C 00 42

; Disable interrupts
; Disable HDMA
STZ $420C               ; $00:89FD: 9C 0C 42
STZ $2140               ; $00:8A00: 9C 40 21
STZ $2142               ; $00:8A03: 9C 42 21
STZ $2143               ; $00:8A06: 9C 43 21
LDA #$FF                ; $00:8A09: A9 FF

; Disable SFX and music stuff
...
CODE_008A4F:
REP #$20                ; $00:8A4F: C2 20
LDA #$0400              ; $00:8A51: A9 00 04
STA $00                 ; $00:8A54: 85 00
LDA.w #DATA_07FC00      ; $00:8A56: A9 00 FC
STA $02                 ; $00:8A59: 85 02
LDY.b #DATA_07FC00>>16  ; $00:8A5B: A0 07
STY $04                 ; $00:8A5D: 84 04
LDA #$0000              ; $00:8A5F: A9 00 00
LDY #$7F                ; $00:8A62: A0 7F

; DMA SPC-700 data to RAM, part 1 of 2
; Source: $07FC00
; Size: $0400 bytes
; Destination: $7F0000
JSL CODE_00866D         ; $00:8A64: 22 6D 86 00
LDA #$8000              ; $00:8A68: A9 00 80
STA $00                 ; $00:8A6B: 85 00
LDA.w #DATA_3B8000      ; $00:8A6D: A9 00 80
STA $02                 ; $00:8A70: 85 02
LDY.b #DATA_3B8000>>16  ; $00:8A72: A0 3B
STY $04                 ; $00:8A74: 84 04
LDA #$0400              ; $00:8A76: A9 00 04
LDY #$7F                ; $00:8A79: A0 7F
JSL CODE_00866D         ; $00:8A7B: 22 6D 86 00

; DMA SPC-700 data to RAM, part 2 of 2
; Source: $3B8000
; Size: $8000 bytes
; Destination: $7F0400
...
STA $01                 ; $00:8A85: 85 01
LDA #$7F                ; $00:8A87: A9 7F

; SPC-700 data source: $7F:0000
STA $02                 ; $00:8A89: 85 02

; Upload SPC engine + SMAS hall and menu audio data
JMP SPCUpload           ; $00:8A8B: 4C AC 8B
...
STA $01                 ; $00:8AA7: 85 01
LDA.b #DATA_0B8000>>16  ; $00:8AA9: A9 0B

; SPC-700 data source: $0B:8000
STA $02                 ; $00:8AAB: 85 02

; Upload SPC data
JSR SPCUpload           ; $00:8AAD: 20 AC 8B
...
; Don't let NMI interrupt the upload
STZ $4200               ; $00:8ABD: 9C 00 42

; No DMA
STZ $420C               ; $00:8AC0: 9C 0C 42

; No HDMA
STZ $420B               ; $00:8AC3: 9C 0B 42
STZ $2140               ; $00:8AC6: 9C 40 21
LDA #$FF                ; $00:8AC9: A9 FF
STA $2141               ; $00:8ACB: 8D 41 21
STZ $2142               ; $00:8ACE: 9C 42 21

; Tell SPC700 we are ready to upload data
STZ $2143               ; $00:8AD1: 9C 43 21
STZ $00                 ; $00:8AD4: 64 00
LDA.b #DATA_0B8000>>8   ; $00:8AD6: A9 80
STA $01                 ; $00:8AD8: 85 01
...
; Upload SPC data
JSR SPCUpload           ; $00:8ADE: 20 AC 8B
...
; Upload SPC-700 data
JSR SPCUpload           ; $00:8B5F: 20 AC 8B
RTL                     ; $00:8B62: 6B
...
STA $02                 ; $00:8B79: 85 02
LDY.b #DATA_07C000>>16  ; $00:8B7B: A0 07
STY $04                 ; $00:8B7D: 84 04
LDA #$0000              ; $00:8B7F: A9 00 00
...
; Upload SPC-700 data
JSR SPCUpload           ; $00:8B96: 20 AC 8B
REP #$20                ; $00:8B99: C2 20
LDA #$C000              ; $00:8B9B: A9 00 C0
STA $00                 ; $00:8B9E: 85 00
...
; index to SPC data
LDY #$0000              ; $00:8BAF: A0 00 00
LDA #$BBAA              ; $00:8BB2: A9 AA BB

CODE_008BB5:
CMP $2140               ; $00:8BB5: CD 40 21

; Wait until SPC-700 is ready
; (it reaches ARAM $0fdc then)
BNE CODE_008BB5         ; $00:8BB8: D0 FB
SEP #$20                ; $00:8BBA: E2 20
LDA #$CC                ; $00:8BBC: A9 CC

; Tell SPC-700 that you're ready, and begin
; uploading data.
BRA CODE_008BE6         ; $00:8BBE: 80 26

CODE_008BC0:
LDA [$00],y             ; $00:8BC0: B7 00
INY                     ; $00:8BC2: C8
XBA                     ; $00:8BC3: EB
...
CODE_008BCD:
CMP $2140               ; $00:8BCD: CD 40 21
BNE CODE_008BCD         ; $00:8BD0: D0 FB
INC A                   ; $00:8BD2: 1A

CODE_008BD3:
REP #$20                ; $00:8BD3: C2 20
...
CODE_008BDD:
CMP $2140               ; $00:8BDD: CD 40 21
BNE CODE_008BDD         ; $00:8BE0: D0 FB

CODE_008BE2:
ADC #$03                ; $00:8BE2: 69 03
BEQ CODE_008BE2         ; $00:8BE4: F0 FC

; SPC-700 data header consists of 2 words/4 bytes

CODE_008BE6:
PHA                     ; $00:8BE6: 48
REP #$20                ; $00:8BE7: C2 20

; This loads the first word:
LDA [$00],y             ; $00:8BE9: B7 00

; Length of SPC-700 data in hex (little-endian)
INY                     ; $00:8BEB: C8
INY                     ; $00:8BEC: C8
TAX                     ; $00:8BED: AA

; This loads the second word:
LDA [$00],y             ; $00:8BEE: B7 00

; Where in ARAM the SPC-700 should be uploaded to
INY                     ; $00:8BF0: C8
...
CODE_008C1B:
SEP #$20                ; $00:8C1B: E2 20
LDA #$00                ; $00:8C1D: A9 00

; Clear debug mode
STA $700007             ; $00:8C1F: 8F 07 00 70
...
CODE_008C4C:
JSR CODE_008D83         ; $00:8C4C: 20 83 8D
...
CODE_008C6E:
LDA #$0000              ; $00:8C6E: A9 00 00
STA $700004             ; $00:8C71: 8F 04 00 70
TAX                     ; $00:8C75: AA

CODE_008C76:
AND #$0003              ; $00:8C76: 29 03 00
ASL A                   ; $00:8C79: 0A
TAY                     ; $00:8C7A: A8
...
CODE_008C87:
LDA $700010,x           ; $00:8C87: BF 10 00 70
CLC                     ; $00:8C8B: 18
...
CODE_008CA6:
LDA $700004             ; $00:8CA6: AF 04 00 70
INC A                   ; $00:8CAA: 1A
...
CODE_008CC1:
JMP CODE_008D80         ; $00:8CC1: 4C 80 8D
...
CODE_008D80:
SEP #$30                ; $00:8D80: E2 30
RTS                     ; $00:8D82: 60

CODE_008D83:
LDA $700004             ; $00:8D83: AF 04 00 70
...
CODE_008D9B:
LDA DATA_008DE3,y       ; $00:8D9B: B9 E3 8D
...
DATA_008DD3:
db $00,$00,$09,$00,$11,$00,$1A,$00

DATA_008DDB:
db $09,$00,$08,$00,$09,$00,$06,$01

DATA_008DE3:
db $FF,$FF,$FF,$04,$04,$00,$FF,$00
...
CODE_008F75:
BRA CODE_008FB6         ; $00:8F75: 80 3F

CODE_008F77:
LDA $0781               ; $00:8F77: AD 81 07
...
CODE_008FB6:
JSR CODE_0092F6         ; $00:8FB6: 20 F6 92
...
CODE_008FD5:
JSR CODE_0092F6         ; $00:8FD5: 20 F6 92
TYA                     ; $00:8FD8: 98
...
CODE_008FE3:
JSR CODE_0092F6         ; $00:8FE3: 20 F6 92
PHX                     ; $00:8FE6: DA
...
CODE_009044:
JSR CODE_0092F6         ; $00:9044: 20 F6 92
TYA                     ; $00:9047: 98
...
CODE_009052:
JSR CODE_0092F6         ; $00:9052: 20 F6 92
PHX                     ; $00:9055: DA
...
CODE_0090D2:
LDA $700010,x           ; $00:90D2: BF 10 00 70
...
CODE_0090FB:
LDA $075A               ; $00:90FB: AD 5A 07
BPL CODE_009102         ; $00:90FE: 10 02
LDA #$04                ; $00:9100: A9 04

CODE_009102:
JSR CODE_0092F6         ; $00:9102: 20 F6 92
PHX                     ; $00:9105: DA
...
CODE_00919C:
LDA $700011,x           ; $00:919C: BF 11 00 70
CMP $03                 ; $00:91A0: C5 03
BCC CODE_0091B4         ; $00:91A2: 90 10

CODE_0091A4:
LDA $700010,x           ; $00:91A4: BF 10 00 70
...
CODE_0092C3:
LDA $700010,x           ; $00:92C3: BF 10 00 70
CLC                     ; $00:92C7: 18
...
CODE_0092F6:
STA $700010,x           ; $00:92F6: 9F 10 00 70
INX                     ; $00:92FA: E8
CLC                     ; $00:92FB: 18
...
; Text used by this error message:
; "THIS GAME PAK IS NOT
; DESIGINED FOR YOUR SUPER
; FAMICOM OR SUPER NES.
; NINTENDO CO.,LTD."

DATA_009307:
db $3B,$00,$1D,$00,$11,$00,$12,$00
...
DATA_009367:
db $0F,$00,$0A,$00,$16,$00,$12,$00
db $0C,$00,$18,$00,$16,$00,$3B,$00
db $18,$00,$1B,$00,$3B,$00,$1C,$00
db $1E,$00,$19,$00,$0E,$00,$1B,$00
db $3B,$00,$17,$00,$0E,$00,$1C,$00
db $26,$00,$3B,$00,$3B,$00,$3B,$00

DATA_009397:
db $3B,$00,$3B,$00,$3B,$00,$17,$00
...
CODE_0093C7:
JSR CODE_0083B9         ; $00:93C7: 20 B9 83

; 8-bit A
SEP #$20                ; $00:93CA: E2 20

; Push direct page
PHD                     ; $00:93CC: 0B

; CGRAM 'pointer': Palette 00
STZ $2121               ; $00:93CD: 9C 21 21

; 16-bit A
REP #$20                ; $00:93D0: C2 20
...
LDA #$4300              ; $00:93E2: A9 00 43

; Direct page: $43xx
TCD                     ; $00:93E5: 5B
LDY #$80                ; $00:93E6: A0 80

; increment after writing $2119/reading $213A
STY $2115               ; $00:93E8: 8C 15 21
LDA #$7FFF              ; $00:93EB: A9 FF 7F
STA $0218               ; $00:93EE: 8D 18 02
LDA #$7FF9              ; $00:93F1: A9 F9 7F
STA $021A               ; $00:93F4: 8D 1A 02
LDA #$7FD0              ; $00:93F7: A9 D0 7F
STA $021C               ; $00:93FA: 8D 1C 02

; 4 colors which are white, and 3 blue tints
; They are located at palette 00
; Colors $0C, $0D, $0E, $0F
LDA #$6AE9              ; $00:93FD: A9 E9 6A
STA $021E               ; $00:9400: 8D 1E 02
LDA #$2200              ; $00:9403: A9 00 22

; Base register: $2122
; Writing mode: 1 regs write once
STA $00                 ; $00:9406: 85 00
LDA #$0200              ; $00:9408: A9 00 02
...
; DMA Size: 0x200/512 bytes.
STA $05                 ; $00:9411: 85 05
LDY #$01                ; $00:9413: A0 01

; Enable DMA channel 1
STY $420B               ; $00:9415: 8C 0B 42
LDA #$1801              ; $00:9418: A9 01 18

; Base register: $2118
; Writing mode: 2 regs write once
STA $00                 ; $00:941B: 85 00
LDA #$0000              ; $00:941D: A9 00 00

; VRAM Address: $0000
STA $2116               ; $00:9420: 8D 16 21
LDA.w #DATA_199800      ; $00:9423: A9 00 98
STA $02                 ; $00:9426: 85 02
LDX.b #DATA_199800>>16  ; $00:9428: A2 19

; DMA Source: $19:9800.
; (SMB2 font GFX is located there)
STX $04                 ; $00:942A: 86 04
LDA #$0800              ; $00:942C: A9 00 08

; DMA Size:
; 0x800 bytes/2kB
STA $05                 ; $00:942F: 85 05

; Enable DMA channel
STY $420B               ; $00:9431: 8C 0B 42
PHB                     ; $00:9434: 8B
LDX #$00                ; $00:9435: A2 00
PHX                     ; $00:9437: DA
...
; Load loop stuff.
LDX #$00                ; $00:9439: A2 00

CODE_00943B:
LDA.w DATA_009307,x     ; $00:943B: BD 07 93
STA $09CA,x             ; $00:943E: 9D CA 09
LDA.w DATA_009337,x     ; $00:9441: BD 37 93
STA $0A4A,x             ; $00:9444: 9D 4A 0A
LDA.w DATA_009367,x     ; $00:9447: BD 67 93
STA $0ACA,x             ; $00:944A: 9D CA 0A
LDA.w DATA_009397,x     ; $00:944D: BD 97 93

; Store all the text to RAM
STA $0C4A,x             ; $00:9450: 9D 4A 0C
INX                     ; $00:9453: E8
INX                     ; $00:9454: E8
CPX #$30                ; $00:9455: E0 30

; If not done yet (uploading 48 * 4 tiles)
;
; Loop
BNE CODE_00943B         ; $00:9457: D0 E2

; Recover program bank.

CODE_009459:
PLB                     ; $00:9459: AB
LDA #$1000              ; $00:945A: A9 00 10

; VRAM address: $1000
STA $2116               ; $00:945D: 8D 16 21
LDA #$0800              ; $00:9460: A9 00 08
STA $02                 ; $00:9463: 85 02
LDX #$00                ; $00:9465: A2 00

; DMA Source: $00:0800
STX $04                 ; $00:9467: 86 04

; DMA Size: 0x800 bytes/2kB
STA $05                 ; $00:9469: 85 05

; Activate DMA channel 1
STY $420B               ; $00:946B: 8C 0B 42

; 8-bit A
SEP #$20                ; $00:946E: E2 20

; Recover direct page.
PLD                     ; $00:9470: 2B
LDA #$10                ; $00:9471: A9 10

; BG1 tilemap address: $4000
STA $2107               ; $00:9473: 8D 07 21

; BG1 char address: $0000
STZ $210B               ; $00:9476: 9C 0B 21
LDA #$01                ; $00:9479: A9 01
STA $212C               ; $00:947B: 8D 2C 21

; Enable BG1 on main screen
; nothing on subscreen
STZ $212D               ; $00:947E: 9C 2D 21
LDA #$01                ; $00:9481: A9 01

; Mode 1, 8x8 tiles
STA $2105               ; $00:9483: 8D 05 21

; No color math?
STZ $2130               ; $00:9486: 9C 30 21

; No CGADSUB
STZ $2131               ; $00:9489: 9C 31 21
STZ $210D               ; $00:948C: 9C 0D 21
STZ $210D               ; $00:948F: 9C 0D 21
STZ $210E               ; $00:9492: 9C 0E 21
...
; NO BG1/2/3 Horizontal and vertical scroll
STZ $2112               ; $00:94AD: 9C 12 21
LDA #$0F                ; $00:94B0: A9 0F

; Maximum brightness
STA $2100               ; $00:94B2: 8D 00 21

CODE_0094B5:
BIT $4212               ; $00:94B5: 2C 12 42

; Wait for VBlank
BPL CODE_0094B5         ; $00:94B8: 10 FB
INC $FD                 ; $00:94BA: E6 FD
LDA $FD                 ; $00:94BC: A5 FD
CMP #$18                ; $00:94BE: C9 18

; From now on everything from here will execute
; on a vblank. Increase frame counter
; If it isn't $18 yet
; branch
BNE CODE_009501         ; $00:94C0: D0 3F

; Store zero to the something
STZ $FD                 ; $00:94C2: 64 FD

; CGRAM Palette destination: 00
STZ $2121               ; $00:94C4: 9C 21 21

; 16-bit A
REP #$20                ; $00:94C7: C2 20
LDA #$4300              ; $00:94C9: A9 00 43

; Direct page:
; $43xx
TCD                     ; $00:94CC: 5B
LDY #$80                ; $00:94CD: A0 80

; increment after writing $2119/reading $213A
STY $2115               ; $00:94CF: 8C 15 21
LDA $021A               ; $00:94D2: AD 1A 02
PHA                     ; $00:94D5: 48
LDA $021C               ; $00:94D6: AD 1C 02
STA $021A               ; $00:94D9: 8D 1A 02
LDA $021E               ; $00:94DC: AD 1E 02
STA $021C               ; $00:94DF: 8D 1C 02
PLA                     ; $00:94E2: 68
...
; load last color
; overwrite 3rd color
; Pull 2nd color from A
; Store into last color
STA $021E               ; $00:94E3: 8D 1E 02
LDA #$2200              ; $00:94E6: A9 00 22
STA $00                 ; $00:94E9: 85 00
LDA #$0200              ; $00:94EB: A9 00 02
STA $02                 ; $00:94EE: 85 02
LDX #$00                ; $00:94F0: A2 00

; Base register: $2122. 1 reg write once
;
;
; Source address: $00:0200
STX $04                 ; $00:94F2: 86 04

; DMA Size: 512 bytes.
STA $05                 ; $00:94F4: 85 05
LDY #$01                ; $00:94F6: A0 01
...
; Recover direct page. 8-bit A
SEP #$20                ; $00:94FF: E2 20

CODE_009501:
BIT $4212               ; $00:9501: 2C 12 42

; If you have Vblank, don't continue
; if out of Vblank then continue with...
BMI CODE_009501         ; $00:9504: 30 FB

; ...Loop forever..... evil coding
JMP CODE_0094B5         ; $00:9506: 4C B5 94
...
; No interrupts
STZ $4200               ; $00:9642: 9C 00 42

; No HDMA
STZ $420C               ; $00:9645: 9C 0C 42

; No DMA
STZ $420B               ; $00:9648: 9C 0B 42
LDA #$80                ; $00:964B: A9 80

; Forced blank
STA $2100               ; $00:964D: 8D 00 21
STZ $2140               ; $00:9650: 9C 40 21
STZ $2141               ; $00:9653: 9C 41 21
...
; Fade out the music
STA $2142               ; $00:965B: 8D 42 21
JSR CODE_0083B9         ; $00:965E: 20 B9 83

; 16-bit A
SEP #$20                ; $00:9661: E2 20

; Push direct page
PHD                     ; $00:9663: 0B

; Palette number: $00
STZ $2121               ; $00:9664: 9C 21 21

; 16-bit A
REP #$20                ; $00:9667: C2 20
...
LDA #$4300              ; $00:9679: A9 00 43

; Direct page: $43xx
TCD                     ; $00:967C: 5B
LDY #$80                ; $00:967D: A0 80

; increment after writing $2119/reading $213A
STY $2115               ; $00:967F: 8C 15 21
LDA #$7FFF              ; $00:9682: A9 FF 7F
STA $0218               ; $00:9685: 8D 18 02
LDA #$7FF9              ; $00:9688: A9 F9 7F
STA $021A               ; $00:968B: 8D 1A 02
LDA #$7FD0              ; $00:968E: A9 D0 7F
...
LDA #$2200              ; $00:969A: A9 00 22

; 1 reg write once. base reg = $2122
; CGRAM write reg
STA $00                 ; $00:969D: 85 00
LDA #$0200              ; $00:969F: A9 00 02
STA $02                 ; $00:96A2: 85 02
...
; DMA size: 0x200/512 bytes.
STA $05                 ; $00:96A8: 85 05
LDY #$01                ; $00:96AA: A0 01

; Enable DMA channel 1
STY $420B               ; $00:96AC: 8C 0B 42
LDA #$1801              ; $00:96AF: A9 01 18

; Base reg: $2118: VRAM Write
; 2 regs write once
STA $00                 ; $00:96B2: 85 00
LDA #$0000              ; $00:96B4: A9 00 00

; VRAM Address: $0000
STA $2116               ; $00:96B7: 8D 16 21
LDA.w #DATA_199800      ; $00:96BA: A9 00 98
STA $02                 ; $00:96BD: 85 02
LDX.b #DATA_199800>>16  ; $00:96BF: A2 19

; DMA Source: $19:9800
; SMB2 font GFX
STX $04                 ; $00:96C1: 86 04
LDA #$0800              ; $00:96C3: A9 00 08

; DMA Size: 0x800 bytes/2kB
STA $05                 ; $00:96C6: 85 05

; Enable DMA channel 1
STY $420B               ; $00:96C8: 8C 0B 42
PHB                     ; $00:96CB: 8B
LDX #$00                ; $00:96CC: A2 00
PHX                     ; $00:96CE: DA
...
; Upload the 'pirated' message to RAM
STA $0CC6,x             ; $00:96F3: 9D C6 0C
INX                     ; $00:96F6: E8
INX                     ; $00:96F7: E8
CPX #$34                ; $00:96F8: E0 34

; If not done yet, continue uploading.
BNE CODE_0096D2         ; $00:96FA: D0 D6

; Continue updating palette animation and loop endlessly.
JMP CODE_009459         ; $00:96FC: 4C 59 94

; Values in a decrementing fashion
...
; Used in "peach saved" sequence at the
dw $4000,$3333,$2AAA,$2492

; end of SMB1 and SMB:TLL for the windowing HDMA
...
; $00 = Set up things for Nintendo Presents Screen

PNTR_009B80:
dw CODE_009C3A

; $01 = Fade into Nintendo Presents Screen
dw CODE_009C7A

; $02 = Show Nintendo Presents Screen
dw CODE_009C8A

; $03 = Fade out of presents screen and setup title screen
dw CODE_009CB6

; $04 = Fade into title screen
dw CODE_009D48

; $05 = Title screen (lights out and everyone is talking)
dw CODE_009D5E
...
; $0A = Get some SPC data uploaded and play title screen music
dw CODE_009EF9
...
; $1C = Title screen (almost time to change)
dw CODE_009F6D

; $1D = Fade out to gameplay example
dw CODE_009F85

; $1E = Wait a bit and play "fade out" SFX
dw CODE_009F9C

; $1F = Triangle fade out routine
dw CODE_009FF1

CODE_009BC0:
LDA $E1                 ; $00:9BC0: A5 E1
ASL A                   ; $00:9BC2: 0A
TAX                     ; $00:9BC3: AA
...
CMP #$04                ; $00:9BC9: C9 04

; If we are not on the title screen yet, branch
BCC CODE_009C39         ; $00:9BCB: 90 6C

; Show "Super Mario-All Stars" and Nintendo copyright text
JSR CODE_00A0E3         ; $00:9BCD: 20 E3 A0
LDA $E1                 ; $00:9BD0: A5 E1
CMP #$05                ; $00:9BD2: C9 05

; If we are not actually displaying the title screen yet, branch
...
; Update CGRAM
INC $0AC0               ; $00:9C19: EE C0 0A

; Move BG1 tilemap to RAM
JSR CODE_00B683         ; $00:9C1C: 20 83 B6
LDA #$50                ; $00:9C1F: A9 50
STA $E3                 ; $00:9C21: 85 E3

; Set timer
STZ $E4                 ; $00:9C23: 64 E4
LDA #$0C                ; $00:9C25: A9 0C
STA $22                 ; $00:9C27: 85 22
...
LDA #$11                ; $00:9C2F: A9 11

; Mute music
STA $0060               ; $00:9C31: 8D 60 00
LDA #$29                ; $00:9C34: A9 29
...
CODE_009C3A:
LDA #$00                ; $00:9C3A: A9 00
STA $701FF2             ; $00:9C3C: 8F F2 1F 70
STZ $E7                 ; $00:9C40: 64 E7
REP #$20                ; $00:9C42: C2 20
LDA #$0200              ; $00:9C44: A9 00 02

; Size
STA $00                 ; $00:9C47: 85 00
LDA.w #DATA_02CA00      ; $00:9C49: A9 00 CA
LDY.b #DATA_02CA00>>16  ; $00:9C4C: A0 02
STA $02                 ; $00:9C4E: 85 02

; Source: $02:CA00
STY $04                 ; $00:9C50: 84 04
LDA #$9200              ; $00:9C52: A9 00 92

; RAM: $7F:9200
LDY #$7F                ; $00:9C55: A0 7F

; Upload to RAM
JSL CODE_00866D         ; $00:9C57: 22 6D 86 00
LDA #$0000              ; $00:9C5B: A9 00 00
STA $7F9000             ; $00:9C5E: 8F 00 90 7F

; Make the first two colors black
STA $7F9200             ; $00:9C62: 8F 00 92 7F
SEP #$20                ; $00:9C66: E2 20

; Update CGRAM
INC $0AC0               ; $00:9C68: EE C0 0A
LDA #$10                ; $00:9C6B: A9 10

; Sprites on main screen
...
; Next sub-game mode
INC $E1                 ; $00:9C78: E6 E1

; Draw sprite

CODE_009C7A:
JSR CODE_00A02B         ; $00:9C7A: 20 2B A0

; Increase screen brightness
INC $0100               ; $00:9C7D: EE 00 01
LDA $0100               ; $00:9C80: AD 00 01
CMP #$0F                ; $00:9C83: C9 0F

; Branch if we haven't reached max brightness
BNE CODE_009C89         ; $00:9C85: D0 02

; Next sub-game mode
INC $E1                 ; $00:9C87: E6 E1

CODE_009C89:
RTS                     ; $00:9C89: 60

CODE_009C8A:
LDA $1C                 ; $00:9C8A: A5 1C
CMP #$61                ; $00:9C8C: C9 61

; If the presents timer reaches this value, branch
BNE CODE_009C9D         ; $00:9C8E: D0 0D
LDA #$01                ; $00:9C90: A9 01
STA $0063               ; $00:9C92: 8D 63 00
STA $24                 ; $00:9C95: 85 24
LDA #$02                ; $00:9C97: A9 02
STA $22                 ; $00:9C99: 85 22

; Blink SFX
;
; Set blinking timer to #$02
STZ $23                 ; $00:9C9B: 64 23

; Keep the sprite drawn

CODE_009C9D:
JSR CODE_00A02B         ; $00:9C9D: 20 2B A0
LDA $24                 ; $00:9CA0: A5 24

; If it is not time to blink, branch
BEQ CODE_009CA7         ; $00:9CA2: F0 03
JSR CODE_00A0A6         ; $00:9CA4: 20 A6 A0

CODE_009CA7:
DEC $1C                 ; $00:9CA7: C6 1C

; Decrease presents timer
; If it is not zero, return
BNE CODE_009C89         ; $00:9CA9: D0 DE

; Next sub-game mode
INC $E1                 ; $00:9CAB: E6 E1
...
STA $2116               ; $00:9CF7: 8D 16 21
LDA #$1801              ; $00:9CFA: A9 01 18
STA $4300               ; $00:9CFD: 8D 00 43
LDA.w #DATA_02A000      ; $00:9D00: A9 00 A0
STA $4302               ; $00:9D03: 8D 02 43
...
LDA #$3E                ; $00:9D3B: A9 3E

; Play chatting noise
STA $0060               ; $00:9D3D: 8D 60 00

; Update CGRAM
INC $0AC0               ; $00:9D40: EE C0 0A

; Screen brightness = 0
STZ $0100               ; $00:9D43: 9C 00 01

; Next subgame mode
INC $E1                 ; $00:9D46: E6 E1

; Fade into title screen

CODE_009D48:
INC $0100               ; $00:9D48: EE 00 01
LDA $0100               ; $00:9D4B: AD 00 01
CMP #$0F                ; $00:9D4E: C9 0F

; If we have not reached max screen brightness branch
BNE CODE_009D5D         ; $00:9D50: D0 0B
REP #$20                ; $00:9D52: C2 20
LDA #$011A              ; $00:9D54: A9 1A 01

; Set timer (amount of time until we change the palette/"turn the lights on")
STA $E3                 ; $00:9D57: 85 E3
SEP #$20                ; $00:9D59: E2 20

; Next sub-game mode
INC $E1                 ; $00:9D5B: E6 E1

CODE_009D5D:
RTS                     ; $00:9D5D: 60

CODE_009D5E:
REP #$20                ; $00:9D5E: C2 20
LDA $E3                 ; $00:9D60: A5 E3
CMP #$0020              ; $00:9D62: C9 20 00

; If the timer is < $0020 branch
BCC CODE_009D6C         ; $00:9D65: 90 05
SEP #$20                ; $00:9D67: E2 20

; Make everyone talk
JSR CODE_00B63F         ; $00:9D69: 20 3F B6

CODE_009D6C:
REP #$20                ; $00:9D6C: C2 20
DEC $E3                 ; $00:9D6E: C6 E3

; If it is not time to "turn the lights on", return
BNE CODE_009DC3         ; $00:9D70: D0 51
SEP #$20                ; $00:9D72: E2 20

; Upload BG1 tilemap
JSR CODE_00B683         ; $00:9D74: 20 83 B6
REP #$20                ; $00:9D77: C2 20
LDA #$0200              ; $00:9D79: A9 00 02
STA $00                 ; $00:9D7C: 85 00
LDA #$CA00              ; $00:9D7E: A9 00 CA
...
; Upload palette buffer to CGRAM
INC $0AC0               ; $00:9DA8: EE C0 0A
REP #$20                ; $00:9DAB: C2 20
LDA #$0445              ; $00:9DAD: A9 45 04

; Set amount of time left on title screen
...
; Next sub-game mode
INC $E1                 ; $00:9DBC: E6 E1
LDA #$11                ; $00:9DBE: A9 11

; Mute sound on this channel
STA $0060               ; $00:9DC0: 8D 60 00

CODE_009DC3:
SEP #$20                ; $00:9DC3: E2 20
RTS                     ; $00:9DC5: 60

CODE_009DC6:
JSR CODE_00BD8D         ; $00:9DC6: 20 8D BD
...
; Handle flashing "Super Mario All-Stars" text

CODE_009E16:
JSR CODE_00BD8D         ; $00:9E16: 20 8D BD
REP #$20                ; $00:9E19: C2 20
DEC $E3                 ; $00:9E1B: C6 E3

; If there is still time left on the title screen, branch
BNE CODE_009E59         ; $00:9E1D: D0 3A
LDA #$0200              ; $00:9E1F: A9 00 02
STA $00                 ; $00:9E22: 85 00
LDA #$CA00              ; $00:9E24: A9 00 CA
LDY #$02                ; $00:9E27: A0 02
...
LDA #$0000              ; $00:9E36: A9 00 00

; Make the first color black
STA $7F9200             ; $00:9E39: 8F 00 92 7F
LDA #$0004              ; $00:9E3D: A9 04 00

; Set timer
STA $E3                 ; $00:9E40: 85 E3
LDA #$0025              ; $00:9E42: A9 25 00
STA $011D               ; $00:9E45: 8D 1D 01
LDA #$004D              ; $00:9E48: A9 4D 00
...
; Update CGRAM
INC $0AC0               ; $00:9E54: EE C0 0A

; Next sub-game mode
INC $E1                 ; $00:9E57: E6 E1

CODE_009E59:
SEP #$20                ; $00:9E59: E2 20
RTS                     ; $00:9E5B: 60

; Move everyone

CODE_009E5C:
JSR CODE_00B63F         ; $00:9E5C: 20 3F B6
REP #$20                ; $00:9E5F: C2 20
DEC $E3                 ; $00:9E61: C6 E3

; Decrease timer
BNE CODE_009EA9         ; $00:9E63: D0 44
LDA #$0060              ; $00:9E65: A9 60 00
STA $22                 ; $00:9E68: 85 22
...
; Update CGRAM
INC $0AC0               ; $00:9EA4: EE C0 0A

; Next sub-game mode
INC $E1                 ; $00:9EA7: E6 E1

CODE_009EA9:
SEP #$20                ; $00:9EA9: E2 20
RTS                     ; $00:9EAB: 60

CODE_009EAC:
REP #$20                ; $00:9EAC: C2 20
DEC $E3                 ; $00:9EAE: C6 E3
...
LDA #$0000              ; $00:9EC9: A9 00 00

; Make the first color black
STA $7F9200             ; $00:9ECC: 8F 00 92 7F
LDA #$0002              ; $00:9ED0: A9 02 00
LDX $E1                 ; $00:9ED3: A6 E1
CPX #$13                ; $00:9ED5: E0 13

; Only mute the music after the last flash
BNE CODE_009EDE         ; $00:9ED7: D0 05
LDX #$11                ; $00:9ED9: A2 11
...
CODE_009EDE:
STA $E3                 ; $00:9EDE: 85 E3
SEP #$20                ; $00:9EE0: E2 20

; Update CGRAM
INC $0AC0               ; $00:9EE2: EE C0 0A

; Next sub-game mode
INC $E1                 ; $00:9EE5: E6 E1
LDA #$21                ; $00:9EE7: A9 21
STA $011D               ; $00:9EE9: 8D 1D 01
LDA #$49                ; $00:9EEC: A9 49
...
CODE_009EF6:
SEP #$20                ; $00:9EF6: E2 20
RTS                     ; $00:9EF8: 60

; Go upload some SPC data

CODE_009EF9:
JSR CODE_008A8E         ; $00:9EF9: 20 8E 8A
LDA #$01                ; $00:9EFC: A9 01
...
CODE_009F01:
INC $E1                 ; $00:9F01: E6 E1
RTS                     ; $00:9F03: 60

; Move everyone

CODE_009F04:
JSR CODE_00B63F         ; $00:9F04: 20 3F B6

; Upload SPC data
JSR CODE_008AE7         ; $00:9F07: 20 E7 8A
REP #$20                ; $00:9F0A: C2 20
LDA #$00FA              ; $00:9F0C: A9 FA 00
...
; Make "All-Stars" text flash

CODE_009F14:
JSR CODE_00BD8D         ; $00:9F14: 20 8D BD

; Move everyone
JSR CODE_00B63F         ; $00:9F17: 20 3F B6
REP #$20                ; $00:9F1A: C2 20
DEC $E3                 ; $00:9F1C: C6 E3
...
; Update CGRAM
INC $0AC0               ; $00:9F5F: EE C0 0A
LDA #$80                ; $00:9F62: A9 80
STA $0211               ; $00:9F64: 8D 11 02

; Go upload some SPC data
JSR CODE_008A8E         ; $00:9F67: 20 8E 8A

CODE_009F6A:
SEP #$20                ; $00:9F6A: E2 20
RTS                     ; $00:9F6C: 60

; Make "All-Stars" text flash

CODE_009F6D:
JSR CODE_00BD8D         ; $00:9F6D: 20 8D BD

; Move everyone
JSR CODE_00B62B         ; $00:9F70: 20 2B B6
REP #$20                ; $00:9F73: C2 20
DEC $E3                 ; $00:9F75: C6 E3
...
CODE_009F82:
SEP #$20                ; $00:9F82: E2 20
RTS                     ; $00:9F84: 60

CODE_009F85:
DEC $0100               ; $00:9F85: CE 00 01

; Decrease screen brightness until black
BNE CODE_009F82         ; $00:9F88: D0 F8
LDA #$00                ; $00:9F8A: A9 00
STA $2100               ; $00:9F8C: 8D 00 21

; screen brightness = 0
STA $0100               ; $00:9F8F: 8D 00 01

; Upload SPC data
JSR CODE_008ABC         ; $00:9F92: 20 BC 8A

; sub-game mode 0
STZ $E1                 ; $00:9F95: 64 E1
LDA #$09                ; $00:9F97: A9 09
...
CODE_009F9C:
JSR CODE_00BD8D         ; $00:9F9C: 20 8D BD
DEC $E3                 ; $00:9F9F: C6 E3

; Decrease timer
BNE CODE_009FAA         ; $00:9FA1: D0 07
LDA #$40                ; $00:9FA3: A9 40

; Play "fade out" SFX
STA $0063               ; $00:9FA5: 8D 63 00

; Next sub-game mode
INC $E1                 ; $00:9FA8: E6 E1

CODE_009FAA:
RTS                     ; $00:9FAA: 60

; Triangle fading graphic offsets. bank is hardcoded at #$02

DATA_009FAB:
dw $9000,$9020,$9040,$9060
...
CODE_009FF1:
LDA $FD                 ; $00:9FF1: A5 FD
LSR A                   ; $00:9FF3: 4A

; Return all odd frames
BCS CODE_00A014         ; $00:9FF4: B0 1E
REP #$20                ; $00:9FF6: C2 20

; Get which step we are on
LDX $E3                 ; $00:9FF8: A6 E3
LDA.l DATA_009FAB,x     ; $00:9FFA: BF AB 9F 00

; Set source data location to upload to BG2's tilemap in $E5
...
; Remeber it
STX $E3                 ; $00:A002: 86 E3
CPX #$46                ; $00:A004: E0 46

; If this isn't the last step branch
BNE CODE_00A014         ; $00:A006: D0 0C

; Next game mode
INC $E0                 ; $00:A008: E6 E0
STZ $E1                 ; $00:A00A: 64 E1

; Clear sub-game mode and timer
STZ $E3                 ; $00:A00C: 64 E3
LDA #$0080              ; $00:A00E: A9 80 00

; Force blank
STA $0100               ; $00:A011: 8D 00 01

CODE_00A014:
SEP #$20                ; $00:A014: E2 20
RTS                     ; $00:A016: 60
...
CODE_00A02B:
LDY #$00                ; $00:A02B: A0 00

; Clear registers X and Y
TYX                     ; $00:A02D: BB
LDA #$50                ; $00:A02E: A9 50

; #$50 to logo Y
STA $00                 ; $00:A030: 85 00

CODE_00A032:
LDA #$60                ; $00:A032: A9 60

; #$60 to logo X
STA $01                 ; $00:A034: 85 01

CODE_00A036:
LDA $01                 ; $00:A036: A5 01
STA $0800,y             ; $00:A038: 99 00 08
LDA $00                 ; $00:A03B: A5 00
...
;
;
; Write logo tiles size to OAM. $00 = 8x8, $02 = 16x16
;
;
;
;
...
;
;
; If not done yet, continue looping
BCC CODE_00A032         ; $00:A06F: 90 C1
RTS                     ; $00:A071: 60

; How long the 4 blinks of the presents logo lasts

DATA_00A072:
db $02,$02,$02,$02

; RGB values for the blink. Kinda hard to figure it out...
...
CODE_00A0A6:
LDA $23                 ; $00:A0A6: A5 23
ASL A                   ; $00:A0A8: 0A
STA $00                 ; $00:A0A9: 85 00
ASL A                   ; $00:A0AB: 0A
CLC                     ; $00:A0AC: 18
ADC $00                 ; $00:A0AD: 65 00

; Multiply $23 with 06 and move it to Y
TAY                     ; $00:A0AF: A8
LDX #$00                ; $00:A0B0: A2 00

CODE_00A0B2:
LDA DATA_00A076,y       ; $00:A0B2: B9 76 A0
STA $7F93EC,x           ; $00:A0B5: 9F EC 93 7F
LDA DATA_00A08E,y       ; $00:A0B9: B9 8E A0
STA $7F93FA,x           ; $00:A0BC: 9F FA 93 7F
INY                     ; $00:A0C0: C8
INX                     ; $00:A0C1: E8
CPX #$06                ; $00:A0C2: E0 06

; Do some logo-blinking stuff
BCC CODE_00A0B2         ; $00:A0C4: 90 EC
INC $0AC0               ; $00:A0C6: EE C0 0A
DEC $22                 ; $00:A0C9: C6 22

; Decrease blinking timer.
BNE CODE_00A0E2         ; $00:A0CB: D0 15
INC $23                 ; $00:A0CD: E6 23
LDX $23                 ; $00:A0CF: A6 23
CPX #$04                ; $00:A0D1: E0 04

; If not done yet with the blinking
; Load the next blink's duration
BCC CODE_00A0DD         ; $00:A0D3: 90 08
STZ $23                 ; $00:A0D5: 64 23
STZ $22                 ; $00:A0D7: 64 22
...
CODE_00A0DD:
LDA DATA_00A072,x       ; $00:A0DD: BD 72 A0
STA $22                 ; $00:A0E0: 85 22

; Load blinking duration, indexed by blink count

CODE_00A0E2:
RTS                     ; $00:A0E2: 60

CODE_00A0E3:
LDA #$48                ; $00:A0E3: A9 48
STA $00                 ; $00:A0E5: 85 00
LDY #$00                ; $00:A0E7: A0 00
TYX                     ; $00:A0E9: BB

CODE_00A0EA:
LDA $00                 ; $00:A0EA: A5 00
...
CODE_00A1A3:
SEP #$10                ; $00:A1A3: E2 10
PLB                     ; $00:A1A5: AB
RTS                     ; $00:A1A6: 60

; Super Mario All-Stars logo OAM tilemap
...
; Same purpose as above table!

DATA_00A248:
db $38,$18,$A0,$02,$40,$18,$A1,$02

; But these tiles are 8x8
...
CODE_00A2D5:
SEP #$20                ; $00:A2D5: E2 20
RTL                     ; $00:A2D7: 6B

CODE_00A2D8:
LDA $70000E             ; $00:A2D8: AF 0E 00 70


STA $D1                 ; $00:A2DC: 85 D1
LDA $70000F             ; $00:A2DE: AF 0F 00 70
STA $D0                 ; $00:A2E2: 85 D0

; Get the last game played
STA $0218               ; $00:A2E4: 8D 18 02
LDX #$01                ; $00:A2E7: A2 01

; Upload GFX and palette
JSL CODE_00849C         ; $00:A2E9: 22 9C 84 00

; Upload SPC data
JSR CODE_008ABC         ; $00:A2ED: 20 BC 8A
LDA #$0F                ; $00:A2F0: A9 0F

; Screen brightness
STA $0100               ; $00:A2F2: 8D 00 01

; Update CGRAM
INC $0AC0               ; $00:A2F5: EE C0 0A

; Next game mode
INC $E0                 ; $00:A2F8: E6 E0

; Sub-game mode 0
STZ $E1                 ; $00:A2FA: 64 E1
LDA #$44                ; $00:A2FC: A9 44

; Set timer
STA $E3                 ; $00:A2FE: 85 E3

; Reset the frame counter
STZ $FD                 ; $00:A300: 64 FD
JSR CODE_00A30D         ; $00:A302: 20 0D A3
LDA #$41                ; $00:A305: A9 41
...
CODE_00A30D:
REP #$10                ; $00:A30D: C2 10
LDY #$0000              ; $00:A30F: A0 00 00

CODE_00A312:
LDA #$00                ; $00:A312: A9 00
XBA                     ; $00:A314: EB
PHY                     ; $00:A315: 5A
TYA                     ; $00:A316: 98
ASL A                   ; $00:A317: 0A
...
CODE_00A341:
LDA #$FF                ; $00:A341: A9 FF

CODE_00A343:
STA $0090,y             ; $00:A343: 99 90 00
...
DATA_00A367:
dw $6127,$612D,$6133,$6139
dw $6187,$618D,$6193,$6199
dw $61E7,$61ED,$61F3,$61F9
dw $6247,$624D,$6253,$6259

DATA_00A387:
db $1F,$21,$22,$22,$64,$F0,$64,$F1
db $64,$F4,$64,$F5,$64,$F2,$64,$F3
db $64,$F6,$64,$F7

CODE_00A39B:
LDA $E1                 ; $00:A39B: A5 E1

; If we are running the game select screen branch
...
CODE_00A3F2:
SEP #$20                ; $00:A3F2: E2 20
RTL                     ; $00:A3F4: 6B

CODE_00A3F5:
LDA $0216               ; $00:A3F5: AD 16 02
...
ORA $F5                 ; $00:A40B: 05 F5
AND #$03                ; $00:A40D: 29 03

; If we are not pressing left or right branch
BEQ CODE_00A46B         ; $00:A40F: F0 5A
AND #$01                ; $00:A411: 29 01

; If we are pressing left, branch
BEQ CODE_00A441         ; $00:A413: F0 2C

; Get the current game we are on
LDA $0218               ; $00:A415: AD 18 02
AND #$03                ; $00:A418: 29 03
TAX                     ; $00:A41A: AA
LDA DATA_00A387,x       ; $00:A41B: BD 87 A3

; Move the correct amount
STA $0216               ; $00:A41E: 8D 16 02

; Next game
INC $0218               ; $00:A421: EE 18 02
LDA #$01                ; $00:A424: A9 01

; Play SFX
STA $0063               ; $00:A426: 8D 63 00

CODE_00A429:
DEC $0216               ; $00:A429: CE 16 02
...
; Same thing up there but the other way

CODE_00A441:
DEC $0218               ; $00:A441: CE 18 02
...
CODE_00A46B:
LDA $0216               ; $00:A46B: AD 16 02
ORA $0217               ; $00:A46E: 0D 17 02

; If we stopped moving, branch
BEQ CODE_00A476         ; $00:A471: F0 03

; Return
JMP CODE_00A771         ; $00:A473: 4C 71 A7

CODE_00A476:
STZ $00                 ; $00:A476: 64 00
...
CODE_00A489:
JMP CODE_00A771         ; $00:A489: 4C 71 A7

CODE_00A48C:
LDA $0220               ; $00:A48C: AD 20 02

; If we did not select a game, branch
BEQ CODE_00A4A6         ; $00:A48F: F0 15
LDA $F4                 ; $00:A491: A5 F4
ORA $F5                 ; $00:A493: 05 F5
ORA $F6                 ; $00:A495: 05 F6
ORA $F7                 ; $00:A497: 05 F7
AND #$40                ; $00:A499: 29 40
STA $00                 ; $00:A49B: 85 00

; If we want to select a new game, branch
...
LDA #$23                ; $00:A520: A9 23

; SFX to play when you move cursor |> over files ABCD
; when selecting a savefile after selecting a game.
STA $63                 ; $00:A522: 85 63
JMP CODE_00A771         ; $00:A524: 4C 71 A7

CODE_00A527:
LDA $D1                 ; $00:A527: A5 D1
...
CODE_00A561:
JMP CODE_00A771         ; $00:A561: 4C 71 A7
...
CODE_00A5EB:
STA $B0,x               ; $00:A5EB: 95 B0
BRA CODE_00A601         ; $00:A5ED: 80 12

CODE_00A5EF:
STA $B0,x               ; $00:A5EF: 95 B0
...
CODE_00A676:
LDA #$4C                ; $00:A676: A9 4C

CODE_00A678:
STA $0063               ; $00:A678: 8D 63 00
...
CODE_00A697:
BRA CODE_00A6AD         ; $00:A697: 80 14

CODE_00A699:
DEC $C0,x               ; $00:A699: D6 C0
...
CODE_00A6BB:
JMP CODE_00A703         ; $00:A6BB: 4C 03 A7

CODE_00A6BE:
INC $00C0,x             ; $00:A6BE: FE C0 00
...
CODE_00A6EC:
LDA #$01                ; $00:A6EC: A9 01
STA $B0,x               ; $00:A6EE: 95 B0

CODE_00A6F0:
LDA #$01                ; $00:A6F0: A9 01

CODE_00A6F2:
STA $00C0,x             ; $00:A6F2: 9D C0 00
...
CODE_00A703:
JSR CODE_00B0DA         ; $00:A703: 20 DA B0
JSR CODE_00B16B         ; $00:A706: 20 6B B1

CODE_00A709:
LDA #$00                ; $00:A709: A9 00
...
CODE_00A7A8:
LDA $B0,x               ; $00:A7A8: B5 B0
DEC A                   ; $00:A7AA: 3A
BPL CODE_00A7AF         ; $00:A7AB: 10 02
LDA #$00                ; $00:A7AD: A9 00

CODE_00A7AF:
STA $700008             ; $00:A7AF: 8F 08 00 70
...
CODE_00A7BA:
STA $700009             ; $00:A7BA: 8F 09 00 70

CODE_00A7BE:
LDA $D1                 ; $00:A7BE: A5 D1
ASL A                   ; $00:A7C0: 0A
ASL A                   ; $00:A7C1: 0A
CLC                     ; $00:A7C2: 18
...
LDY $0004               ; $00:A85B: AC 04 00

CODE_00A85E:
LDA $0010,y             ; $00:A85E: B9 10 00
STA $7FFB00,x           ; $00:A861: 9F 00 FB 7F
INY                     ; $00:A865: C8
INX                     ; $00:A866: E8
CPX $0E                 ; $00:A867: E4 0E

; Load $120 bytes of saved data from SRAM Offset of last save data accessed
; into RAM $7FFB00 whenever you load a game. Doesn't matter which.
BNE CODE_00A85E         ; $00:A869: D0 F3
LDA $0008               ; $00:A86B: AD 08 00

; Which world you have loaded, from SRAM into RAM
STA $7FFB00             ; $00:A86E: 8F 00 FB 7F
LDA $0009               ; $00:A872: AD 09 00

; Which level you have loaded, from SRAM into RAM
STA $7FFB01             ; $00:A875: 8F 01 FB 7F
LDA $70000F             ; $00:A879: AF 0F 00 70

; Loaded game into scratch RAM
STA $00                 ; $00:A87D: 85 00
BNE CODE_00A896         ; $00:A87F: D0 15
LDA $70000E             ; $00:A881: AF 0E 00 70
ASL A                   ; $00:A885: 0A
ASL A                   ; $00:A886: 0A
ORA $00                 ; $00:A887: 05 00
XBA                     ; $00:A889: EB
LDA #$00                ; $00:A88A: A9 00
XBA                     ; $00:A88C: EB

; If SMB1
; Save slot # << 4
; OR $00  (isn't this always 00 because we only get here when it's 00?)
;
; Clear high byte, to index.
;
; (so basically save slot * 4 to index)
TAX                     ; $00:A88D: AA
LDA $700490,x           ; $00:A88E: BF 90 04 70

; More difficult quest flag for SMB1
STA $7FFB05             ; $00:A892: 8F 05 FB 7F

CODE_00A896:
LDA #$00                ; $00:A896: A9 00
XBA                     ; $00:A898: EB
LDA $70000E             ; $00:A899: AF 0E 00 70
AND #$03                ; $00:A89D: 29 03
ASL A                   ; $00:A89F: 0A
ASL A                   ; $00:A8A0: 0A
TAX                     ; $00:A8A1: AA
...
CODE_00A8AE:
REP #$20                ; $00:A8AE: C2 20
LDX #$80                ; $00:A8B0: A2 80
LDA #$02FF              ; $00:A8B2: A9 FF 02

CODE_00A8B5:
STA $0400,x             ; $00:A8B5: 9D 00 04
...
CODE_00A8DB:
LDA DATA_00A9B1,x       ; $00:A8DB: BD B1 A9
...
CODE_00A8F3:
LDX #$00                ; $00:A8F3: A2 00
LDA #$0360              ; $00:A8F5: A9 60 03

CODE_00A8F8:
STA $04BE,x             ; $00:A8F8: 9D BE 04
...
CODE_00A93A:
LDA DATA_00A9C7,x       ; $00:A93A: BD C7 A9
...
CODE_00A9AE:
SEP #$20                ; $00:A9AE: E2 20
RTS                     ; $00:A9B0: 60

; Tilemap in selected game's file select

DATA_00A9B1:
db $F2,$02,$F3,$02,$F4,$02,$51,$03

; Tilemap of "FILE" and the entire line below it
db $51,$03,$51,$03,$70,$03,$71,$03

; Tilemap for all the 4 "FILE" sections. Format: tile, properties. FF marks end
db $72,$03,$73,$03,$FF,$FF

; Tilemap in selected game's file select

DATA_00A9C7:
db $64,$03,$65,$03,$66,$03,$67,$03

; Tilemap of the small controller in the bottom-left corner

DATA_00A9CF:
db $74,$03,$75,$03,$76,$03,$77,$03

; Format: tile, properties

DATA_00A9D7:
db $6B,$03,$6C,$03,$6D,$03,$6E,$03

; Tilemap is for controller -only-, not for the text around it.

DATA_00A9DF:
db $7B,$03,$7C,$03,$7D,$03,$7E,$03
...
CODE_00AA6E:
JSR CODE_00ADF8         ; $00:AA6E: 20 F8 AD
BRA CODE_00AAA0         ; $00:AA71: 80 2D

CODE_00AA73:
LDA $021B               ; $00:AA73: AD 1B 02
...
CODE_00AA9D:
JSR CODE_00AEC9         ; $00:AA9D: 20 C9 AE

CODE_00AAA0:
RTS                     ; $00:AAA0: 60

CODE_00AAA1:
LDA $0220               ; $00:AAA1: AD 20 02
BEQ CODE_00AAA9         ; $00:AAA4: F0 03
JMP CODE_00AB2E         ; $00:AAA6: 4C 2E AB

CODE_00AAA9:
LDA $021B               ; $00:AAA9: AD 1B 02
...
CODE_00ABDF:
JSR CODE_00ADF8         ; $00:ABDF: 20 F8 AD
BRA CODE_00AC11         ; $00:ABE2: 80 2D

CODE_00ABE4:
LDA $021B               ; $00:ABE4: AD 1B 02
...
CODE_00AC0E:
JSR CODE_00AEC9         ; $00:AC0E: 20 C9 AE

CODE_00AC11:
RTS                     ; $00:AC11: 60

CODE_00AC12:
LDA $0220               ; $00:AC12: AD 20 02
BEQ CODE_00AC1A         ; $00:AC15: F0 03
JMP CODE_00ACAA         ; $00:AC17: 4C AA AC

CODE_00AC1A:
LDA $021B               ; $00:AC1A: AD 1B 02
...
CODE_00ACF0:
LDX #$00                ; $00:ACF0: A2 00
TXY                     ; $00:ACF2: 9B

CODE_00ACF3:
LDA ($00),y             ; $00:ACF3: B1 00
INC A                   ; $00:ACF5: 1A
...
CODE_00AD5C:
LDX #$00                ; $00:AD5C: A2 00
TXY                     ; $00:AD5E: 9B

CODE_00AD5F:
LDA ($00),y             ; $00:AD5F: B1 00
INC A                   ; $00:AD61: 1A
...
DATA_00ADEC:
db $9C,$03,$9E,$03,$00,$00,$00,$00
db $9C,$03,$9E,$03

CODE_00ADF8:
LDA $7F8000             ; $00:ADF8: AF 00 80 7F
TAX                     ; $00:ADFC: AA
...
CODE_00AE4A:
LDA $7F8000             ; $00:AE4A: AF 00 80 7F
TAX                     ; $00:AE4E: AA
...
CODE_00AEC9:
LDA $7F8000             ; $00:AEC9: AF 00 80 7F
TAX                     ; $00:AECD: AA
...
CODE_00AF3C:
LDA $7F8000             ; $00:AF3C: AF 00 80 7F
TAX                     ; $00:AF40: AA
...
DATA_00B1C2:
db $71,$79,$7F,$79,$70,$7D,$61,$79

DATA_00B1CA:
db $51,$03,$9B,$03

CODE_00B1CE:
REP #$30                ; $00:B1CE: C2 30
...
CODE_00B53A:
LDA DATA_00B5C2,y       ; $00:B53A: B9 C2 B5
...
PHK                     ; $00:B640: 4B
PLB                     ; $00:B641: AB
DEC $020E               ; $00:B642: CE 0E 02

; Decrease timer (used for making Mario, toad, move the carpet bird/pidget, move luigi's hand, and birdo's tail move)
; If it is not zero branch
BNE CODE_00B65D         ; $00:B645: D0 16

; Move luigi's hand
JSR CODE_00B6DA         ; $00:B647: 20 DA B6

; Move toad
JSR CODE_00B748         ; $00:B64A: 20 48 B7

; Move birdo's tail
JSR CODE_00B7BD         ; $00:B64D: 20 BD B7

; Move pidget
JSR CODE_00B88C         ; $00:B650: 20 8C B8

; Move Mario
JSR CODE_00B8B5         ; $00:B653: 20 B5 B8
LDA #$09                ; $00:B656: A9 09

; Reset timer
STA $020E               ; $00:B658: 8D 0E 02

; Upload tilemap
STA $DE                 ; $00:B65B: 85 DE

CODE_00B65D:
DEC $0210               ; $00:B65D: CE 10 02

; If it is not time to make bowser talk and luigi's head move, branch
BNE CODE_00B66F         ; $00:B660: D0 0D

; Move luigi's head
JSR CODE_00B6B9         ; $00:B662: 20 B9 B6

; Move bowser
JSR CODE_00B970         ; $00:B665: 20 70 B9
LDA #$10                ; $00:B668: A9 10

; Reset timer
STA $0210               ; $00:B66A: 8D 10 02

; Upload tilemap
STA $DE                 ; $00:B66D: 85 DE

CODE_00B66F:
DEC $0211               ; $00:B66F: CE 11 02

; If it is not time to make peach talk and birdo talk, branch
BNE CODE_00B681         ; $00:B672: D0 0D

; Move peach
JSR CODE_00B723         ; $00:B674: 20 23 B7

; Move birdo
JSR CODE_00B79C         ; $00:B677: 20 9C B7
LDA #$20                ; $00:B67A: A9 20

; Reset timer
STA $0211               ; $00:B67C: 8D 11 02

; Upload tilemap
STA $DE                 ; $00:B67F: 85 DE
...
; Upload BG1 tilemap to RAM
REP #$30                ; $00:B686: C2 30

; Source low and high byte
LDX.w #DATA_3C8000      ; $00:B688: A2 00 80

; Destination low and high byte
LDY #$0000              ; $00:B68B: A0 00 00

; Amount of byte to transfer
LDA #$07FF              ; $00:B68E: A9 FF 07

; Transfer $3C:8000 -> $7F:0000
MVN $7F,DATA_3C8000>>16 ; $00:B691: 54 7F 3C
SEP #$30                ; $00:B694: E2 30
PLB                     ; $00:B696: AB
LDA #$0B                ; $00:B697: A9 0B
STA $020E               ; $00:B699: 8D 0E 02
LDA #$08                ; $00:B69C: A9 08
STA $020F               ; $00:B69E: 8D 0F 02
...
LDA #$01                ; $00:B6AB: A9 01

; Upload $7F:0000 to BG1 tilemap
STA $DE                 ; $00:B6AD: 85 DE
STZ $0212               ; $00:B6AF: 9C 12 02
LDA #$60                ; $00:B6B2: A9 60
STA $22                 ; $00:B6B4: 85 22

; Set flash timer for the "All-Stars" text on the title screen
STZ $23                 ; $00:B6B6: 64 23
RTS                     ; $00:B6B8: 60

CODE_00B6B9:
LDA #$00                ; $00:B6B9: A9 00
...
; Get frame
LDA $0200               ; $00:B6BF: AD 00 02
AND #$0003              ; $00:B6C2: 29 03 00
ASL A                   ; $00:B6C5: 0A
TAX                     ; $00:B6C6: AA
LDA DATA_00B705,x       ; $00:B6C7: BD 05 B7

; Source low and high byte
STA $00                 ; $00:B6CA: 85 00
LDA #$03A6              ; $00:B6CC: A9 A6 03

; Destination
STA $04                 ; $00:B6CF: 85 04

; Put tilemap in RAM
JSR CODE_00B9AB         ; $00:B6D1: 20 AB B9
SEP #$30                ; $00:B6D4: E2 30

; Next frame
INC $0200               ; $00:B6D6: EE 00 02
RTS                     ; $00:B6D9: 60

CODE_00B6DA:
LDA #$00                ; $00:B6DA: A9 00
...
; Get the frame
LDA $0201               ; $00:B6E0: AD 01 02
AND #$00FF              ; $00:B6E3: 29 FF 00
ASL A                   ; $00:B6E6: 0A
TAX                     ; $00:B6E7: AA
LDA DATA_00B70D,x       ; $00:B6E8: BD 0D B7

; Source low and high
STA $00                 ; $00:B6EB: 85 00
LDA #$0464              ; $00:B6ED: A9 64 04

; Destination
STA $04                 ; $00:B6F0: 85 04

; Put tilemap in RAM
JSR CODE_00B9AB         ; $00:B6F2: 20 AB B9
...
DATA_00B705:
dw $B9D1,$B9F5,$BA19,$B9F5

DATA_00B70D:
dw $BA3D,$BA3D,$BA4D,$BA4D
dw $BA4D,$BA4D,$BA5D,$BA4D
dw $BA4D,$BA4D,$BA4D

CODE_00B723:
LDA #$00                ; $00:B723: A9 00
...
DATA_00B8AD:
db $D7,$BB,$01,$BC,$2B,$BC,$55,$BC

CODE_00B8B5:
LDA #$00                ; $00:B8B5: A9 00
...
DATA_00B938:
dw $BC7F,$BC9D,$BCBB,$BC9D
dw $BC7F,$BC9D,$BCBB,$BCD9
dw $BCD9,$BCD9

DATA_00B94C:
dw $BCF7,$BCFF,$BD07,$BCFF
...
; X= #$0000
LDY #$0000              ; $00:B9AD: A0 00 00

; Get destination

CODE_00B9B0:
LDX $04                 ; $00:B9B0: A6 04

; Read data

CODE_00B9B2:
LDA [$00],y             ; $00:B9B2: B7 00

; If it is zero, branch
BEQ CODE_00B9C2         ; $00:B9B4: F0 0C

; If it is negative, return
BMI CODE_00B9CE         ; $00:B9B6: 30 16

; Write data
STA $7F0000,x           ; $00:B9B8: 9F 00 00 7F
INX                     ; $00:B9BC: E8
INX                     ; $00:B9BD: E8
INY                     ; $00:B9BE: C8
...
CODE_00BD8D:
LDA $22                 ; $00:BD8D: A5 22

; If the timer is not zero, branch
; (if it is not time to "flash")
BNE CODE_00BE05         ; $00:BD8F: D0 74
LDA $23                 ; $00:BD91: A5 23
CMP #$1C                ; $00:BD93: C9 1C

; If we finished flashing branch
BEQ CODE_00BDF1         ; $00:BD95: F0 5A
LDA #$04                ; $00:BD97: A9 04
...
AND #$00FF              ; $00:BDA2: 29 FF 00

; Get which step we are on
STA $04                 ; $00:BDA5: 85 04
CMP #$000E              ; $00:BDA7: C9 0E 00

; If we are halfway through it or more, branch
...
; Change the palette
LDY #$00                ; $00:BDB3: A0 00

CODE_00BDB5:
LDA DATA_00BE08,y       ; $00:BDB5: B9 08 BE
...
CODE_00BDCA:
LDA $23                 ; $00:BDCA: A5 23
AND #$00FF              ; $00:BDCC: 29 FF 00
SEC                     ; $00:BDCF: 38

; Change the palette
SBC #$000C              ; $00:BDD0: E9 0C 00
TAY                     ; $00:BDD3: A8
LDX #$00                ; $00:BDD4: A2 00

CODE_00BDD6:
LDA DATA_00BE08,y       ; $00:BDD6: B9 08 BE
...
CODE_00BDE5:
INC $23                 ; $00:BDE5: E6 23

; Increase the timer
INC $23                 ; $00:BDE7: E6 23

; Update CGRAM
INC $0AC0               ; $00:BDE9: EE C0 0A
SEP #$20                ; $00:BDEC: E2 20
PLB                     ; $00:BDEE: AB
BRA CODE_00BE05         ; $00:BDEF: 80 14

CODE_00BDF1:
REP #$20                ; $00:BDF1: C2 20
LDA $E3                 ; $00:BDF3: A5 E3
CMP $0225               ; $00:BDF5: CD 25 02

; If the amount of time left on the title screen is not equal to $0225,  branch
BNE CODE_00BE01         ; $00:BDF8: D0 07
LDA #$00F0              ; $00:BDFA: A9 F0 00
STA $22                 ; $00:BDFD: 85 22

; Reset timers
STZ $23                 ; $00:BDFF: 64 23

CODE_00BE01:
SEP #$20                ; $00:BE01: E2 20
BRA CODE_00BE07         ; $00:BE03: 80 02

; Decrease the timer

CODE_00BE05:
DEC $22                 ; $00:BE05: C6 22

CODE_00BE07:
RTS                     ; $00:BE07: 60

; Palette of the shining ALL*STARS logo
...
CODE_00C060:
REP #$20                ; $00:C060: C2 20

; Set loop count
LDX #$1E                ; $00:C062: A2 1E

CODE_00C064:
LDA $0753               ; $00:C064: AD 53 07

; Branch if Mario
BEQ CODE_00C06F         ; $00:C067: F0 06

; Get Luigi palette data
LDA.l DATA_00C0A3,x     ; $00:C069: BF A3 C0 00
BRA CODE_00C073         ; $00:C06D: 80 04

; Get Mario palette data

CODE_00C06F:
LDA.l DATA_00C083,x     ; $00:C06F: BF 83 C0 00

; Write palette data to RAM

CODE_00C073:
STA $1160,x             ; $00:C073: 9D 60 11
DEX                     ; $00:C076: CA
DEX                     ; $00:C077: CA
...
; Write princess peach thank you message
JSR CODE_00C0C3         ; $00:C07F: 20 C3 C0
RTL                     ; $00:C082: 6B

; Mario portrait Palette of princess saved sequence

DATA_00C083:
dw $734E,$7FFF,$14A5,$5D68
dw $762E,$27BF,$31BB,$3ABF
dw $0000,$152F,$355D,$525F
dw $169B,$1C9F,$0C19,$0C19

; Luigi portrait Palette of princess saved sequence
...
STA $120B               ; $00:C0D4: 8D 0B 12
LDA #$02                ; $00:C0D7: A9 02

; Sub Screen designation
STA $120C               ; $00:C0D9: 8D 0C 12

; 16 bit xy
REP #$10                ; $00:C0DC: C2 10
LDA $075F               ; $00:C0DE: AD 5F 07

; Push world number onto stack
PHA                     ; $00:C0E1: 48
LDA $70000F             ; $00:C0E2: AF 0F 00 70

; Branch if loaded game isn't SMB1
BNE CODE_00C0F2         ; $00:C0E6: D0 0A
LDA $07FC               ; $00:C0E8: AD FC 07

; When SMB1
; If not more difficult quest flag, branch
BEQ CODE_00C0F2         ; $00:C0EB: F0 05
LDA #$0C                ; $00:C0ED: A9 0C

; Set world number to... 0C?
STA $075F               ; $00:C0EF: 8D 5F 07

CODE_00C0F2:
LDA #$00                ; $00:C0F2: A9 00
...
;
; into $00
STA $00                 ; $00:C0FA: 85 00
LDA $075F               ; $00:C0FC: AD 5F 07
AND #$08                ; $00:C0FF: 29 08
LSR A                   ; $00:C101: 4A
LSR A                   ; $00:C102: 4A
ORA $00                 ; $00:C103: 05 00

; World number
; Bit is set when in world D
; >> 2
;
; OR with the player thing
; to index
TAY                     ; $00:C105: A8
PLA                     ; $00:C106: 68

; restore world number
STA $075F               ; $00:C107: 8D 5F 07
...
CODE_00C111:
LDA DATA_00C12E,y       ; $00:C111: B9 2E C1
STA $1702,x             ; $00:C114: 9D 02 17
INC A                   ; $00:C117: 1A

; Check if the written byte is FF
; if so, return
BEQ CODE_00C11E         ; $00:C118: F0 04
INX                     ; $00:C11A: E8
INY                     ; $00:C11B: C8
BRA CODE_00C111         ; $00:C11C: 80 F3

CODE_00C11E:
STX $1700               ; $00:C11E: 8E 00 17
SEP #$10                ; $00:C121: E2 10
PLX                     ; $00:C123: FA
PLB                     ; $00:C124: AB
RTS                     ; $00:C125: 60

; Indices to the messages of peach to the player
...
DATA_00C12E:
db $59,$05,$00,$11,$1D,$20,$11,$20

; Now try a more difficult quest..." text.
db $0A,$20,$17,$20,$14,$20,$28,$20
db $22,$20,$18,$20,$1E,$20,$59,$45

; Encoded in stripe image.
db $00,$0B,$16,$20,$0A,$20,$1B,$20
...
; "Thank you Mario for restoring peace to our
db $59,$05,$00,$11,$1D,$20,$11,$20

; kingdom. Hurrah to our hero, Mario!" text
db $0A,$20,$17,$20,$14,$20,$28,$20
db $22,$20,$18,$20,$1E,$20,$59,$45

; Encoded in stripe image.
db $00,$11,$16,$20,$0A,$20,$1B,$20
...
; "Thank you Luigi! The kingdom is saved!
db $59,$05,$00,$11,$1D,$20,$11,$20
...
; "Thank you Luigi for restoring peace to our
db $59,$05,$00,$11,$1D,$20,$11,$20

; kingdom. Hurrah to our hero, Luigi!" text
db $0A,$20,$17,$20,$14,$20,$28,$20
db $22,$20,$18,$20,$1E,$20,$59,$45

; Encoded in stripe image.
db $00,$11,$15,$20,$1E,$20,$12,$20
...
CODE_00C402:
LDA.l DATA_00C43F,x     ; $00:C402: BF 3F C4 00
...
LDA #$A0                ; $00:C471: A9 A0

; Set player X position to A0
; (This happens when the princess hugs the player and gets pushed back 2 pixels or so)
STA $0219               ; $00:C473: 8D 19 02

CODE_00C476:
LDA.l DATA_00C4B4,x     ; $00:C476: BF B4 C4 00
STA $0F88               ; $00:C47A: 8D 88 0F

CODE_00C47D:
LDA $0F87               ; $00:C47D: AD 87 0F
ASL A                   ; $00:C480: 0A
TAX                     ; $00:C481: AA
...
CODE_00C4E9:
LDA.l DATA_00C54E,x     ; $00:C4E9: BF 4E C5 00

CODE_00C4ED:
STA $08B2               ; $00:C4ED: 8D B2 08
CLC                     ; $00:C4F0: 18
...
DATA_00C59B:
db $00,$A0,$00,$D1,$00,$D8,$00,$E0
db $00,$E8,$00,$F0,$00,$F8,$00,$D0
db $00,$D8,$00,$E8

DATA_00C5AF:
db $00,$30,$80,$60,$00,$64,$00,$68
...
CODE_00C654:
LDA $70000F             ; $00:C654: AF 0F 00 70
...
CODE_00C68B:
LDA $70000F             ; $00:C68B: AF 0F 00 70
...
CODE_00C6B8:
LDA #$08                ; $00:C6B8: A9 08
STA $0F80               ; $00:C6BA: 8D 80 0F

CODE_00C6BD:
INC $0F81               ; $00:C6BD: EE 81 0F
...
CODE_00C6D5:
LDA DATA_00CAFA,x       ; $00:C6D5: BD FA CA
...
CODE_00C6FF:
LDA DATA_00CAFA,x       ; $00:C6FF: BD FA CA
...
CODE_00C715:
LDX $ED                 ; $00:C715: A6 ED

CODE_00C717:
LDA DATA_00CAFA,x       ; $00:C717: BD FA CA
...
CODE_00C74E:
LDA DATA_00CAFA,x       ; $00:C74E: BD FA CA
...
CODE_00C7B8:
SEP #$10                ; $00:C7B8: E2 10
JSR CODE_00CA21         ; $00:C7BA: 20 21 CA
PLB                     ; $00:C7BD: AB
RTL                     ; $00:C7BE: 6B

CODE_00C7BF:
LDA $075F               ; $00:C7BF: AD 5F 07
...
CODE_00C80C:
LDA DATA_00CAFA,x       ; $00:C80C: BD FA CA
...
CODE_00C836:
LDA DATA_00CAFA,x       ; $00:C836: BD FA CA
...
CODE_00C84C:
LDX $ED                 ; $00:C84C: A6 ED

CODE_00C84E:
LDA DATA_00CAFA,x       ; $00:C84E: BD FA CA
...
CODE_00C885:
LDA DATA_00CAFA,x       ; $00:C885: BD FA CA
...
CODE_00C91A:
LDA #$09                ; $00:C91A: A9 09
STA $0F80               ; $00:C91C: 8D 80 0F

CODE_00C91F:
INC $0F81               ; $00:C91F: EE 81 0F
...
CODE_00C937:
LDA DATA_00CAFA,x       ; $00:C937: BD FA CA
...
CODE_00C961:
LDA DATA_00CAFA,x       ; $00:C961: BD FA CA
...
CODE_00C977:
LDX $ED                 ; $00:C977: A6 ED

CODE_00C979:
LDA DATA_00CAFA,x       ; $00:C979: BD FA CA
...
CODE_00C9B0:
LDA DATA_00CAFA,x       ; $00:C9B0: BD FA CA
...
CODE_00CA1A:
SEP #$10                ; $00:CA1A: E2 10
JSR CODE_00CA21         ; $00:CA1C: 20 21 CA
PLB                     ; $00:CA1F: AB
RTL                     ; $00:CA20: 6B

CODE_00CA21:
LDA $0753               ; $00:CA21: AD 53 07
...
CODE_00D4ED:
LDA #$C000              ; $00:D4ED: A9 00 C0

CODE_00D4F0:
STA $0285               ; $00:D4F0: 8D 85 02
...
CODE_00D5AA:
LDA #$00                ; $00:D5AA: A9 00
STA $0F7D               ; $00:D5AC: 8D 7D 0F

CODE_00D5AF:
STA $00                 ; $00:D5AF: 85 00
...
CODE_00D5DD:
LDA #$80                ; $00:D5DD: A9 80

CODE_00D5DF:
STA $0902               ; $00:D5DF: 8D 02 09
INC A                   ; $00:D5E2: 1A
...
CODE_00D638:
LDA PNTR_00D645,x       ; $00:D638: BD 45 D6
...
CODE_00D6C7:
DEC $0B9F               ; $00:D6C7: CE 9F 0B

CODE_00D6CA:
LDA $03AE               ; $00:D6CA: AD AE 03
...
CODE_00D704:
JSR CODE_00E566         ; $00:D704: 20 66 E5
...
CODE_00D74C:
DEC $0B9F               ; $00:D74C: CE 9F 0B

CODE_00D74F:
LDA $03AE               ; $00:D74F: AD AE 03
...
CODE_00D7F7:
JSR CODE_00D831         ; $00:D7F7: 20 31 D8
JMP CODE_00E1D5         ; $00:D7FA: 4C D5 E1

CODE_00D7FD:
LDY #$A0                ; $00:D7FD: A0 A0
...
CODE_00D80C:
JSR CODE_00D8ED         ; $00:D80C: 20 ED D8
BRA CODE_00D814         ; $00:D80F: 80 03

CODE_00D811:
JSR CODE_00D924         ; $00:D811: 20 24 D9

CODE_00D814:
INC $0BA0               ; $00:D814: EE A0 0B

CODE_00D817:
LDA $03AE               ; $00:D817: AD AE 03
...
CODE_00D865:
JSR CODE_00E1FC         ; $00:D865: 20 FC E1
TYA                     ; $00:D868: 98
CLC                     ; $00:D869: 18
...
CODE_00D885:
JSR CODE_00E1FC         ; $00:D885: 20 FC E1
...
CODE_00D99C:
JSR CODE_00D9D7         ; $00:D99C: 20 D7 D9
JMP CODE_00E1D5         ; $00:D99F: 4C D5 E1

CODE_00D9A2:
LDA $0BA1               ; $00:D9A2: AD A1 0B
...
CODE_00D9AE:
JSR CODE_00E529         ; $00:D9AE: 20 29 E5
LDA #$02                ; $00:D9B1: A9 02

CODE_00D9B3:
STA $0B9E               ; $00:D9B3: 8D 9E 0B
LDX #$03                ; $00:D9B6: A2 03
LDY #$00                ; $00:D9B8: A0 00

CODE_00D9BA:
LDA $0BB0,x             ; $00:D9BA: BD B0 0B
...
CODE_00D9D7:
JSR CODE_00E566         ; $00:D9D7: 20 66 E5
...
CODE_00DA60:
DEC $0BB2               ; $00:DA60: CE B2 0B

CODE_00DA63:
STA $01                 ; $00:DA63: 85 01
TYA                     ; $00:DA65: 98
CLC                     ; $00:DA66: 18
...
CODE_00DA97:
INC $0BB3               ; $00:DA97: EE B3 0B

CODE_00DA9A:
STA $01                 ; $00:DA9A: 85 01
TYA                     ; $00:DA9C: 98
CLC                     ; $00:DA9D: 18
...
CODE_00DAAB:
LDY #$B0                ; $00:DAAB: A0 B0
LDX #$00                ; $00:DAAD: A2 00

CODE_00DAAF:
LDA $03AE               ; $00:DAAF: AD AE 03
...
CODE_00DAEB:
LDA DATA_00DB88,x       ; $00:DAEB: BD 88 DB
...
CODE_00DBEA:
STA $00A1               ; $00:DBEA: 8D A1 00

CODE_00DBED:
INC $0B9F               ; $00:DBED: EE 9F 0B
...
CODE_00DC0C:
JSR CODE_00DD86         ; $00:DC0C: 20 86 DD
JMP CODE_00E1D5         ; $00:DC0F: 4C D5 E1

CODE_00DC12:
LDA $03AE               ; $00:DC12: AD AE 03
CLC                     ; $00:DC15: 18
...
CODE_00DD56:
LDX #$03                ; $00:DD56: A2 03

CODE_00DD58:
LDA $0BB8,x             ; $00:DD58: BD B8 0B
LSR A                   ; $00:DD5B: 4A
LSR A                   ; $00:DD5C: 4A
LSR A                   ; $00:DD5D: 4A
TAY                     ; $00:DD5E: A8
...
CODE_00DDBF:
LDA DATA_00DDDA,x       ; $00:DDBF: BD DA DD
...
CODE_00DE9B:
JSR CODE_00DF4C         ; $00:DE9B: 20 4C DF
...
CODE_00E0D9:
JMP CODE_00E1D5         ; $00:E0D9: 4C D5 E1
...
CODE_00E117:
LDA DATA_00E1B5,x       ; $00:E117: BD B5 E1
...
CODE_00E145:
LDA #$C0                ; $00:E145: A9 C0

CODE_00E147:
STA $091D               ; $00:E147: 8D 1D 09
...
ASL A                   ; $00:E3D5: 0A
TAX                     ; $00:E3D6: AA
LDA PNTR_00E3E4,x       ; $00:E3D7: BD E4 E3
STA $00                 ; $00:E3DA: 85 00
LDA PNTR_00E3E4+1,x     ; $00:E3DC: BD E5 E3
STA $01                 ; $00:E3DF: 85 01

; Jump to pointers according to the world number
...
CODE_00E561:
JSL CODE_0E8171         ; $00:E561: 22 71 81 0E
RTS                     ; $00:E565: 60

CODE_00E566:
LDA $0BA4               ; $00:E566: AD A4 0B
...
; ROM NAME
db "SUPER MARIO ALL_STARS"

; MAP MODE. Mode 20 = 2.68 MHz (= Normal Speed)
db $20

; CARTRIDGE TYPE: ROM + RAM + SRAM
db $02
...
; DEST. CODE: North America. This ROM comes from America?
db $01

; FIXED VALUE. It was supposed to be 33?
db $01

; MASK ROM VERSION. This ROM is not revised.
db $00

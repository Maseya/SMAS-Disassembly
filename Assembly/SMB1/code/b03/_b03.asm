...
; Jump to the main game: SMB1

CODE_038000:
JMP CODE_038015         ; $03:8000: 4C 15 80

; Jump to NMI data of SMB1

CODE_038003:
JMP CODE_038308         ; $03:8003: 4C 08 83

; Jump to SMB1 IRQ Data

CODE_038006:
JMP CODE_03850D         ; $03:8006: 4C 0D 85
db $00,$00,$03,$03,$04,$04,$05,$05

; Show "Pirated game" message

CODE_038011:
JML CODE_009641         ; $03:8011: 5C 41 96 00
...
BNE CODE_038011         ; $03:8021: D0 EE
LDA #$55                ; $03:8023: A9 55
STA $702000             ; $03:8025: 8F 00 20 70
CMP $700000             ; $03:8029: CF 00 00 70

; Check if the ROM is pirated. If so
; Show the pirated message
BNE CODE_038011         ; $03:802D: D0 E2

; Set interrupt flag
SEI                     ; $03:802F: 78
STZ $4200               ; $03:8030: 9C 00 42
STZ $420C               ; $03:8033: 9C 0C 42

; Disable NMI, IRQ, DMA, HDMA, Auto-read joypad
STZ $420B               ; $03:8036: 9C 0B 42
LDA #$80                ; $03:8039: A9 80

; Forced blanking
STA $2100               ; $03:803B: 8D 00 21
PHK                     ; $03:803E: 4B

; Set program bank to current bank
PLB                     ; $03:803F: AB
LDA #$01                ; $03:8040: A9 01

; Stop most of the animations
STA $028C               ; $03:8042: 8D 8C 02
LDA #$03                ; $03:8045: A9 03
STA $0F0B               ; $03:8047: 8D 0B 0F
STZ $028E               ; $03:804A: 9C 8E 02
STZ $0E67               ; $03:804D: 9C 67 0E
LDA $700009             ; $03:8050: AF 09 00 70
STA $0E24               ; $03:8054: 8D 24 0E

; Load the current level number loaded from save
; Should be 0, because you can't specify which level you want to load in SMB1.
STA $7FFB01             ; $03:8057: 8F 01 FB 7F
LDA $700008             ; $03:805B: AF 08 00 70
STA $075F               ; $03:805F: 8D 5F 07

; Load the current world number loaded from save
;
; Store into world number to load from the title screen
STA $7FFB00             ; $03:8062: 8F 00 FB 7F

; | Current world number * 4 + current level number
ASL A                   ; $03:8066: 0A
...
STA $700009             ; $03:8072: 8F 09 00 70

; Load from level table
; Store into current level number loaded from save
; And some other address
STA $7FFB02             ; $03:8076: 8F 02 FB 7F

; / And into current level
STA $0760               ; $03:807A: 8D 60 07
LDA #$03                ; $03:807D: A9 03

; OBJ size: 8x8 and 16x16 sprites
; OBJ VRAM: $C000
STA $2101               ; $03:807F: 8D 01 21
LDA #$01                ; $03:8082: A9 01

; BG1 Tilemap address: $0000
; Enable BG1 tilemap horizontal mirroring
STA $2107               ; $03:8084: 8D 07 21
LDA #$09                ; $03:8087: A9 09

; BG2 Tilemap address: $1000
; Enable BG2 tilemap horizontal mirroring
STA $2108               ; $03:8089: 8D 08 21
LDA #$59                ; $03:808C: A9 59

; BG3 Tilemap address: $B000
; Enable BG3 tilemap horizontal mirroring
STA $2109               ; $03:808E: 8D 09 21

; no BG4, but set it just in case.
STZ $210A               ; $03:8091: 9C 0A 21
LDA #$11                ; $03:8094: A9 11

; BG1 and 2 gfx address: $2000
STA $210B               ; $03:8096: 8D 0B 21
LDA #$05                ; $03:8099: A9 05

; BG3 and 4 gfx address: $A000
STA $210C               ; $03:809B: 8D 0C 21

; Nothing special about screen mode/video select
STZ $2133               ; $03:809E: 9C 33 21

; unused?
STZ $1681               ; $03:80A1: 9C 81 16

; Set level mode to Title Screen
STZ $0770               ; $03:80A4: 9C 70 07
STZ $0772               ; $03:80A7: 9C 72 07
LDA #$00                ; $03:80AA: A9 00
STA $7FFFFF             ; $03:80AC: 8F FF FF 7F

; Reset scroll registers
JSL CODE_04FDC4         ; $03:80B0: 22 C4 FD 04
LDA #$09                ; $03:80B4: A9 09

; Game mode and character size
; Mode 1 with mode 1 bg3 priority bit enabled
STA $120D               ; $03:80B6: 8D 0D 12

; No screen pixelation/mosaic
STZ $0E7E               ; $03:80B9: 9C 7E 0E
LDA #$10                ; $03:80BC: A9 10

; Main screen designation
STA $120B               ; $03:80BE: 8D 0B 12

; Sub screen designation
STZ $120C               ; $03:80C1: 9C 0C 12

; Window mask settings for BG1 and 2
STZ $1204               ; $03:80C4: 9C 04 12

; Window mask settings for BG3 and 4
STZ $1205               ; $03:80C7: 9C 05 12

; Window mask settings for OAM and color window
STZ $1206               ; $03:80CA: 9C 06 12

; Color addition select
STZ $1209               ; $03:80CD: 9C 09 12
LDA #$20                ; $03:80D0: A9 20

; CGADSUB settings
STA $120A               ; $03:80D2: 8D 0A 12
LDA #$20                ; $03:80D5: A9 20
STA $0EE0               ; $03:80D7: 8D E0 0E
LDA #$40                ; $03:80DA: A9 40
...
; HDMA enable: none
STZ $1203               ; $03:80E4: 9C 03 12
LDA #$80                ; $03:80E7: A9 80

; Enable f-blank
STA $1201               ; $03:80E9: 8D 01 12

; Reset lag thing.
STZ $0154               ; $03:80EC: 9C 54 01
STZ $15E5               ; $03:80EF: 9C E5 15

; Related to when fading in and out
STZ $0E7F               ; $03:80F2: 9C 7F 0E

; Clear pause flag
STZ $0776               ; $03:80F5: 9C 76 07
...
; Clear flag to upload animated graphics
STZ $028D               ; $03:810F: 9C 8D 02
...
CODE_038205:
JSL CODE_05E634         ; $03:8205: 22 34 E6 05
...
CODE_0382B2:
JSL CODE_05D472         ; $03:82B2: 22 72 D4 05
...
CODE_0382C3:
JSR CODE_038545         ; $03:82C3: 20 45 85
LDA $1680               ; $03:82C6: AD 80 16
BMI CODE_0382CD         ; $03:82C9: 30 02
BRA CODE_0382D4         ; $03:82CB: 80 07

CODE_0382CD:
JSR CODE_0385F3         ; $03:82CD: 20 F3 85
JSL CODE_05DE4C         ; $03:82D0: 22 4C DE 05

CODE_0382D4:
JSL CODE_05DD7A         ; $03:82D4: 22 7A DD 05
STZ $0154               ; $03:82D8: 9C 54 01
JMP CODE_0381A9         ; $03:82DB: 4C A9 81

DATA_0382DE:
db $02,$B8,$B8,$B8,$B8,$02,$02,$02

; Low bytes of VRAM addresses used by the stripe image uploader
db $B8,$B8,$B8,$B8,$B9,$DE,$03,$56
db $81,$BC,$DB

; High bytes of VRAM addresses used by the stripe image uploader

DATA_0382F1:
db $17,$99,$99,$99,$99,$17,$1A,$1A
...
; Set program bank to this one
PLB                     ; $03:830A: AB

; Read to enable NMI
LDA $4210               ; $03:830B: AD 10 42
LDA $1201               ; $03:830E: AD 01 12
BNE CODE_038315         ; $03:8311: D0 02
...
CODE_038315:
STA $2100               ; $03:8315: 8D 00 21

; Reset HDMA
STZ $420C               ; $03:8318: 9C 0C 42
LDA $0154               ; $03:831B: AD 54 01

; Make sure the game runs at normal fps?
BEQ CODE_038323         ; $03:831E: F0 03
JMP CODE_038448         ; $03:8320: 4C 48 84

CODE_038323:
INC $0154               ; $03:8323: EE 54 01
...
JSR CODE_039BBC         ; $03:8337: 20 BC 9B
...
; Stripe image uploader
JSR CODE_039BBC         ; $03:834F: 20 BC 9B
...
; 16-bit XY
REP #$10                ; $03:8359: C2 10
LDA #$81                ; $03:835B: A9 81

; Address increment mode: after every write to 2119
; Also, every 32 bytes. so vertical
STA $2115               ; $03:835D: 8D 15 21
LDY #$1801              ; $03:8360: A0 01 18

; 2 regs write once
; DMA destination: $2118
STY $00                 ; $03:8363: 84 00

; What is this address?
LDA $0EF9               ; $03:8365: AD F9 0E
BEQ CODE_03839D         ; $03:8368: F0 33
LDA #$7F                ; $03:836A: A9 7F
...
CODE_03837A:
STA $2116               ; $03:837A: 8D 16 21
TXA                     ; $03:837D: 8A
CLC                     ; $03:837E: 18
ADC #$0004              ; $03:837F: 69 04 00

; DMA source address: $0000 + 4 something?
STA $02                 ; $03:8382: 85 02

; DMA size: $0040 bytes
STY $05                 ; $03:8384: 84 05
TXA                     ; $03:8386: 8A
CLC                     ; $03:8387: 18
ADC #$0042              ; $03:8388: 69 42 00

; add $42 bytes
TAX                     ; $03:838B: AA
LDA #$0001              ; $03:838C: A9 01 00
...
; CGRAM address: $00
STZ $2121               ; $03:83E9: 9C 21 21
LDY #$2200              ; $03:83EC: A0 00 22

; 1 reg write once to $2122
STY $00                 ; $03:83EF: 84 00
LDY #$1000              ; $03:83F1: A0 00 10
STY $02                 ; $03:83F4: 84 02
LDA #$00                ; $03:83F6: A9 00
STA $04                 ; $03:83F8: 85 04
...
CPX #$06                ; $03:8416: E0 06

; Branch if stripe image destination VRAM index is not 6
...
; Direct page: $2100
TCD                     ; $03:844E: 5B
LDA $120E               ; $03:844F: AD 0E 12

; $7E:120E: Window 2 left position
STA $28                 ; $03:8452: 85 28
LDA $120F               ; $03:8454: AD 0F 12

; $7E:120F: Window 2 right position
STA $29                 ; $03:8457: 85 29
LDA $1210               ; $03:8459: AD 10 12

; $7E:1210: Direct color register
STA $32                 ; $03:845C: 85 32
LDA $1209               ; $03:845E: AD 09 12

; $7E:1209: Color addition select
STA $30                 ; $03:8461: 85 30
LDA $120A               ; $03:8463: AD 0A 12

; $7E:120A: CGADSUB
STA $31                 ; $03:8466: 85 31
LDA $1204               ; $03:8468: AD 04 12

; $7E:1204: Window mask settings for BG1 and 2
STA $23                 ; $03:846B: 85 23
LDA $1205               ; $03:846D: AD 05 12

; $7E:1205: Window mask settings for BG3 and 4
STA $24                 ; $03:8470: 85 24
LDA $1206               ; $03:8472: AD 06 12

; $7E:1206: Window mask settings for OAM and color wind
STA $25                 ; $03:8475: 85 25
LDA $120B               ; $03:8477: AD 0B 12

; $7E:120B: Main screen designation
STA $2C                 ; $03:847A: 85 2C
LDA $120C               ; $03:847C: AD 0C 12

; $7E:120C: Subscreen designation
STA $2D                 ; $03:847F: 85 2D
LDA $1207               ; $03:8481: AD 07 12

; $7E:1207: Window mask designation for main screen
STA $2E                 ; $03:8484: 85 2E
LDA $1208               ; $03:8486: AD 08 12

; $7E:1208: Window mask designation for sub screen
STA $2F                 ; $03:8489: 85 2F
LDA $0EE0               ; $03:848B: AD E0 0E

; $7E:0EE0: Direct color register
STA $32                 ; $03:848E: 85 32
LDA $0EE1               ; $03:8490: AD E1 0E

; $7E:0EE1: Direct color register
STA $32                 ; $03:8493: 85 32
LDA $0EE2               ; $03:8495: AD E2 0E

; $7E:0EE2: Direct color register
STA $32                 ; $03:8498: 85 32
LDA $120D               ; $03:849A: AD 0D 12

; $7E:120D: Screen mode
STA $05                 ; $03:849D: 85 05
LDA $0E7E               ; $03:849F: AD 7E 0E

; $7E:0E7E: Screen pixelation/Mosaic
STA $06                 ; $03:84A2: 85 06
STZ $11                 ; $03:84A4: 64 11

; Zero out BG3 horizontal scroll
STZ $11                 ; $03:84A6: 64 11
LDA $0ED1               ; $03:84A8: AD D1 0E
BEQ CODE_0384B7         ; $03:84AB: F0 0A
LDA $0EEE               ; $03:84AD: AD EE 0E
STA $0F                 ; $03:84B0: 85 0F
LDA $0EEF               ; $03:84B2: AD EF 0E

; If starry night IRQ flag is set
;
; $7E:0EEE-$7E:0EEF: BG2 Horizontal scroll
STA $0F                 ; $03:84B5: 85 0F

CODE_0384B7:
LDA $0E7F               ; $03:84B7: AD 7F 0E

; Fade in/out things?
BNE CODE_0384E6         ; $03:84BA: D0 2A
LDA $073F               ; $03:84BC: AD 3F 07

; $7E:073F: BG1 xscroll offset low byte
STA $0D                 ; $03:84BF: 85 0D
LDA $071A               ; $03:84C1: AD 1A 07

; $7E:071A: BG1 xscroll offset high byte
STA $0D                 ; $03:84C4: 85 0D
LDA $0ED1               ; $03:84C6: AD D1 0E

; If not Starry Night BG flag
BNE CODE_0384D5         ; $03:84C9: D0 0A
LDA $0EFD               ; $03:84CB: AD FD 0E

; $7E:0EFD: BG2 xscroll offset low byte
STA $0F                 ; $03:84CE: 85 0F
LDA $0EFE               ; $03:84D0: AD FE 0E

; $7E:0EFE: BG2 xscroll offset high byte
STA $0F                 ; $03:84D3: 85 0F

CODE_0384D5:
LDA $0740               ; $03:84D5: AD 40 07

; $7E:0E70: BG1 yscroll offset low byte
STA $0E                 ; $03:84D8: 85 0E

; BG1 yscroll offset high byte hardcoded at #$00
STZ $0E                 ; $03:84DA: 64 0E
LDA $0ED2               ; $03:84DC: AD D2 0E

; $7E:0ED2: BG2 yscroll offset low byte
STA $10                 ; $03:84DF: 85 10
LDA $0ED3               ; $03:84E1: AD D3 0E

; $7E:0ED3: BG2 yscroll offset high byte
...
; If IRQ is active
LDA $0EDE               ; $03:84EF: AD DE 0E

; Read IRQ flag for whatever reason
BEQ CODE_038508         ; $03:84F2: F0 14
LDA $4211               ; $03:84F4: AD 11 42
LDA $0EF2               ; $03:84F7: AD F2 0E
STA $4209               ; $03:84FA: 8D 09 42

; $7E:0EF2: V-timer
; With $00 as vtimer high byte
STZ $420A               ; $03:84FD: 9C 0A 42
STZ $4207               ; $03:8500: 9C 07 42

; Store zero to h-timer
STZ $4208               ; $03:8503: 9C 08 42
LDX #$A1                ; $03:8506: A2 A1

; Enable NMI, IRQ, and auto joypad-read enable, only

CODE_038508:
STX $4200               ; $03:8508: 8E 00 42

; when IRQ itself is enabled.
PLB                     ; $03:850B: AB
...
; SMB1 IRQ Routine
; Set current program bank
PLB                     ; $03:850F: AB
LDA $4211               ; $03:8510: AD 11 42

; If IRQ isn't fired yet, return
BPL CODE_038543         ; $03:8513: 10 2E
LDA $0EDE               ; $03:8515: AD DE 0E

; If this bit is set, return
; (Seems like it is used as "IRQ enable flag")
BEQ CODE_038543         ; $03:8518: F0 29
LDA $0ED1               ; $03:851A: AD D1 0E

; "Starry night" IRQ mode
; If it is clear, go to normal IRQ mode
BEQ CODE_038532         ; $03:851D: F0 13

CODE_03851F:
BIT $4212               ; $03:851F: 2C 12 42

; Wait till H-blank
BVC CODE_03851F         ; $03:8522: 50 FB
LDA $0EFD               ; $03:8524: AD FD 0E
STA $210F               ; $03:8527: 8D 0F 21
LDA $0EFE               ; $03:852A: AD FE 0E

; Used with Starry BG IRQ mode
; Set BG2 Horizontal scroll
STA $210F               ; $03:852D: 8D 0F 21

; Return
BRA CODE_038543         ; $03:8530: 80 11

CODE_038532:
BIT $4212               ; $03:8532: 2C 12 42

; Wait till H-blank
BVC CODE_038532         ; $03:8535: 50 FB
LDA $0EEE               ; $03:8537: AD EE 0E
STA $2111               ; $03:853A: 8D 11 21
LDA $0EEF               ; $03:853D: AD EF 0E

; Set BG3 horizontal scroll
STA $2111               ; $03:8540: 8D 11 21

CODE_038543:
PLB                     ; $03:8543: AB

; Recover program bank & return
RTL                     ; $03:8544: 6B

CODE_038545:
LDA $0770               ; $03:8545: AD 70 07
ASL A                   ; $03:8548: 0A
TAX                     ; $03:8549: AA

; Set up 'operation modes' (RAM $7E:0770)
; see pointers below for effects
...
; $01 Normal playing and controls
dw CODE_03AD60
...
CODE_038555:
LDA $0770               ; $03:8555: AD 70 07
CMP #$02                ; $03:8558: C9 02

; Branch if operation mode is Defeated Bowser
BEQ CODE_03857B         ; $03:855A: F0 1F
CMP #$01                ; $03:855C: C9 01

; Branch if operation mode is Main
BNE CODE_0385BE         ; $03:855E: D0 5E
...
BEQ CODE_03857C         ; $03:8576: F0 04

; Branch if Timer to disable start button = 0
; Otherwise, keep decreasing
DEC $0777               ; $03:8578: CE 77 07

CODE_03857B:
RTS                     ; $03:857B: 60

CODE_03857C:
LDA $0F                 ; $03:857C: A5 0F
...
CODE_03859C:
LDA $0B75               ; $03:859C: AD 75 0B

; If within pause screen
; Skip "pause game" button check function
BNE CODE_0385BE         ; $03:859F: D0 1D
LDY $0753               ; $03:85A1: AC 53 07
LDA $0FF6,y             ; $03:85A4: B9 F6 0F
AND #$10                ; $03:85A7: 29 10

; Get controller data based on player #.
; if start is not pressed, continue
BEQ CODE_0385BE         ; $03:85A9: F0 13
LDA #$11                ; $03:85AB: A9 11

; Else, pause the game
; Timer to disable start button (works while outside pause, fading into it)
STA $0777               ; $03:85AD: 8D 77 07
LDA #$F1                ; $03:85B0: A9 F1
STA $1602               ; $03:85B2: 8D 02 16
LDA #$43                ; $03:85B5: A9 43

; Tone down volume of music
; Play pause sound
...
CODE_0385BF:
JSR CODE_0385F8         ; $03:85BF: 20 F8 85
RTL                     ; $03:85C2: 6B

CODE_0385C3:
PHD                     ; $03:85C3: 0B
LDA #$0B                ; $03:85C4: A9 0B
XBA                     ; $03:85C6: EB
...
CODE_0385F3:
LDA $0E67               ; $03:85F3: AD 67 0E
BNE CODE_03866C         ; $03:85F6: D0 74

CODE_0385F8:
LDX #$00                ; $03:85F8: A2 00
LDA #$F0                ; $03:85FA: A9 F0

CODE_0385FC:
STA $0801,x             ; $03:85FC: 9D 01 08
...
CODE_038635:
LDX #$00                ; $03:8635: A2 00
LDA #$F0                ; $03:8637: A9 F0

CODE_038639:
STA $0841,x             ; $03:8639: 9D 41 08
...
STZ $1601               ; $03:869E: 9C 01 16
STZ $1602               ; $03:86A1: 9C 02 16

; Clear sound registers so no sound plays during title screen
STZ $1603               ; $03:86A4: 9C 03 16
RTS                     ; $03:86A7: 60

CODE_0386A8:
STZ $1602               ; $03:86A8: 9C 02 16
RTS                     ; $03:86AB: 60
...
CODE_0386B4:
LDA $701FF4             ; $03:86B4: AF F4 1F 70
...
CODE_0386D9:
LDA $0FF6               ; $03:86D9: AD F6 0F
ORA $0FF7               ; $03:86DC: 0D F7 0F
BIT #$10                ; $03:86DF: 89 10
BEQ CODE_03871A         ; $03:86E1: F0 37

CODE_0386E3:
JMP CODE_0387D3         ; $03:86E3: 4C D3 87

CODE_0386E6:
LDA #$00                ; $03:86E6: A9 00
STA $701FE0             ; $03:86E8: 8F E0 1F 70
STA $701FE1             ; $03:86EC: 8F E1 1F 70
...
; Reset highscore and its display
STA $07CD               ; $03:870F: 8D CD 07
LDA #$08                ; $03:8712: A9 08

; High score resetted sound
STA $1600               ; $03:8714: 8D 00 16
JMP CODE_0387C1         ; $03:8717: 4C C1 87

CODE_03871A:
LDA $701FF2             ; $03:871A: AF F2 1F 70
...
; Reset highscore
BEQ CODE_0386E6         ; $03:872C: F0 B8

CODE_03872E:
LDA $0FF9               ; $03:872E: AD F9 0F
...
; Reset highscore
BEQ CODE_0386E6         ; $03:873A: F0 AA

CODE_03873C:
LDA $7FFB06             ; $03:873C: AF 06 FB 7F
...
CODE_038774:
LDA $701FF2             ; $03:8774: AF F2 1F 70
...
CODE_038799:
STA $077A               ; $03:8799: 8D 7A 07
LDA #$01                ; $03:879C: A9 01

; Collected a coin sound
STA $1603               ; $03:879E: 8D 03 16

CODE_0387A1:
JSR CODE_038861         ; $03:87A1: 20 61 88

CODE_0387A4:
STZ $0FF4               ; $03:87A4: 9C F4 0F

CODE_0387A7:
LDA $1603               ; $03:87A7: AD 03 16
PHA                     ; $03:87AA: 48
...
CODE_0387D3:
LDY $07B2               ; $03:87D3: AC B2 07
BEQ CODE_0387C1         ; $03:87D6: F0 E9
LDA #$29                ; $03:87D8: A9 29
...
; Enable hidden 1-up flag
INC $075D               ; $03:87F0: EE 5D 07
INC $0764               ; $03:87F3: EE 64 07
INC $0757               ; $03:87F6: EE 57 07
INC $0770               ; $03:87F9: EE 70 07

; "More difficult quest" flag. Sets $076A too and shows star next to world
LDA $07FC               ; $03:87FC: AD FC 07

; "More difficult quest" flag
STA $076A               ; $03:87FF: 8D 6A 07
...
DATA_038853:
db $0C,$00,$02,$49,$80,$05,$2E,$12
db $24,$10,$24,$10,$FF,$FF

CODE_038861:
REP #$20                ; $03:8861: C2 20
LDY #$0C                ; $03:8863: A0 0C

CODE_038865:
LDA DATA_038853,y       ; $03:8865: B9 53 88
...
; Current demo action in X

CODE_038883:
LDX $0717               ; $03:8883: AE 17 07
LDA $0718               ; $03:8886: AD 18 07

; If timer isn't 0 yet, branch
BNE CODE_0388A4         ; $03:8889: D0 19
INX                     ; $03:888B: E8
...
CODE_03889B:
LDA.l DATA_05EDF4,x     ; $03:889B: BF F4 ED 05

CODE_03889F:
STA $0718               ; $03:889F: 8D 18 07
BEQ CODE_0388BA         ; $03:88A2: F0 16

CODE_0388A4:
LDA $07FB               ; $03:88A4: AD FB 07
BEQ CODE_0388AF         ; $03:88A7: F0 06
LDA.l DATA_05EE0A,x     ; $03:88A9: BF 0A EE 05
BRA CODE_0388B3         ; $03:88AD: 80 04

CODE_0388AF:
LDA.l DATA_05EDDF,x     ; $03:88AF: BF DF ED 05

CODE_0388B3:
STA $0FF4               ; $03:88B3: 8D F4 0F
...
; Princess/toad saved sequence activator

CODE_0388BB:
JSR CODE_0388D0         ; $03:88BB: 20 D0 88
...
CODE_0388CA:
JSR CODE_03FD13         ; $03:88CA: 20 13 FD
JMP CODE_03F710         ; $03:88CD: 4C 10 F7

CODE_0388D0:
LDA $1680               ; $03:88D0: AD 80 16
BNE CODE_0388E4         ; $03:88D3: D0 0F
LDA $075F               ; $03:88D5: AD 5F 07
CMP #$07                ; $03:88D8: C9 07

; If world 8, jump to other pointers
...
; $00 - Collapse bridge if necessary
...
; $0D - Wait until player pushes start button
dw CODE_038989

; $0E - Player chose to continue the game
dw CODE_03898E

CODE_038918:
LDA $0754               ; $03:8918: AD 54 07
...
CODE_038955:
INC $0772               ; $03:8955: EE 72 07
INC $0F87               ; $03:8958: EE 87 0F
RTS                     ; $03:895B: 60

CODE_03895C:
JSL CODE_04DC5F         ; $03:895C: 22 5F DC 04
...
CODE_03896B:
LDA #$0E                ; $03:896B: A9 0E

; Princess got rescued music.
STA $1602               ; $03:896D: 8D 02 16

; Upload new palettes for the kiss sequence
JSL CODE_04D860         ; $03:8970: 22 60 D8 04
JSL CODE_05DE12         ; $03:8974: 22 12 DE 05
INC $0772               ; $03:8978: EE 72 07
RTS                     ; $03:897B: 60

CODE_03897C:
JSL CODE_04DE8B         ; $03:897C: 22 8B DE 04
RTS                     ; $03:8980: 60

CODE_038981:
INC $0772               ; $03:8981: EE 72 07
RTS                     ; $03:8984: 60

CODE_038985:
INC $0772               ; $03:8985: EE 72 07
RTS                     ; $03:8988: 60

; Rescued Peach - Operation $0D

; Wait until player pushes the start button

CODE_038989:
JSL CODE_04862A         ; $03:8989: 22 2A 86 04
RTS                     ; $03:898D: 60

; Rescued Peach - Operation $0E

; Player chose to continue the game

CODE_03898E:
JSL CODE_048640         ; $03:898E: 22 40 86 04
RTS                     ; $03:8992: 60

CODE_038993:
LDA $07B1               ; $03:8993: AD B1 07
CMP #$06                ; $03:8996: C9 06
BCS CODE_0389CB         ; $03:8998: B0 31
LDA $2143               ; $03:899A: AD 43 21
AND #$7F                ; $03:899D: 29 7F
CMP #$4B                ; $03:899F: C9 4B

; If the "Timer converted to score" sound...
; ...is already playing, skip the SFX write
BEQ CODE_0389A8         ; $03:89A1: F0 05
LDA #$4B                ; $03:89A3: A9 4B

; "Timer converted to score" sound
; (At level end)
STA $1603               ; $03:89A5: 8D 03 16

CODE_0389A8:
JSR CODE_03D809         ; $03:89A8: 20 09 D8
LDA $07E9               ; $03:89AB: AD E9 07
ORA $07EA               ; $03:89AE: 0D EA 07
ORA $07EB               ; $03:89B1: 0D EB 07

; Check if timer is 000 during decrement
; at level end. If it is not 000, return
BNE CODE_0389CB         ; $03:89B4: D0 15
LDA #$4C                ; $03:89B6: A9 4C

; Stops the "Timer converted to score" sound
; (at level end)
STA $1603               ; $03:89B8: 8D 03 16
...
CODE_038A21:
LDA $0219               ; $03:8A21: AD 19 02
CMP #$54                ; $03:8A24: C9 54
BCS CODE_038A2C         ; $03:8A26: B0 04

CODE_038A28:
INC $0204               ; $03:8A28: EE 04 02
INY                     ; $03:8A2B: C8
...
CODE_038A4F:
LDA $0204               ; $03:8A4F: AD 04 02
BEQ CODE_038AD0         ; $03:8A52: F0 7C
RTS                     ; $03:8A54: 60

CODE_038A55:
INC $0F7F               ; $03:8A55: EE 7F 0F
...
CODE_038AD0:
INC $0772               ; $03:8AD0: EE 72 07

CODE_038AD3:
RTS                     ; $03:8AD3: 60

CODE_038AD4:
LDA $07B1               ; $03:8AD4: AD B1 07
...
; If current world is 8 or higher, branch
BCS CODE_038B08         ; $03:8AE3: B0 23
LDA #$00                ; $03:8AE5: A9 00
STA $0760               ; $03:8AE7: 8D 60 07
STA $075C               ; $03:8AEA: 8D 5C 07

; Zero out player's current level number (set it to level 1)
; Set operation mode to 0
STA $0772               ; $03:8AED: 8D 72 07

; Increase current world by 1
INC $075F               ; $03:8AF0: EE 5F 07
LDA #$01                ; $03:8AF3: A9 01

; Lock animations (for some reason)
STA $0BA5               ; $03:8AF5: 8D A5 0B

; Swap players if necessary
JSR CODE_03A22B         ; $03:8AF8: 20 2B A2

; Set level number and map type
JSL CODE_04C00B         ; $03:8AFB: 22 0B C0 04

; Increase operation mode to $01 (because it was set to 0 earlier)
INC $0757               ; $03:8AFF: EE 57 07
LDA #$01                ; $03:8B02: A9 01

; Game operation mode = main
STA $0770               ; $03:8B04: 8D 70 07
...
CODE_038B08:
JMP CODE_03A204         ; $03:8B08: 4C 04 A2
RTS                     ; $03:8B0B: 60
...
JSL CODE_048596         ; $03:8B5A: 22 96 85 04
LDA #$05                ; $03:8B5E: A9 05

; 1-UP sound
STA $1603               ; $03:8B60: 8D 03 16

CODE_038B63:
LDA DATA_038B24,y       ; $03:8B63: B9 24 8B
LSR A                   ; $03:8B66: 4A
LSR A                   ; $03:8B67: 4A
LSR A                   ; $03:8B68: 4A
LSR A                   ; $03:8B69: 4A
...
CODE_038B76:
LDX $9E                 ; $03:8B76: A6 9E
LDY $0B58               ; $03:8B78: AC 58 0B

CODE_038B7B:
LDA $0801,y             ; $03:8B7B: B9 01 08
...
CODE_038C12:
STA $0807,y             ; $03:8C12: 99 07 08
LDX $9E                 ; $03:8C15: A6 9E
RTS                     ; $03:8C17: 60

CODE_038C18:
LDA $073C               ; $03:8C18: AD 3C 07
ASL A                   ; $03:8C1B: 0A
TAX                     ; $03:8C1C: AA

; Current screen routine to execute
...
CODE_038C50:
LDA $0744               ; $03:8C50: AD 44 07
PHA                     ; $03:8C53: 48
LDA $0756               ; $03:8C54: AD 56 07

; Save background color and current player status
; to stack
PHA                     ; $03:8C57: 48

; Small powerup
STZ $0756               ; $03:8C58: 9C 56 07
LDA #$02                ; $03:8C5B: A9 02

; Set background color
STA $0744               ; $03:8C5D: 8D 44 07
JSL CODE_049A88         ; $03:8C60: 22 88 9A 04
PLA                     ; $03:8C64: 68
STA $0756               ; $03:8C65: 8D 56 07
PLA                     ; $03:8C68: 68

; Restore background color and current player powerup
STA $0744               ; $03:8C69: 8D 44 07

; To next screen routine

CODE_038C6C:
JMP CODE_038F0D         ; $03:8C6C: 4C 0D 8F

CODE_038C6F:
LDA $5C                 ; $03:8C6F: A5 5C
JSL CODE_05C8FB         ; $03:8C71: 22 FB C8 05
JSL CODE_04956B         ; $03:8C75: 22 6B 95 04
RTS                     ; $03:8C79: 60

CODE_038C7A:
JSL CODE_049A7D         ; $03:8C7A: 22 7D 9A 04
RTS                     ; $03:8C7E: 60

CODE_038C7F:
INC $073C               ; $03:8C7F: EE 3C 07
RTS                     ; $03:8C82: 60

CODE_038C83:
LDA #$00                ; $03:8C83: A9 00
JSR CODE_0390FC         ; $03:8C85: 20 FC 90
JMP CODE_038F0D         ; $03:8C88: 4C 0D 8F

CODE_038C8B:
JSR CODE_03BD5E         ; $03:8C8B: 20 5E BD
...
CODE_038D75:
INC $073C               ; $03:8D75: EE 3C 07
JMP CODE_038F0D         ; $03:8D78: 4C 0D 8F

CODE_038D7B:
LDA $0770               ; $03:8D7B: AD 70 07

; Skip if game operating mode is title screen
BEQ CODE_038DC9         ; $03:8D7E: F0 49
CMP #$03                ; $03:8D80: C9 03

; If game over, display game over instead.
BEQ CODE_038DCF         ; $03:8D82: F0 4B
LDA $0752               ; $03:8D84: AD 52 07

; Branch if an exit is taken
BNE CODE_038DC9         ; $03:8D87: D0 40
LDY $5C                 ; $03:8D89: A4 5C
CPY #$03                ; $03:8D8B: C0 03

; Check for castle level
; If castle, branch
BEQ CODE_038D94         ; $03:8D8D: F0 05
LDA $0769               ; $03:8D8F: AD 69 07

; Go to next screen task if level preview skip flag is set
BNE CODE_038DC9         ; $03:8D92: D0 35

CODE_038D94:
JSR CODE_03F861         ; $03:8D94: 20 61 F8
JSL CODE_0492E7         ; $03:8D97: 22 E7 92 04
LDA #$01                ; $03:8D9B: A9 01

CODE_038D9D:
JSR CODE_0390FC         ; $03:8D9D: 20 FC 90
...
; No screen pixelation/mosaic
STZ $0E7E               ; $03:8DC2: 9C 7E 0E
STZ $0774               ; $03:8DC5: 9C 74 07
RTS                     ; $03:8DC8: 60

CODE_038DC9:
LDA #$08                ; $03:8DC9: A9 08
STA $073C               ; $03:8DCB: 8D 3C 07
RTS                     ; $03:8DCE: 60

CODE_038DCF:
STZ $1204               ; $03:8DCF: 9C 04 12
...
CODE_038E19:
LDA DATA_039030,x       ; $03:8E19: BD 30 90
...
CODE_038E2D:
JSL CODE_0486DF         ; $03:8E2D: 22 DF 86 04

CODE_038E31:
STZ $0EC3               ; $03:8E31: 9C C3 0E
JMP CODE_038F16         ; $03:8E34: 4C 16 8F

CODE_038E37:
LDA $0774               ; $03:8E37: AD 74 07
...
CODE_038E7F:
LDA.w DATA_05D850,y     ; $03:8E7F: B9 50 D8
...
CODE_038EAD:
JMP CODE_038F0D         ; $03:8EAD: 4C 0D 8F
...
CODE_038EF1:
LDA $0770               ; $03:8EF1: AD 70 07
BNE CODE_038F16         ; $03:8EF4: D0 20
LDX #$00                ; $03:8EF6: A2 00

CODE_038EF8:
STA $0300,x             ; $03:8EF8: 9D 00 03
...
CODE_038F0A:
JSR CODE_038861         ; $03:8F0A: 20 61 88

CODE_038F0D:
INC $073C               ; $03:8F0D: EE 3C 07
RTS                     ; $03:8F10: 60

; Write Top Score screen routine task

CODE_038F11:
LDA #$FA                ; $03:8F11: A9 FA
JSR CODE_03BD64         ; $03:8F13: 20 64 BD

; Move to next screen routine task

CODE_038F16:
INC $0772               ; $03:8F16: EE 72 07
RTS                     ; $03:8F19: 60
...
DATA_0390D8:
db $04,$03,$02,$00,$24,$05,$24,$00
db $08,$07,$06,$00

DATA_0390E4:
dw $0000,$0000,$0037,$0037
...
CODE_039109:
LDA $077A               ; $03:9109: AD 7A 07
BNE CODE_03910F         ; $03:910C: D0 01
INY                     ; $03:910E: C8

CODE_03910F:
STY $F3                 ; $03:910F: 84 F3
TYA                     ; $03:9111: 98
ASL A                   ; $03:9112: 0A
...
CODE_039123:
LDA DATA_038F1A,x       ; $03:9123: BD 1A 8F
...
CODE_03917C:
LDA $E5                 ; $03:917C: A5 E5
STA $1710,x             ; $03:917E: 9D 10 17
LDA $E6                 ; $03:9181: A5 E6

CODE_039183:
STA $1712,x             ; $03:9183: 9D 12 17
...
CODE_0391BE:
LDA DATA_0390BA,y       ; $03:91BE: B9 BA 90
...
CODE_0391CF:
LDA DATA_0390CE,y       ; $03:91CF: B9 CE 90
...
CODE_0391F3:
LDA DATA_0390D8,x       ; $03:91F3: BD D8 90
...
CODE_03922D:
LDA $0EC9               ; $03:922D: AD C9 0E

; If done rendering the level, return?
; or: If no objects flag is set, return?
BNE CODE_03922C         ; $03:9230: D0 FA

; 16-bit AXY
REP #$30                ; $03:9232: C2 30
LDY $1A00               ; $03:9234: AC 00 1A

; ??
STY $00                 ; $03:9237: 84 00
LDA $0720               ; $03:9239: AD 20 07

; Load layer 1 drawing handler
; Store it here?
STA $1A02,y             ; $03:923C: 99 02 1A
XBA                     ; $03:923F: EB

; Swap high and low bytes and store it here.
STA $0ECC               ; $03:9240: 8D CC 0E
...
CODE_039327:
LDA $0ECE               ; $03:9327: AD CE 0E
ORA #$0100              ; $03:932A: 09 00 01
STA $0ECE               ; $03:932D: 8D CE 0E

CODE_039330:
JSL CODE_048D71         ; $03:9330: 22 71 8D 04
...
CODE_039384:
JSR CODE_039390         ; $03:9384: 20 90 93

; Increase the counter for the amount of bounce blocks you've created.
INC $03F2               ; $03:9387: EE F2 03
DEC $03EE,x             ; $03:938A: DE EE 03
RTS                     ; $03:938D: 60

CODE_03938E:
LDA #$00                ; $03:938E: A9 00

CODE_039390:
LDY #$03                ; $03:9390: A0 03
...
; Execute Pointer Short routine
;
; (ExecutePtrShort)
JMP ($0006)             ; $03:9B1A: 6C 06 00

; Bunch of DMA routines

CODE_039B1D:
STZ $00                 ; $03:9B1D: 64 00

; Direct page = $4300. DMA mode: 1 reg write once
REP #$20                ; $03:9B1F: C2 20

; OAM address low byte: 00
STZ $2102               ; $03:9B21: 9C 02 21
LDA #$0004              ; $03:9B24: A9 04 00
STA $01                 ; $03:9B27: 85 01
LDA #$0008              ; $03:9B29: A9 08 00

; Dest: data for OAM write $2104
; Source: $00:0800
STA $03                 ; $03:9B2C: 85 03
LDA #$0220              ; $03:9B2E: A9 20 02

; $220 bytes
STA $05                 ; $03:9B31: 85 05
LDX #$01                ; $03:9B33: A2 01
STX $420B               ; $03:9B35: 8E 0B 42

; Enable DMA
SEP #$20                ; $03:9B38: E2 20
LDA #$80                ; $03:9B3A: A9 80
STA $2103               ; $03:9B3C: 8D 03 21

; Regular sprite priority.
; OAM address: $0000
STZ $2102               ; $03:9B3F: 9C 02 21
LDA $0B76               ; $03:9B42: AD 76 0B
BNE CODE_039B54         ; $03:9B45: D0 0D
JSR CODE_039B95         ; $03:9B47: 20 95 9B
LDA $028C               ; $03:9B4A: AD 8C 02

; Flag to disable animated graphics
BNE CODE_039B91         ; $03:9B4D: D0 42
LDA $028D               ; $03:9B4F: AD 8D 02

; Flag to upload animated graphics
BEQ CODE_039B91         ; $03:9B52: F0 3D

CODE_039B54:
REP #$20                ; $03:9B54: C2 20
LDA $028A               ; $03:9B56: AD 8A 02

; VRAM address
STA $2116               ; $03:9B59: 8D 16 21
LDA #$1801              ; $03:9B5C: A9 01 18

; 2 regs write once
; to 2118
STA $00                 ; $03:9B5F: 85 00
LDA $0285               ; $03:9B61: AD 85 02
STA $02                 ; $03:9B64: 85 02
LDY $0287               ; $03:9B66: AC 87 02

; DMA source: [$7E0285]
STY $04                 ; $03:9B69: 84 04
LDA $0288               ; $03:9B6B: AD 88 02

; DMA size: $0288
STA $05                 ; $03:9B6E: 85 05

; enable DMA
STX $420B               ; $03:9B70: 8E 0B 42
...
; Clear flag so graphics animate again

CODE_039B91:
STZ $028C               ; $03:9B91: 9C 8C 02
RTS                     ; $03:9B94: 60

CODE_039B95:
LDA $028E               ; $03:9B95: AD 8E 02
...
; Stripe image uploader. Stripe image format:

CODE_039BBC:
REP #$10                ; $03:9BBC: C2 10
LDA #$03                ; $03:9BBE: A9 03

; VVVVVVVV VVVVVVVV DRLLLLLL LLLLLLLL <data bytes>
; DMA source: bank 3
STA $4314               ; $03:9BC0: 8D 14 43

; Clear something
STZ $06                 ; $03:9BC3: 64 06

; Go into a loop
LDY #$0000              ; $03:9BC5: A0 00 00
LDA [$00],y             ; $03:9BC8: B7 00

; Load next byte
; If positive, go process stripe image
BPL CODE_039BCF         ; $03:9BCA: 10 03
SEP #$30                ; $03:9BCC: E2 30

; else, return.
RTS                     ; $03:9BCE: 60

CODE_039BCF:
STA $04                 ; $03:9BCF: 85 04
INY                     ; $03:9BD1: C8
LDA [$00],y             ; $03:9BD2: B7 00
STA $03                 ; $03:9BD4: 85 03

; Store stripe image VRAM destination in
; RAM $03-$04.
INY                     ; $03:9BD6: C8
LDA [$00],y             ; $03:9BD7: B7 00
AND #$80                ; $03:9BD9: 29 80
ASL A                   ; $03:9BDB: 0A
ROL A                   ; $03:9BDC: 2A
...
; Get RLE bit. 1 = RLE, 0 = normal
;
;
;
; DMA transfer becomes RLE if necessary
STA $4310               ; $03:9BEA: 8D 10 43
LDA #$18                ; $03:9BED: A9 18

; DMA destination: $2118, $2119
STA $4311               ; $03:9BEF: 8D 11 43

; 16-bit A
REP #$20                ; $03:9BF2: C2 20
LDA $03                 ; $03:9BF4: A5 03
...
; Get RLE DMA direction. Either horz or vert
STA $2115               ; $03:9C24: 8D 15 21
LDA #$02                ; $03:9C27: A9 02

; Enable RLE DMA. This will DMA the low bytes of VRAM
; data.
STA $420B               ; $03:9C29: 8D 0B 42
LDA #$19                ; $03:9C2C: A9 19

; RLE DMA dest: $2119
STA $4311               ; $03:9C2E: 8D 11 43
REP #$21                ; $03:9C31: C2 21
TYA                     ; $03:9C33: 98
ADC $00                 ; $03:9C34: 65 00
INC A                   ; $03:9C36: 1A

; Setup RLE DMA source
STA $4312               ; $03:9C37: 8D 12 43
LDA $03                 ; $03:9C3A: A5 03

; Setup RLE VRAM dest
STA $2116               ; $03:9C3C: 8D 16 21

; Setup RLE size
STX $4315               ; $03:9C3F: 8E 15 43
LDX #$0002              ; $03:9C42: A2 02 00

CODE_039C45:
STX $03                 ; $03:9C45: 86 03
TYA                     ; $03:9C47: 98
CLC                     ; $03:9C48: 18
ADC $03                 ; $03:9C49: 65 03
TAY                     ; $03:9C4B: A8

; Increase pointer to stripe image data
SEP #$20                ; $03:9C4C: E2 20
LDA $07                 ; $03:9C4E: A5 07
ORA #$80                ; $03:9C50: 09 80

; Set VRAM DMA mode: Either horz/vert
; normal address increment mode
STA $2115               ; $03:9C52: 8D 15 21
LDA #$02                ; $03:9C55: A9 02

; Enable DMA to upload stripe image to VRAM
STA $420B               ; $03:9C57: 8D 0B 42
LDA [$00],y             ; $03:9C5A: B7 00

; If done, return
BMI CODE_039C61         ; $03:9C5C: 30 03

; Otherwise, process next chunk.
JMP CODE_039BCF         ; $03:9C5E: 4C CF 9B

CODE_039C61:
SEP #$30                ; $03:9C61: E2 30

; Return from stripe image uploading.
RTS                     ; $03:9C63: 60
...
CODE_039D0B:
LDX #$05                ; $03:9D0B: A2 05

CODE_039D0D:
LDA $0145,x             ; $03:9D0D: BD 45 01
CLC                     ; $03:9D10: 18
...
CODE_039D91:
LDA #$80                ; $03:9D91: A9 80

; F-Blank
STA $1201               ; $03:9D93: 8D 01 12
LDA #$FF                ; $03:9D96: A9 FF
STA $1102               ; $03:9D98: 8D 02 11
LDA #$7F                ; $03:9D9B: A9 7F

; Palette 8, color 1 = white
...
CODE_039DE0:
JSR CODE_039F3E         ; $03:9DE0: 20 3E 9F
...
CODE_039E0C:
LDY $075F               ; $03:9E0C: AC 5F 07
CPY #$08                ; $03:9E0F: C0 08

; Load current world
; If it is smaller than world 08
; Skip the "set level number & map type" routine
BCC CODE_039E17         ; $03:9E11: 90 04

; set level number & map type
JSL CODE_04C00B         ; $03:9E13: 22 0B C0 04

CODE_039E17:
LDY #$4B                ; $03:9E17: A0 4B
JSR CODE_039F3E         ; $03:9E19: 20 3E 9F
LDX #$29                ; $03:9E1C: A2 29
LDA #$00                ; $03:9E1E: A9 00

CODE_039E20:
STA $0788,x             ; $03:9E20: 9D 88 07
DEX                     ; $03:9E23: CA
...
CODE_039E95:
INC $06CC               ; $03:9E95: EE CC 06

CODE_039E98:
LDA $075B               ; $03:9E98: AD 5B 07
BEQ CODE_039EA2         ; $03:9E9B: F0 05
LDA #$02                ; $03:9E9D: A9 02
STA $0710               ; $03:9E9F: 8D 10 07

CODE_039EA2:
LDA $DB                 ; $03:9EA2: A5 DB
CMP #$21                ; $03:9EA4: C9 21

; Load background image
; If it is the 'real' Bowser's Castle background
; branch
BEQ CODE_039EB1         ; $03:9EA6: F0 09
CMP #$02                ; $03:9EA8: C9 02

; If it is the underwater Bowser's Castle background
; Branch
BEQ CODE_039EB1         ; $03:9EAA: F0 05
LDA #$80                ; $03:9EAC: A9 80

; Fade out the music
STA $1602               ; $03:9EAE: 8D 02 16

CODE_039EB1:
LDA #$01                ; $03:9EB1: A9 01
STA $0E7F               ; $03:9EB3: 8D 7F 0E
INC $0772               ; $03:9EB6: EE 72 07
RTS                     ; $03:9EB9: 60

CODE_039EBA:
LDA $2142               ; $03:9EBA: AD 42 21
...
CODE_039F1A:
LDA DATA_039D70,x       ; $03:9F1A: BD 70 9D
...
; "More difficult quest" flag. Sets $076A too and shows star next to world
LDA $07FC               ; $03:9F77: AD FC 07

; "More difficult quest" flag
STA $076A               ; $03:9F7A: 8D 6A 07
LDX #$40                ; $03:9F7D: A2 40

CODE_039F7F:
STZ $0F00,x             ; $03:9F7F: 9E 00 0F
INX                     ; $03:9F82: E8
...
; "More difficult quest" flag
STA $076A               ; $03:9F97: 8D 6A 07

; "More difficult quest" flag. Sets 076A too and shows star next to world.
STA $07FC               ; $03:9F9A: 8D FC 07
...
CODE_039FF1:
LDA DATA_039FA2,y       ; $03:9FF1: B9 A2 9F
...
CODE_03A086:
LDY DATA_03A050,x       ; $03:A086: BC 50 A0
...
CODE_03A0B6:
STZ $073C               ; $03:A0B6: 9C 3C 07
STZ $0722               ; $03:A0B9: 9C 22 07
LDA #$0A                ; $03:A0BC: A9 0A
...
CODE_03A101:
STZ $1603               ; $03:A101: 9C 03 16

CODE_03A104:
STY $0F06               ; $03:A104: 8C 06 0F

CODE_03A107:
LDA $0F8A               ; $03:A107: AD 8A 0F
...
CODE_03A132:
LDA $075F               ; $03:A132: AD 5F 07
CMP #$08                ; $03:A135: C9 08
BNE CODE_03A13A         ; $03:A137: D0 01
RTS                     ; $03:A139: 60

CODE_03A13A:
LDA $1603               ; $03:A13A: AD 03 16
BEQ CODE_03A142         ; $03:A13D: F0 03
STZ $0B78               ; $03:A13F: 9C 78 0B

; Frame counter which runs when the game is paused

CODE_03A142:
INC $0B78               ; $03:A142: EE 78 0B
...
CODE_03A155:
LDA DATA_03A0C8,y       ; $03:A155: B9 C8 A0
...
CODE_03A168:
LDA #$20                ; $03:A168: A9 20
STA $0F8A               ; $03:A16A: 8D 8A 0F
LDA #$43                ; $03:A16D: A9 43

; Pause sound
STA $1600               ; $03:A16F: 8D 00 16
LDA #$F3                ; $03:A172: A9 F3
...
; "Save & Continue/Save & Quit" sound
STA $1600               ; $03:A17E: 8D 00 16

CODE_03A181:
JSR CODE_03A142         ; $03:A181: 20 42 A1
...
CODE_03A1C1:
STA $07CE,y             ; $03:A1C1: 99 CE 07
DEY                     ; $03:A1C4: 88
BPL CODE_03A1C1         ; $03:A1C5: 10 FA

; Enable hidden 1-up flag
INC $075D               ; $03:A1C7: EE 5D 07
...
CODE_03A1DE:
JMP CODE_03A1EF         ; $03:A1DE: 4C EF A1

CODE_03A1E1:
JMP CODE_03A0CF         ; $03:A1E1: 4C CF A0

CODE_03A1E4:
JSL CODE_048650         ; $03:A1E4: 22 50 86 04
...
CODE_03A1EF:
JSR CODE_03A22B         ; $03:A1EF: 20 2B A2
BCC CODE_03A204         ; $03:A1F2: 90 10
LDA $075F               ; $03:A1F4: AD 5F 07

; Residual from NES: store world number into secret continue function variable
...
CODE_03A204:
JSL CODE_04C00B         ; $03:A204: 22 0B C0 04
LDA $1680               ; $03:A208: AD 80 16

; Skip clear powerup if new world from rescuing the princess
BNE CODE_03A21A         ; $03:A20B: D0 0D
LDA $077A               ; $03:A20D: AD 7A 07

; Skip clear powerup if 2 players
BNE CODE_03A21A         ; $03:A210: D0 08
LDA #$01                ; $03:A212: A9 01
STA $0754               ; $03:A214: 8D 54 07

; Set small player flag
; Clear powerup
STZ $0756               ; $03:A217: 9C 56 07

; set some timer flag

CODE_03A21A:
INC $0757               ; $03:A21A: EE 57 07

; Clear everything freezes timer
STZ $0747               ; $03:A21D: 9C 47 07

; set player action to 0
STZ $0F                 ; $03:A220: 64 0F

; Operation mode task to execute is 0
STZ $0772               ; $03:A222: 9C 72 07
LDA #$01                ; $03:A225: A9 01

; Operation mode: main
STA $0770               ; $03:A227: 8D 70 07
...
CODE_03A47F:
LDA DATA_03A3C4,x       ; $03:A47F: BD C4 A3
...
CODE_03A4AE:
STZ $0EF0               ; $03:A4AE: 9C F0 0E
BRA CODE_03A4BD         ; $03:A4B1: 80 0A

CODE_03A4B3:
CPY #$03                ; $03:A4B3: C0 03
...
CODE_03A4F7:
LDA DATA_03A416,y       ; $03:A4F7: B9 16 A4
...
CODE_03A50E:
LDY #$00                ; $03:A50E: A0 00

CODE_03A510:
LDA DATA_03CA9F,y       ; $03:A510: B9 9F CA
...
CODE_03A527:
LDA $07                 ; $03:A527: A5 07

CODE_03A529:
STA $06A1,x             ; $03:A529: 9D A1 06
...
CODE_03A58F:
INC $06AD               ; $03:A58F: EE AD 06

CODE_03A592:
JSR CODE_03A5CC         ; $03:A592: 20 CC A5
...
; Increase index to level data by 2. If it's a 3-byte object, increase by 3.
...
; Offset of current level data in index

CODE_03A77D:
LDY $072C               ; $03:A77D: AC 2C 07

; Get level data's high nibble (x-coordinate?)
...
; If vertically extendable object
; Set flag for 3 byte object
...
; If vertically extendable objects, branch
...
; Build vertically extendable objects appropriately and place them into the level
...
CODE_03ACE0:
LDA $0726               ; $03:ACE0: AD 26 07
ASL A                   ; $03:ACE3: 0A
ASL A                   ; $03:ACE4: 0A
ASL A                   ; $03:ACE5: 0A
ASL A                   ; $03:ACE6: 0A

; Turn screen X-coordinates from 16x16 to pixel coordinate
RTS                     ; $03:ACE7: 60

CODE_03ACE8:
LDA $07                 ; $03:ACE8: A5 07
ASL A                   ; $03:ACEA: 0A
ASL A                   ; $03:ACEB: 0A
ASL A                   ; $03:ACEC: 0A
ASL A                   ; $03:ACED: 0A
...
; Account for the statusbar area
...
PHK                     ; $03:AD0C: 4B
PLB                     ; $03:AD0D: AB
JSR CODE_03AC78         ; $03:AD0E: 20 78 AC
PLB                     ; $03:AD11: AB
...
PHK                     ; $03:AD14: 4B
PLB                     ; $03:AD15: AB
JSR CODE_03ACB6         ; $03:AD16: 20 B6 AC
PLB                     ; $03:AD19: AB

; Check if object is fixed or varied length
RTL                     ; $03:AD1A: 6B
...
PHK                     ; $03:AD1C: 4B
PLB                     ; $03:AD1D: AB
JSR CODE_03ACE0         ; $03:AD1E: 20 E0 AC
PLB                     ; $03:AD21: AB
...
PHK                     ; $03:AD24: 4B
PLB                     ; $03:AD25: AB
JSR CODE_03AA5E         ; $03:AD26: 20 5E AA
PLB                     ; $03:AD29: AB

; Jump to get free sprite slot
RTL                     ; $03:AD2A: 6B
...
CODE_03AD70:
JSL CODE_05C860         ; $03:AD70: 22 60 C8 05

CODE_03AD74:
LDA $28                 ; $03:AD74: A5 28
BNE CODE_03AD7B         ; $03:AD76: D0 03
STZ $0E67               ; $03:AD78: 9C 67 0E

CODE_03AD7B:
LDX $0753               ; $03:AD7B: AE 53 07
...
CODE_03AD8F:
JSR CODE_03B680         ; $03:AD8F: 20 80 B6
LDX #$00                ; $03:AD92: A2 00

CODE_03AD94:
STX $9E                 ; $03:AD94: 86 9E
...
CODE_03AD9F:
JSR CODE_03C3B0         ; $03:AD9F: 20 B0 C3
...
CODE_03ADBD:
JSR CODE_03FD9C         ; $03:ADBD: 20 9C FD
...
CODE_03AE23:
JSR CODE_03B2FD         ; $03:AE23: 20 FD B2

CODE_03AE26:
LDA $0A                 ; $03:AE26: A5 0A
STA $0D                 ; $03:AE28: 85 0D
STZ $0C                 ; $03:AE2A: 64 0C

CODE_03AE2C:
LDA $0773               ; $03:AE2C: AD 73 07
...
CODE_03AE4A:
JSR CODE_03A295         ; $03:AE4A: 20 95 A2

CODE_03AE4D:
RTS                     ; $03:AE4D: 60

CODE_03AE4E:
LDA $06FF               ; $03:AE4E: AD FF 06
CLC                     ; $03:AE51: 18
...
CODE_03AED5:
STZ $0775               ; $03:AED5: 9C 75 07

CODE_03AED8:
LDX #$00                ; $03:AED8: A2 00
...
; $00 - Set up level load game timer
...
CODE_03AF6C:
JSR CODE_03B259         ; $03:AF6C: 20 59 B2
...
; Enter pipe/hurt sound
STA $1600               ; $03:AF93: 8D 00 16

CODE_03AF96:
STA $0E4E               ; $03:AF96: 8D 4E 0E
...
CODE_03AFF4:
STZ $0B7A               ; $03:AFF4: 9C 7A 0B

CODE_03AFF7:
LDA $0F                 ; $03:AFF7: A5 0F
...
CODE_03B00D:
STZ $0FF4               ; $03:B00D: 9C F4 0F

CODE_03B010:
LDA $0FF4               ; $03:B010: AD F4 0F
...
CODE_03B035:
LDA $700007             ; $03:B035: AF 07 00 70

; Branch if debug mode
BNE CODE_03B03E         ; $03:B039: D0 03
JMP CODE_03B129         ; $03:B03B: 4C 29 B1

CODE_03B03E:
PHX                     ; $03:B03E: DA
LDX $0EC3               ; $03:B03F: AE C3 0E
LDA $0FF8,x             ; $03:B042: BD F8 0F

; Get controller input
PLX                     ; $03:B045: FA
AND #$C0                ; $03:B046: 29 C0
BEQ CODE_03B060         ; $03:B048: F0 16
AND #$80                ; $03:B04A: 29 80

; Skip if no input
;
; Branch if controller input is $40
BEQ CODE_03B058         ; $03:B04C: F0 0A

; Clear small player flag.  Controller input is $80
STZ $0754               ; $03:B04E: 9C 54 07
LDA #$01                ; $03:B051: A9 01

; Set mushroom powerup
STA $0756               ; $03:B053: 8D 56 07
BRA CODE_03B060         ; $03:B056: 80 08

; Clear small player flag

CODE_03B058:
STZ $0754               ; $03:B058: 9C 54 07
LDA #$02                ; $03:B05B: A9 02

; Set fireflower powerup
STA $0756               ; $03:B05D: 8D 56 07

CODE_03B060:
LDA $0FF6               ; $03:B060: AD F6 0F
AND #$20                ; $03:B063: 29 20

; If select not pressed, branch
BEQ CODE_03B075         ; $03:B065: F0 0E
EOR $0E73               ; $03:B067: 4D 73 0E
STA $0E73               ; $03:B06A: 8D 73 0E
LSR A                   ; $03:B06D: 4A
LSR A                   ; $03:B06E: 4A
LSR A                   ; $03:B06F: 4A
LSR A                   ; $03:B070: 4A
...
; Select pressed.
; Pass through objects debug mode flag
LSR A                   ; $03:B071: 4A

; Disable collision detection
STA $0716               ; $03:B072: 8D 16 07

CODE_03B075:
LDA $0E73               ; $03:B075: AD 73 0E
BNE CODE_03B07D         ; $03:B078: D0 03
JMP CODE_03B129         ; $03:B07A: 4C 29 B1

CODE_03B07D:
LDA #$10                ; $03:B07D: A9 10
...
CODE_03B0C3:
JSL CODE_048596         ; $03:B0C3: 22 96 85 04
JMP CODE_03B12C         ; $03:B0C7: 4C 2C B1

CODE_03B0CA:
JMP CODE_03B12C         ; $03:B0CA: 4C 2C B1

CODE_03B0CD:
LDA $09                 ; $03:B0CD: A5 09
...
CODE_03B129:
JSR CODE_03B373         ; $03:B129: 20 73 B3

CODE_03B12C:
LDY #$01                ; $03:B12C: A0 01
...
CODE_03B148:
STA $46                 ; $03:B148: 85 46

CODE_03B14A:
JSR CODE_03AE4E         ; $03:B14A: 20 4E AE
...
CODE_03B17F:
LDA $0256               ; $03:B17F: AD 56 02
AND #$CE                ; $03:B182: 29 CE
ORA #$20                ; $03:B184: 09 20

CODE_03B186:
STA $0256               ; $03:B186: 8D 56 02

CODE_03B189:
LDA $BB                 ; $03:B189: A5 BB
...
CODE_03B217:
JSR CODE_03AE4E         ; $03:B217: 20 4E AE
...
CODE_03B31D:
JMP CODE_03AFEA         ; $03:B31D: 4C EA AF

CODE_03B320:
INC $0F                 ; $03:B320: E6 0F
RTS                     ; $03:B322: 60
...
CODE_03B32B:
JSL CODE_05CBE5         ; $03:B32B: 22 E5 CB 05
...
; Enable hidden 1-up flag
INC $075D               ; $03:B35C: EE 5D 07

CODE_03B35F:
INC $0760               ; $03:B35F: EE 60 07
STZ $075B               ; $03:B362: 9C 5B 07
JSR CODE_03A22B         ; $03:B365: 20 2B A2

CODE_03B368:
JSL CODE_04C00B         ; $03:B368: 22 0B C0 04
...
CODE_03B382:
STA $0714               ; $03:B382: 8D 14 07

CODE_03B385:
JSR CODE_03B4AD         ; $03:B385: 20 AD B4
...
CODE_03B3DB:
LDA $070A               ; $03:B3DB: AD 0A 07
STA $0709               ; $03:B3DE: 8D 09 07

CODE_03B3E1:
LDA $0704               ; $03:B3E1: AD 04 07
...
BNE CODE_03B4CF         ; $03:B4C5: D0 08
LDA #$30                ; $03:B4C7: A9 30

; Each 7 frames,
;
; Mario is climbing vine sound plays.
STA $1600               ; $03:B4C9: 8D 00 16
...
CODE_03B4CF:
LDX DATA_03B4AA,y       ; $03:B4CF: BE AA B4
...
CODE_03B4DF:
STA $070C               ; $03:B4DF: 8D 0C 07
RTS                     ; $03:B4E2: 60

CODE_03B4E3:
LDA $070E               ; $03:B4E3: AD 0E 07
...
CODE_03B4F2:
JMP CODE_03B580         ; $03:B4F2: 4C 80 B5

CODE_03B4F5:
LDA $28                 ; $03:B4F5: A5 28
...
CODE_03B54F:
LDA DATA_03B481,y       ; $03:B54F: B9 81 B4
...
LDA #$0E                ; $03:B56B: A9 0E

; Mario is swimming sound
...
CODE_03B57B:
LDA #$01                ; $03:B57B: A9 01

; Mario is jumping sound
STA $1601               ; $03:B57D: 8D 01 16

CODE_03B580:
LDY #$00                ; $03:B580: A0 00
...
CODE_03B5B7:
INC $00                 ; $03:B5B7: E6 00
BRA CODE_03B5C0         ; $03:B5B9: 80 05

CODE_03B5BB:
LDA #$0A                ; $03:B5BB: A9 0A
STA $078B               ; $03:B5BD: 8D 8B 07

CODE_03B5C0:
LDA DATA_03B49D,y       ; $03:B5C0: B9 9D B4
...
CODE_03B5CE:
LDA DATA_03B4A0,y       ; $03:B5CE: B9 A0 B4
...
CODE_03B60E:
STA $0703               ; $03:B60E: 8D 03 07
BRA CODE_03B624         ; $03:B611: 80 11

CODE_03B613:
LDA $0700               ; $03:B613: AD 00 07
...
CODE_03B624:
LDA DATA_03B5ED,y       ; $03:B624: B9 ED B5
STA $070C               ; $03:B627: 8D 0C 07
RTS                     ; $03:B62A: 60

CODE_03B62B:
AND $0480               ; $03:B62B: 2D 80 04
...
CODE_03B67C:
STA $0700               ; $03:B67C: 8D 00 07
RTS                     ; $03:B67F: 60

CODE_03B680:
LDA $0756               ; $03:B680: AD 56 07
...
LDA #$06                ; $03:B6AB: A9 06

; Mario threw a fireball sound
...
; Fireball Xspeed: Right, left

DATA_03B6E4:
db $40,$C0

CODE_03B6E6:
STX $9E                 ; $03:B6E6: 86 9E
...
CODE_03B6F0:
LDY $33,x               ; $03:B6F0: B4 33
BNE CODE_03B6F7         ; $03:B6F2: D0 03

; If fireball exists in any form,
; Go to fireball throw routine
; Otherwise, return
JMP CODE_03B779         ; $03:B6F4: 4C 79 B7
...
LDA $0219               ; $03:B6FA: AD 19 02
ADC #$04                ; $03:B6FD: 69 04

; Load Mario's Xpos (Xpos in single page)
; Add $04/$05 to it (Depends on C flag)
; Store into Fireball creation Xpos
STA $0224,x             ; $03:B6FF: 9D 24 02
LDA $78                 ; $03:B702: A5 78
ADC #$00                ; $03:B704: 69 00

; Load Mario's Xpos (Page # player is in)
; Add $00/$01 to it (Depends on C flag)
; Store into page of Fireball (not sure)
STA $83,x               ; $03:B706: 95 83
LDA $0237               ; $03:B708: AD 37 02

; Load Mario's Ypos
; Store into Ypos of fireball
STA $0242,x             ; $03:B70B: 9D 42 02
LDA #$01                ; $03:B70E: A9 01

; Set the "Mario threw a fireball" flag
STA $C6,x               ; $03:B710: 95 C6
LDY $0202               ; $03:B712: AC 02 02
DEY                     ; $03:B715: 88
LDA DATA_03B6E4,y       ; $03:B716: B9 E4 B6

; Mario's direction
; Decrease with 1
; Load Horizontal speeds
; Store into Fireball horizontal speed
STA $68,x               ; $03:B719: 95 68
LDA #$04                ; $03:B71B: A9 04

; Y-speed of the fireball when it gets thrown
; Which is of course, downwards
STA $AB,x               ; $03:B71D: 95 AB
LDA #$07                ; $03:B71F: A9 07
STA $049A,x             ; $03:B721: 9D 9A 04

; Decrease fireball state timer
DEC $33,x               ; $03:B724: D6 33
...
; Impose gravity
JSR CODE_03C18B         ; $03:B735: 20 8B C1
...
CODE_03B76F:
JSR CODE_03DC70         ; $03:B76F: 20 70 DC

CODE_03B772:
JMP CODE_03F443         ; $03:B772: 4C 43 F4

CODE_03B775:
LDA #$00                ; $03:B775: A9 00
...
CODE_03B77A:
JSR CODE_03FD23         ; $03:B77A: 20 23 FD
JMP CODE_03F4A8         ; $03:B77D: 4C A8 F4

CODE_03B780:
LDA $07B8,x             ; $03:B780: BD B8 07
...
LDA $078F               ; $03:B7F3: AD 8F 07

; If timer countdown timer isn't, $00 yet, branch
BNE CODE_03B838         ; $03:B7F6: D0 40
LDA $07E9               ; $03:B7F8: AD E9 07
ORA $07EA               ; $03:B7FB: 0D EA 07
ORA $07EB               ; $03:B7FE: 0D EB 07

; Branch if timer is 000
BEQ CODE_03B82F         ; $03:B801: F0 2C
LDY $07E9               ; $03:B803: AC E9 07
DEY                     ; $03:B806: 88

; Check hundreds, if not $00, branch
BNE CODE_03B816         ; $03:B807: D0 0D
LDA $07EA               ; $03:B809: AD EA 07
ORA $07EB               ; $03:B80C: 0D EB 07

; Check the tens and ones. If not $00, branch
BNE CODE_03B816         ; $03:B80F: D0 05
LDA #$FF                ; $03:B811: A9 FF

; Time is running out! sound
STA $1600               ; $03:B813: 8D 00 16

CODE_03B816:
LDA #$18                ; $03:B816: A9 18

; Control the timer countdown speed
STA $078F               ; $03:B818: 8D 8F 07
...
CODE_03B82A:
LDA #$A4                ; $03:B82A: A9 A4
JMP CODE_039C82         ; $03:B82C: 4C 82 9C

CODE_03B82F:
STA $0756               ; $03:B82F: 8D 56 07
...
; Erase sprite
JMP CODE_03CDE2         ; $03:B84B: 4C E2 CD

CODE_03B84E:
LDA $5C                 ; $03:B84E: A5 5C
...
CODE_03B8D9:
STA $78                 ; $03:B8D9: 85 78

CODE_03B8DB:
LDA #$10                ; $03:B8DB: A9 10
...
; Impose gravity
JMP CODE_03C18B         ; $03:B8E8: 4C 8B C1
...
CODE_03B93D:
BRA CODE_03B982         ; $03:B93D: 80 43

CODE_03B93F:
LDY $010F               ; $03:B93F: AC 0F 01
...
CODE_03B962:
LDA #$60                ; $03:B962: A9 60

CODE_03B964:
STA $02FD               ; $03:B964: 8D FD 02
...
CODE_03B982:
JSR CODE_03FDCB         ; $03:B982: 20 CB FD
...
CODE_03B991:
JSR CODE_03FDCB         ; $03:B991: 20 CB FD
...
CODE_03B9AE:
DEC $0237               ; $03:B9AE: CE 37 02
DEC $0237               ; $03:B9B1: CE 37 02

CODE_03B9B4:
LDA $5E,x               ; $03:B9B4: B5 5E
CLC                     ; $03:B9B6: 18
...
; Springboard launch force
STA $06DB               ; $03:B9CD: 8D DB 06

CODE_03B9D0:
CPY #$03                ; $03:B9D0: C0 03
BNE CODE_03B9E1         ; $03:B9D2: D0 0D
LDA $06DB               ; $03:B9D4: AD DB 06

; Store springboard launch force into player Y speed
STA $A0                 ; $03:B9D7: 85 A0
STZ $070E               ; $03:B9D9: 9C 0E 07
LDA #$08                ; $03:B9DC: A9 08

; Springboard sound
STA $1603               ; $03:B9DE: 8D 03 16

CODE_03B9E1:
JSR CODE_03FD39         ; $03:B9E1: 20 39 FD
...
CODE_03B9FD:
LDA #$2F                ; $03:B9FD: A9 2F

; Sprite to generate: Vine
STA $1C,x               ; $03:B9FF: 95 1C
LDA #$01                ; $03:BA01: A9 01

; Sprite on screen
STA $10,x               ; $03:BA03: 95 10
LDA $0085,y             ; $03:BA05: B9 85 00

; Spawn in current screen of course
STA $79,x               ; $03:BA08: 95 79
LDA $0226,y             ; $03:BA0A: B9 26 02

; Spawn at the brick
; Store into Sprite X-Position
STA $021A,x             ; $03:BA0D: 9D 1A 02
LDA $0244,y             ; $03:BA10: B9 44 02
BNE CODE_03BA17         ; $03:BA13: D0 02
LDA #$F0                ; $03:BA15: A9 F0

; If brick position is at the very top
; Spawn vine from the very bottom
; (Used in level entrances with the vine animation?)

CODE_03BA17:
STA $0238,x             ; $03:BA17: 9D 38 02
...
; Erase sprite
JSR CODE_03CDE2         ; $03:BA76: 20 E2 CD
STA $0398               ; $03:BA79: 8D 98 03
STA $0399               ; $03:BA7C: 8D 99 03

CODE_03BA7F:
LDA $0399               ; $03:BA7F: AD 99 03
...
; Sprite to generate: Bullet bill
STA $1C,x               ; $03:BAFB: 95 1C
BRA CODE_03BB12         ; $03:BAFD: 80 13

CODE_03BAFF:
LDA $1C,x               ; $03:BAFF: B5 1C
...
CODE_03BB7F:
JSR CODE_03CDE2         ; $03:BB7F: 20 E2 CD
RTS                     ; $03:BB82: 60
...
; Set extended sprite clipping size?
STA $049C,y             ; $03:BBB4: 99 9C 04
SEC                     ; $03:BBB7: 38
RTS                     ; $03:BBB8: 60

CODE_03BBB9:
LDX $9E                 ; $03:BBB9: A6 9E
CLC                     ; $03:BBBB: 18

; Get sprite index and clear carry flag
RTS                     ; $03:BBBC: 60

CODE_03BBBD:
LDA $0747               ; $03:BBBD: AD 47 07
BEQ CODE_03BBC5         ; $03:BBC0: F0 03
JMP CODE_03BC4B         ; $03:BBC2: 4C 4B BC

CODE_03BBC5:
LDA $39,x               ; $03:BBC5: B5 39
...
DEX                     ; $03:BBFE: CA
LDA DATA_03BB8C,x       ; $03:BBFF: BD 8C BB
LDX $9E                 ; $03:BC02: A6 9E

; Hammer bros hammer x-speed depending on direction
STA $6E,x               ; $03:BC04: 95 6E

CODE_03BC06:
DEC $39,x               ; $03:BC06: D6 39
LDA $39,x               ; $03:BC08: B5 39
CMP #$81                ; $03:BC0A: C9 81
BNE CODE_03BC13         ; $03:BC0C: D0 05
LDA #$37                ; $03:BC0E: A9 37

; Hammer thrown sound
STA $1600               ; $03:BC10: 8D 00 16

CODE_03BC13:
LDA $001C,y             ; $03:BC13: B9 1C 00
...
CODE_03BC48:
JSR CODE_03DDBE         ; $03:BC48: 20 BE DD

CODE_03BC4B:
JSR CODE_03FDB7         ; $03:BC4B: 20 B7 FD
...
CODE_03BC59:
JSR CODE_03BCA7         ; $03:BC59: 20 A7 BC
...
CODE_03BC73:
JSR CODE_03BCA7         ; $03:BC73: 20 A7 BC
...
CODE_03BCA7:
LDY #$08                ; $03:BCA7: A0 08

CODE_03BCA9:
LDA $0039,y             ; $03:BCA9: B9 39 00
...
CODE_03BCB5:
STY $06B7               ; $03:BCB5: 8C B7 06
RTS                     ; $03:BCB8: 60

; extended sprites MAIN core

CODE_03BCB9:
LDX #$08                ; $03:BCB9: A2 08

CODE_03BCBB:
STX $9E                 ; $03:BCBB: 86 9E
LDA $39,x               ; $03:BCBD: B5 39

; Iterate through 9 slots of extended sprites
;
; Continue iterating if empty slot.
BEQ CODE_03BD22         ; $03:BCBF: F0 61
ASL A                   ; $03:BCC1: 0A

; If bit 7 set, it's a jumping coin (from [?] blocks)
BCC CODE_03BCCA         ; $03:BCC2: 90 06

; Otherwise, it's a hammer
JSR CODE_03BBBD         ; $03:BCC4: 20 BD BB

; Continue iterating
JMP CODE_03BD22         ; $03:BCC7: 4C 22 BD

; proc jumping coin

CODE_03BCCA:
LDY $39,x               ; $03:BCCA: B4 39
DEY                     ; $03:BCCC: 88

; Check sprite state. if #$01, run jumping coin.
BEQ CODE_03BCEE         ; $03:BCCD: F0 1F
INC $39,x               ; $03:BCCF: F6 39
LDA $022A,x             ; $03:BCD1: BD 2A 02
CLC                     ; $03:BCD4: 18
ADC $0775               ; $03:BCD5: 6D 75 07
STA $022A,x             ; $03:BCD8: 9D 2A 02
LDA $89,x               ; $03:BCDB: B5 89
ADC #$00                ; $03:BCDD: 69 00

; Add camera scrolling X-Speed to the sprite X position and store it there.
STA $89,x               ; $03:BCDF: 95 89
LDA $39,x               ; $03:BCE1: B5 39
CMP #$30                ; $03:BCE3: C9 30
BNE CODE_03BD0D         ; $03:BCE5: D0 26
LDA #$00                ; $03:BCE7: A9 00

; Check if floating score reached peak. If so, clear this jumping coin.
STA $39,x               ; $03:BCE9: 95 39

; Continue iterating through extended sprites.
JMP CODE_03BD22         ; $03:BCEB: 4C 22 BD
...
; Increase index for next subroutine
TAX                     ; $03:BCF2: AA
LDA #$50                ; $03:BCF3: A9 50

; set downward movement amount
STA $00                 ; $03:BCF5: 85 00
LDA #$06                ; $03:BCF7: A9 06
...
; Impose gravity
JSR CODE_03C18B         ; $03:BD00: 20 8B C1
...
; Increase lives by one
JSL CODE_048596         ; $03:BD47: 22 96 85 04
LDA #$05                ; $03:BD4B: A9 05

; 1-up sound
STA $1603               ; $03:BD4D: 8D 03 16

CODE_03BD50:
LDA #$02                ; $03:BD50: A9 02
STA $0149               ; $03:BD52: 8D 49 01

CODE_03BD55:
LDX $0753               ; $03:BD55: AE 53 07
LDY DATA_03BD28,x       ; $03:BD58: BC 28 BD
JSR CODE_039CF9         ; $03:BD5B: 20 F9 9C

CODE_03BD5E:
LDY $0EC3               ; $03:BD5E: AC C3 0E
LDA DATA_03BD2A,y       ; $03:BD61: B9 2A BD

CODE_03BD64:
JSR CODE_039C82         ; $03:BD64: 20 82 9C
...
CODE_03BD7A:
STA $16F6,y             ; $03:BD7A: 99 F6 16

CODE_03BD7D:
LDX $9E                 ; $03:BD7D: A6 9E
RTS                     ; $03:BD7F: 60

CODE_03BD80:
LDA #$2E                ; $03:BD80: A9 2E
...
CODE_03BDB5:
STA $020C               ; $03:BDB5: 8D 0C 02

CODE_03BDB8:
LDA #$30                ; $03:BDB8: A9 30
STA $0260               ; $03:BDBA: 8D 60 02
LDA #$02                ; $03:BDBD: A9 02

; Item out of [?] box sound
STA $1603               ; $03:BDBF: 8D 03 16
RTS                     ; $03:BDC2: 60

CODE_03BDC3:
LDX #$09                ; $03:BDC3: A2 09
...
CODE_03BE60:
JSR CODE_03FD39         ; $03:BE60: 20 39 FD
...
; code related to breaking bricks.

CODE_03BE75:
PHA                     ; $03:BE75: 48

; Make brick bounce block shatter
LDA #$11                ; $03:BE76: A9 11

; Get bounce block index
LDX $03F0               ; $03:BE78: AE F0 03
LDY $0754               ; $03:BE7B: AC 54 07

; Skip next instruction if big Mario
BNE CODE_03BE82         ; $03:BE7E: D0 02
LDA #$12                ; $03:BE80: A9 12

; Make brick bounce block bounce

CODE_03BE82:
STA $35,x               ; $03:BE82: 95 35
...
CODE_03BF01:
JSR CODE_03BF2C         ; $03:BF01: 20 2C BF

CODE_03BF04:
LDA $03F0               ; $03:BF04: AD F0 03
EOR #$01                ; $03:BF07: 49 01
STA $03F0               ; $03:BF09: 8D F0 03
RTS                     ; $03:BF0C: 60

CODE_03BF0D:
LDA $0219               ; $03:BF0D: AD 19 02
CLC                     ; $03:BF10: 18
...
CODE_03BF2C:
JSR CODE_03BFC6         ; $03:BF2C: 20 C6 BF
LDA #$01                ; $03:BF2F: A9 01

; "Mario hits ceiling/shell hits wall" sound
STA $1600               ; $03:BF31: 8D 00 16

; Horizontal speed of bounce blocks: none
STZ $6A,x               ; $03:BF34: 74 6A
STZ $0449,x             ; $03:BF36: 9E 49 04

; No vertical speed of Mario
STZ $A0                 ; $03:BF39: 64 A0
LDA #$FE                ; $03:BF3B: A9 FE

; Vertical speed of bounce blocks
...
CODE_03BF6E:
LDA #$00                ; $03:BF6E: A9 00

; Spawn Mushroom
BRA CODE_03BF78         ; $03:BF70: 80 06

CODE_03BF72:
LDA #$02                ; $03:BF72: A9 02

; Spawn Star
BRA CODE_03BF78         ; $03:BF74: 80 02

CODE_03BF76:
LDA #$03                ; $03:BF76: A9 03

; Spawn 1-Up

CODE_03BF78:
STA $020C               ; $03:BF78: 8D 0C 02
JMP CODE_03BD80         ; $03:BF7B: 4C 80 BD

CODE_03BF7E:
LDX #$09                ; $03:BF7E: A2 09
LDY $03F0               ; $03:BF80: AC F0 03

; Spawn vine?
; Sprite index: $09
; Y = ???
JSR CODE_03B9FD         ; $03:BF83: 20 FD B9
...
CODE_03BF95:
LDY #$0D                ; $03:BF95: A0 0D

CODE_03BF97:
CMP DATA_03BF87,y       ; $03:BF97: D9 87 BF
...
CODE_03C062:
JSR CODE_03C153         ; $03:C062: 20 53 C1
...
CODE_03C081:
STZ $35,x               ; $03:C081: 74 35

CODE_03C083:
RTS                     ; $03:C083: 60

CODE_03C084:
LDX #$01                ; $03:C084: A2 01

CODE_03C086:
STX $9E                 ; $03:C086: 86 9E
...
CODE_03C0B4:
LDA $070E               ; $03:C0B4: AD 0E 07
BNE CODE_03C0FC         ; $03:C0B7: D0 43
TAX                     ; $03:C0B9: AA

CODE_03C0BA:
LDA $5D,x               ; $03:C0BA: B5 5D
ASL A                   ; $03:C0BC: 0A
ASL A                   ; $03:C0BD: 0A
ASL A                   ; $03:C0BE: 0A
ASL A                   ; $03:C0BF: 0A
...
CODE_03C136:
LDY #$7F                ; $03:C136: A0 7F
BNE CODE_03C13C         ; $03:C138: D0 02

CODE_03C13A:
LDY #$0F                ; $03:C13A: A0 0F

CODE_03C13C:
LDA #$02                ; $03:C13C: A9 02
BNE CODE_03C144         ; $03:C13E: D0 04

CODE_03C140:
LDY #$1C                ; $03:C140: A0 1C

CODE_03C142:
LDA #$03                ; $03:C142: A9 03

CODE_03C144:
STY $00                 ; $03:C144: 84 00
INX                     ; $03:C146: E8
...
CODE_03C181:
JSR CODE_03C18B         ; $03:C181: 20 8B C1
...
CODE_03C187:
JSR CODE_03C18B         ; $03:C187: 20 8B C1
RTL                     ; $03:C18A: 6B
...
CODE_03C2AB:
DEC $1201               ; $03:C2AB: CE 01 12
BNE CODE_03C317         ; $03:C2AE: D0 67

CODE_03C2B0:
LDA #$80                ; $03:C2B0: A9 80
...
CODE_03C2D3:
JML CODE_0080DE         ; $03:C2D3: 5C DE 80 00

CODE_03C2D7:
JSL CODE_048000         ; $03:C2D7: 22 00 80 04

; Hide all OAM tiles
JSR CODE_03C33B         ; $03:C2DB: 20 3B C3
JSR CODE_03C366         ; $03:C2DE: 20 66 C3

CODE_03C2E1:
STZ $0E7F               ; $03:C2E1: 9C 7F 0E
LDA #$01                ; $03:C2E4: A9 01
STA $0774               ; $03:C2E6: 8D 74 07
BRA CODE_03C317         ; $03:C2E9: 80 2C

CODE_03C2EB:
LDA $0E66               ; $03:C2EB: AD 66 0E
...
CODE_03C30F:
STZ $0E7F               ; $03:C30F: 9C 7F 0E
STZ $0774               ; $03:C312: 9C 74 07
STZ $0E                 ; $03:C315: 64 0E

CODE_03C317:
LDA $0E4F               ; $03:C317: AD 4F 0E

; Skip if no pipe fadeout
BEQ CODE_03C33A         ; $03:C31A: F0 1E

; Hide all OAM tiles
JSR CODE_03C33B         ; $03:C31C: 20 3B C3
LDA $1201               ; $03:C31F: AD 01 12
EOR #$0F                ; $03:C322: 49 0F
ASL A                   ; $03:C324: 0A
ASL A                   ; $03:C325: 0A
ASL A                   ; $03:C326: 0A
ASL A                   ; $03:C327: 0A
ORA #$0F                ; $03:C328: 09 0F

; Load brightness
; Inverse the last nibble
; \<<4
; |
; |
; /
; Affect all the layers
; Store into mosaic register
STA $0E7E               ; $03:C32A: 8D 7E 0E

; Also hide player's OAM tiles for good measure (even though they were hidden already)
JSR CODE_03C354         ; $03:C32D: 20 54 C3
LDA $1201               ; $03:C330: AD 01 12
CMP #$0F                ; $03:C333: C9 0F
BNE CODE_03C33A         ; $03:C335: D0 03

; Clear pipe fading flag if brightness is maximum
...
CODE_03C340:
STA $0801,x             ; $03:C340: 9D 01 08
STA $0901,x             ; $03:C343: 9D 01 09
STZ $0C00,x             ; $03:C346: 9E 00 0C
STZ $0D00,x             ; $03:C349: 9E 00 0D
INX                     ; $03:C34C: E8
INX                     ; $03:C34D: E8
INX                     ; $03:C34E: E8
INX                     ; $03:C34F: E8
...
; Hides all OAM tiles by setting their Y position off-screen
; and also zeroing out something
RTS                     ; $03:C353: 60
...
; Hides player's OAM tiles by setting their Y position off-screen
RTS                     ; $03:C365: 60

CODE_03C366:
JSL CODE_04ED2E         ; $03:C366: 22 2E ED 04
...
CODE_03C392:
STZ $0E66               ; $03:C392: 9C 66 0E

CODE_03C395:
STZ $0E                 ; $03:C395: 64 0E
RTS                     ; $03:C397: 60

ORG $03C3B0

CODE_03C3B0:
LDA $10,x               ; $03:C3B0: B5 10
PHA                     ; $03:C3B2: 48
ASL A                   ; $03:C3B3: 0A
...
CODE_03C3F5:
LDA $78                 ; $03:C3F5: A5 78
SEC                     ; $03:C3F7: 38
SBC #$04                ; $03:C3F8: E9 04

; Take the player back 4 pages
STA $78                 ; $03:C3FA: 85 78
LDA $0725               ; $03:C3FC: AD 25 07
SEC                     ; $03:C3FF: 38
SBC #$04                ; $03:C400: E9 04

; Take the screen number back 4 pages
STA $0725               ; $03:C402: 8D 25 07
LDA $071A               ; $03:C405: AD 1A 07
SEC                     ; $03:C408: 38
SBC #$04                ; $03:C409: E9 04

; Take coordinates of left side of screen back 4 pages
STA $071A               ; $03:C40B: 8D 1A 07
LDA $071B               ; $03:C40E: AD 1B 07
SEC                     ; $03:C411: 38
SBC #$04                ; $03:C412: E9 04

; Take coordinates of right sight of screen back 4 pages
STA $071B               ; $03:C414: 8D 1B 07
LDA $072A               ; $03:C417: AD 2A 07
SEC                     ; $03:C41A: 38
SBC #$04                ; $03:C41B: E9 04

; Take screen number relative to page skips back 4 pages
STA $072A               ; $03:C41D: 8D 2A 07
LDA #$00                ; $03:C420: A9 00
STA $073B               ; $03:C422: 8D 3B 07
STA $072B               ; $03:C425: 8D 2B 07

; Zero out 16-bit enemy data pointer index
STA $0739               ; $03:C428: 8D 39 07

; Zero out page of the most recent sprite
STA $073A               ; $03:C42B: 8D 3A 07
PHX                     ; $03:C42E: DA
TYX                     ; $03:C42F: BB
LDA.l DATA_04C000,x     ; $03:C430: BF 00 C0 04

; Set level object data pointer index according to
STA $072C               ; $03:C434: 8D 2C 07
PLX                     ; $03:C437: FA
STZ $0EDB               ; $03:C438: 9C DB 0E
RTS                     ; $03:C43B: 60

CODE_03C43C:
LDA $0745               ; $03:C43C: AD 45 07
BNE CODE_03C444         ; $03:C43F: D0 03
JMP CODE_03C4C7         ; $03:C441: 4C C7 C4

CODE_03C444:
LDA $0726               ; $03:C444: AD 26 07
...
CODE_03C482:
LDA $0EDB               ; $03:C482: AD DB 0E
BNE CODE_03C48F         ; $03:C485: D0 08
LDA #$2A                ; $03:C487: A9 2A

; "Incorrect" sound
STA $1603               ; $03:C489: 8D 03 16
STA $0EDB               ; $03:C48C: 8D DB 0E

CODE_03C48F:
INC $06DA               ; $03:C48F: EE DA 06
...
; Branch if all maze checkpoints taken correctly
BRA CODE_03C4BC         ; $03:C4A0: 80 1A

CODE_03C4A2:
LDA $075F               ; $03:C4A2: AD 5F 07
CMP #$06                ; $03:C4A5: C9 06
...
; "Incorrect" sound
STA $1603               ; $03:C4B0: 8D 03 16
STA $0EDB               ; $03:C4B3: 8D DB 0E

CODE_03C4B6:
JSR CODE_03C3F5         ; $03:C4B6: 20 F5 C3
JSR CODE_03D56B         ; $03:C4B9: 20 6B D5

CODE_03C4BC:
LDA #$00                ; $03:C4BC: A9 00
STA $06DA               ; $03:C4BE: 8D DA 06
STA $06D9               ; $03:C4C1: 8D D9 06

; Zero out correct checkpoints counter
; The total checkpoints counter

CODE_03C4C4:
STZ $0745               ; $03:C4C4: 9C 45 07

CODE_03C4C7:
LDA $06CD               ; $03:C4C7: AD CD 06
...
CODE_03C4DA:
LDY $0739               ; $03:C4DA: AC 39 07
LDA [$FD],y             ; $03:C4DD: B7 FD
CMP #$FF                ; $03:C4DF: C9 FF

; Get offset of enemy sprite data
; Check first byte
; If end of data, continue
BNE CODE_03C4E6         ; $03:C4E1: D0 03

; else, jump
JMP CODE_03C5B6         ; $03:C4E3: 4C B6 C5

CODE_03C4E6:
AND #$0F                ; $03:C4E6: 29 0F
...
CODE_03C5D0:
JMP CODE_03CB2B         ; $03:C5D0: 4C 2B CB
...
CODE_03C5EF:
BRA CODE_03C5FC         ; $03:C5EF: 80 0B

CODE_03C5F1:
LDY $0739               ; $03:C5F1: AC 39 07
...
CODE_03C5FC:
INC $0739               ; $03:C5FC: EE 39 07

CODE_03C5FF:
INC $0739               ; $03:C5FF: EE 39 07
INC $0739               ; $03:C602: EE 39 07
STZ $073B               ; $03:C605: 9C 3B 07
LDX $9E                 ; $03:C608: A6 9E
RTS                     ; $03:C60A: 60

; Load Sprite number into A

CODE_03C60B:
LDA $1C,x               ; $03:C60B: B5 1C
CMP #$15                ; $03:C60D: C9 15
BCS CODE_03C620         ; $03:C60F: B0 0F

; If it is #15 or greater
; Branch
; Transfer A to Y
TAY                     ; $03:C611: A8
LDA $0238,x             ; $03:C612: BD 38 02
ADC #$08                ; $03:C615: 69 08

; no need for CLC since the CMP above did that.
; sprites $1-$14: Add 8 pixels to the sprite Y position
STA $0238,x             ; $03:C617: 9D 38 02
LDA #$01                ; $03:C61A: A9 01

; Set initialized flag?
STA $03D9,x             ; $03:C61C: 9D D9 03
TYA                     ; $03:C61F: 98

CODE_03C620:
ASL A                   ; $03:C620: 0A

; Sprite number*2, for index purposes
TAY                     ; $03:C621: A8
LDA PNTR_03C62F,y       ; $03:C622: B9 2F C6
STA $04                 ; $03:C625: 85 04
LDA PNTR_03C62F+1,y     ; $03:C627: B9 30 C6
STA $05                 ; $03:C62A: 85 05

; Load sprite pointers and jump to them
; for the INIT routine
...
CODE_03C69E:
JSR CODE_03C6D3         ; $03:C69E: 20 D3 C6
JMP CODE_03C707         ; $03:C6A1: 4C 07 C7

CODE_03C6A4:
LDA #$D0                ; $03:C6A4: A9 D0

; Podoboo init
; Set Ypos to bottom of the screen
STA $0238,x             ; $03:C6A6: 9D 38 02
...
; Reset some timer
STZ $0B09,x             ; $03:C6B5: 9E 09 0B
LDA #$27                ; $03:C6B8: A9 27
...
CODE_03C6C0:
LDA $075F               ; $03:C6C0: AD 5F 07
CMP #$07                ; $03:C6C3: C9 07

; INIT routine of Toad/Peach. Load current world
; If you are NOT in world 8
; Set Ypos of sprite to $B8 (floor)
BNE CODE_03C6CD         ; $03:C6C5: D0 06
LDA #$70                ; $03:C6C7: A9 70

; Otherwise, set Ypos to $70 (inside cage)

CODE_03C6C9:
STA $0238,x             ; $03:C6C9: 9D 38 02

; Return
RTS                     ; $03:C6CC: 60

CODE_03C6CD:
LDA #$B8                ; $03:C6CD: A9 B8

; Set Ypos of sprite to $B8
BRA CODE_03C6C9         ; $03:C6CF: 80 F8

; Sprite initial xspeed. Format: Normal difficulty, hard difficulty

DATA_03C6D1:
db $F8,$F4

CODE_03C6D3:
LDY #$01                ; $03:C6D3: A0 01
LDA $076A               ; $03:C6D5: AD 6A 07
BNE CODE_03C6DB         ; $03:C6D8: D0 01
DEY                     ; $03:C6DA: 88

CODE_03C6DB:
LDA DATA_03C6D1,y       ; $03:C6DB: B9 D1 C6

; INIT routine for most walking sprites
; Set sprite initial Xspeed depending on difficulty

CODE_03C6DE:
STA $5E,x               ; $03:C6DE: 95 5E
JMP CODE_03C71D         ; $03:C6E0: 4C 1D C7

CODE_03C6E3:
JSR CODE_03C6D3         ; $03:C6E3: 20 D3 C6
...
CODE_03C700:
LDA #$00                ; $03:C700: A9 00
JMP CODE_03C6DE         ; $03:C702: 4C DE C6

CODE_03C705:
STZ $5E,x               ; $03:C705: 74 5E

CODE_03C707:
LDA #$09                ; $03:C707: A9 09
BNE CODE_03C71F         ; $03:C709: D0 14

CODE_03C70B:
LDY #$30                ; $03:C70B: A0 30
...
CODE_03C71D:
LDA #$03                ; $03:C71D: A9 03

CODE_03C71F:
STA $0490,x             ; $03:C71F: 9D 90 04
LDA #$02                ; $03:C722: A9 02

; Set sprite direction to left
STA $47,x               ; $03:C724: 95 47

CODE_03C726:
LDA #$00                ; $03:C726: A9 00

; Set sprite vertical speed to 0
STA $A1,x               ; $03:C728: 95 A1

; Set ??? to 0? Related to sprite falling and other movement-related speed.
STA $043D,x             ; $03:C72A: 9D 3D 04
RTS                     ; $03:C72D: 60

CODE_03C72E:
LDA #$02                ; $03:C72E: A9 02

; INIT routine of bullet bill
; Set direction of sprite to left
STA $47,x               ; $03:C730: 95 47
LDA #$09                ; $03:C732: A9 09
STA $0490,x             ; $03:C734: 9D 90 04
RTS                     ; $03:C737: 60

CODE_03C738:
JSR CODE_03C707         ; $03:C738: 20 07 C7
...
CODE_03C749:
LDA $06CB               ; $03:C749: AD CB 06
BNE CODE_03C759         ; $03:C74C: D0 0B

CODE_03C74E:
LDA #$00                ; $03:C74E: A9 00
...
CODE_03C759:
JMP CODE_03CDE2         ; $03:C759: 4C E2 CD
...
CODE_03C797:
STZ $29,x               ; $03:C797: 74 29
LDA #$11                ; $03:C799: A9 11

; Sprite to generate: Lakitu
STA $1C,x               ; $03:C79B: 95 1C
JSR CODE_03C74E         ; $03:C79D: 20 4E C7
LDA #$20                ; $03:C7A0: A9 20
JSR CODE_03C9E8         ; $03:C7A2: 20 E8 C9

CODE_03C7A5:
LDX $9E                 ; $03:C7A5: A6 9E

CODE_03C7A7:
RTS                     ; $03:C7A7: 60

CODE_03C7A8:
LDA $0237               ; $03:C7A8: AD 37 02
...
CODE_03C7D4:
LDA DATA_03C75C,y       ; $03:C7D4: B9 5C C7
...
CODE_03C7F9:
JSR CODE_03C707         ; $03:C7F9: 20 07 C7
...
; Castle firebar rotation speed data. Format:
...
; When bit 4 is set, the firebar will rotate C-Clockwise
...
CODE_03C81E:
JSR CODE_03C946         ; $03:C81E: 20 46 C9

; No sprite horizontal speed.

CODE_03C821:
STZ $5E,x               ; $03:C821: 74 5E
LDA $1C,x               ; $03:C823: B5 1C
SEC                     ; $03:C825: 38
SBC #$1B                ; $03:C826: E9 1B

; Load sprite number
; Subtract $1B from it so we get index to 5 fire types
; Prepare for indexing
TAY                     ; $03:C828: A8
LDA DATA_03C814,y       ; $03:C829: B9 14 C8

; Get firebar rotation speed
; Store in spinning speed of firebars
STA $0388,x             ; $03:C82C: 9D 88 03
LDA DATA_03C819,y       ; $03:C82F: B9 19 C8

; Get firebar rotation direction...
STA $0203,x             ; $03:C832: 9D 03 02
LDA $0238,x             ; $03:C835: BD 38 02
CLC                     ; $03:C838: 18
ADC #$04                ; $03:C839: 69 04

; Sprite Ypos is 4px higher
; So it will become centered in 1 16x16 block
STA $0238,x             ; $03:C83B: 9D 38 02
LDA $021A,x             ; $03:C83E: BD 1A 02
CLC                     ; $03:C841: 18
ADC #$04                ; $03:C842: 69 04

; Sprite Xpos is 4px more to the right
; So it will become centered in 1 16x16 block
STA $021A,x             ; $03:C844: 9D 1A 02
LDA $79,x               ; $03:C847: B5 79
ADC #$00                ; $03:C849: 69 00

; Handle page boundary stuff or whatever.
...
CODE_03C918:
JSR CODE_03C946         ; $03:C918: 20 46 C9

; Index of Bowser's sprite
STX $0368               ; $03:C91B: 8E 68 03
...
; Time until next Bowser's fireball
STA $079C               ; $03:C92C: 8D 9C 07
...
; Bowser's health vs player's fireballs
STA $0283               ; $03:C93E: 8D 83 02
LSR A                   ; $03:C941: 4A
...
; Store the index of the free sprite slot
STY $06CF               ; $03:C94E: 8C CF 06

; Sprite index -> to accumulator
TXA                     ; $03:C951: 8A

; OR 0x80
ORA #$80                ; $03:C952: 09 80

; Store into sprite on-screen flag
STA $0010,y             ; $03:C954: 99 10 00
LDA $79,x               ; $03:C957: B5 79

; Sprite xpos high byte
STA $0079,y             ; $03:C959: 99 79 00
LDA $021A,x             ; $03:C95C: BD 1A 02

; Sprite xpos low byte
STA $021A,y             ; $03:C95F: 99 1A 02
LDA #$01                ; $03:C962: A9 01

; Sprite off-screen flag?
STA $10,x               ; $03:C964: 95 10

; "position of sprites"
STA $00BC,y             ; $03:C966: 99 BC 00
LDA $0238,x             ; $03:C969: BD 38 02

; Sprite Ypos
STA $0238,y             ; $03:C96C: 99 38 02

CODE_03C96F:
RTS                     ; $03:C96F: 60

; Bowser's fireball possible Y positions

DATA_03C970:
db $90,$80,$70,$90

; Fireball Y Speed (both directions, but fireball goes 1-way only?)

DATA_03C974:
db $FF,$01

CODE_03C976:
LDA $079B               ; $03:C976: AD 9B 07

; Bowser Fire generator. Timer to spawn next sprite (generators)
; If $079B != 00, return
BNE CODE_03C96F         ; $03:C979: D0 F4

; Sprite vertical speed = 00
STA $043D,x             ; $03:C97B: 9D 3D 04
LDA #$06                ; $03:C97E: A9 06
STA $014B               ; $03:C980: 8D 4B 01
LDA #$18                ; $03:C983: A9 18
STA $0F4C               ; $03:C985: 8D 4C 0F
STX $0F4D               ; $03:C988: 8E 4D 0F
LDY $0368               ; $03:C98B: AC 68 03
LDA $001C,y             ; $03:C98E: B9 1C 00
CMP #$2D                ; $03:C991: C9 2D

; Index of bowser sprite
;
; If there's a bowser sprite present, branch
BEQ CODE_03C9E4         ; $03:C993: F0 4F
JSR CODE_03D6F9         ; $03:C995: 20 F9 D6
CLC                     ; $03:C998: 18

; Get timer delay of next fireball spawn
; make them spawn #$20 frames later by default.
ADC #$20                ; $03:C999: 69 20
LDY $06CC               ; $03:C99B: AC CC 06
BEQ CODE_03C9A3         ; $03:C99E: F0 03
SEC                     ; $03:C9A0: 38
SBC #$10                ; $03:C9A1: E9 10

; If hard flag (levels after 5-3)
;
; make fireballs spawn #$10 frames later instead of #$20
;
; Timer to spawn next sprite

CODE_03C9A3:
STA $079B               ; $03:C9A3: 8D 9B 07
LDA $07B7,x             ; $03:C9A6: BD B7 07
AND #$03                ; $03:C9A9: 29 03
STA $041D,x             ; $03:C9AB: 9D 1D 04
TAY                     ; $03:C9AE: A8
LDA DATA_03C970,y       ; $03:C9AF: B9 70 C9

; Get random value and make the range $00-$03
;
;
; Store bowser fireball Y position and into $041D,x
...
; X-Position of fireball = right border of screen + #$20
STA $79,x               ; $03:C9C3: 95 79

; Zero out how long bowser holds his breath for firing
STZ $0F4C               ; $03:C9C5: 9C 4C 0F
STZ $0F4D               ; $03:C9C8: 9C 4D 0F
STZ $014B               ; $03:C9CB: 9C 4B 01
LDA #$17                ; $03:C9CE: A9 17

; Play fire sound effect
; (note: NOT fire breath from bowser)
STA $1603               ; $03:C9D0: 8D 03 16
LDA #$08                ; $03:C9D3: A9 08
STA $0490,x             ; $03:C9D5: 9D 90 04
LDA #$01                ; $03:C9D8: A9 01
STA $BC,x               ; $03:C9DA: 95 BC

; Set sprite on-screen flag
STA $10,x               ; $03:C9DC: 95 10
...
; Generated sprite index to $0F4E

CODE_03C9E4:
STX $0F4E               ; $03:C9E4: 8E 4E 0F
RTS                     ; $03:C9E7: 60

CODE_03C9E8:
STA $0238,x             ; $03:C9E8: 9D 38 02
...
CODE_03CA29:
LDA DATA_03C974,y       ; $03:CA29: B9 74 C9
STA $043D,x             ; $03:CA2C: 9D 3D 04
STZ $06CB               ; $03:CA2F: 9C CB 06

CODE_03CA32:
LDA #$08                ; $03:CA32: A9 08
...
LDA $07B7,x             ; $03:CAC0: BD B7 07
CMP #$AA                ; $03:CAC3: C9 AA
BCC CODE_03CAC8         ; $03:CAC5: 90 01
INY                     ; $03:CAC7: C8

CODE_03CAC8:
LDA $075F               ; $03:CAC8: AD 5F 07
...
; Sprite to generate: Red or Green swimming cheep-cheep depending on above pseudocode

CODE_03CAD7:
STA $1C,x               ; $03:CAD7: 95 1C
...
CODE_03CB22:
LDA #$09                ; $03:CB22: A9 09

; Quake sound?
STA $1603               ; $03:CB24: 8D 03 16
LDA #$08                ; $03:CB27: A9 08
BNE CODE_03CAD7         ; $03:CB29: D0 AC

CODE_03CB2B:
LDY #$00                ; $03:CB2B: A0 00
SEC                     ; $03:CB2D: 38
...
CODE_03CB5D:
STY $06D3               ; $03:CB5D: 8C D3 06

CODE_03CB60:
LDX #$FF                ; $03:CB60: A2 FF
...
CODE_03CB96:
JMP CODE_03C5FF         ; $03:CB96: 4C FF C5

CODE_03CB99:
LDA #$01                ; $03:CB99: A9 01
...
CODE_03CBB5:
LDA $1C,x               ; $03:CBB5: B5 1C

; Generator to create
; Store in this RAM address
STA $06CB               ; $03:CBB7: 8D CB 06
SEC                     ; $03:CBBA: 38

; Subtract sprite by $12
...
LDA PNTR_03CBCC+1,y     ; $03:CBC4: B9 CD CB
STA $05                 ; $03:CBC7: 85 05

; Jump to pointer tables below.
...
CODE_03CBD9:
LDY #$09                ; $03:CBD9: A0 09

CODE_03CBDB:
LDA $001C,y             ; $03:CBDB: B9 1C 00
CMP #$11                ; $03:CBDE: C9 11
BNE CODE_03CBE7         ; $03:CBE0: D0 05
LDA #$01                ; $03:CBE2: A9 01
STA $0029,y             ; $03:CBE4: 99 29 00
...
STA $10,x               ; $03:CBEF: 95 10

; Disable lakitu generator sprite
; Find a lakitu, any of them
;
;
; Tell them to go off-screen to the left without doing
; anything.
;
;
;
;
; Disable on-screen flag?
; Return
RTS                     ; $03:CBF1: 60

CODE_03CBF2:
LDA #$02                ; $03:CBF2: A9 02
STA $47,x               ; $03:CBF4: 95 47
LDA #$F8                ; $03:CBF6: A9 F8
STA $5E,x               ; $03:CBF8: 95 5E

CODE_03CBFA:
LDA #$03                ; $03:CBFA: A9 03

CODE_03CBFC:
STA $0490,x             ; $03:CBFC: 9D 90 04
RTS                     ; $03:CBFF: 60

CODE_03CC00:
DEC $0238,x             ; $03:CC00: DE 38 02
...
CODE_03CC4D:
JSR CODE_03C726         ; $03:CC4D: 20 26 C7

CODE_03CC50:
LDA #$05                ; $03:CC50: A9 05
...
CODE_03CC5F:
STA $0490,x             ; $03:CC5F: 9D 90 04
RTS                     ; $03:CC62: 60

CODE_03CC63:
JSR CODE_03CC6F         ; $03:CC63: 20 6F CC
JMP CODE_03CC6C         ; $03:CC66: 4C 6C CC

CODE_03CC69:
JSR CODE_03CC7B         ; $03:CC69: 20 7B CC

CODE_03CC6C:
JMP CODE_03CC50         ; $03:CC6C: 4C 50 CC

CODE_03CC6F:
LDA #$10                ; $03:CC6F: A9 10
...
CODE_03CCA8:
LDX $9E                 ; $03:CCA8: A6 9E

; load 00 into A
LDA #$00                ; $03:CCAA: A9 00

; Load current sprite number
LDY $1C,x               ; $03:CCAC: B4 1C
CPY #$15                ; $03:CCAE: C0 15

; If it is smaller than $15 ($00-$14)
; Skip the next 2 lines.
BCC CODE_03CCB5         ; $03:CCB0: 90 03
TYA                     ; $03:CCB2: 98
SBC #$14                ; $03:CCB3: E9 14

; Get index to next table
...
CODE_03CD09:
JSR CODE_03FDCB         ; $03:CD09: 20 CB FD
JSR CODE_03FD39         ; $03:CD0C: 20 39 FD
JMP CODE_03EEEF         ; $03:CD0F: 4C EF EE

CODE_03CD12:
LDA #$20                ; $03:CD12: A9 20

; Handle sprites $00-$14
; Set sprite onscreen flag
STA $0257,x             ; $03:CD14: 9D 57 02
...
BNE CODE_03CD34         ; $03:CD2F: D0 03

; Update sprite movement when game is not locked
JSR CODE_03CD37         ; $03:CD31: 20 37 CD

CODE_03CD34:
JMP CODE_03DC03         ; $03:CD34: 4C 03 DC

; Load sprite number

CODE_03CD37:
LDA $1C,x               ; $03:CD37: B5 1C
...
LDA PNTR_03CD48+1,y     ; $03:CD40: B9 49 CD
STA $05                 ; $03:CD43: 85 05

; Jump to the sprite routines
...
CODE_03CD73:
JSR CODE_03D70B         ; $03:CD73: 20 0B D7
...
CODE_03CD85:
JSR CODE_03D1E1         ; $03:CD85: 20 E1 D1
JMP CODE_03DC03         ; $03:CD88: 4C 03 DC

CODE_03CD8B:
JSR CODE_03FDCB         ; $03:CD8B: 20 CB FD
...
CODE_03CDA3:
JSR CODE_03FDCB         ; $03:CDA3: 20 CB FD
...
; Clear enemy interval timer

CODE_03CDE2:
STZ $07A2,x             ; $03:CDE2: 9E A2 07

CODE_03CDE5:
CPX #$09                ; $03:CDE5: E0 09

; Check if sprite index is that of a [?]-block item
; If not, skip.
BNE CODE_03CE00         ; $03:CDE7: D0 17
...
; No cloud terrain
STZ $0743               ; $03:CDFD: 9C 43 07

; Clear sprite flag

CODE_03CE00:
STZ $10,x               ; $03:CE00: 74 10

; Clear sprite number
STZ $1C,x               ; $03:CE02: 74 1C

; Clear sprite state
STZ $29,x               ; $03:CE04: 74 29

; Clear shell chain counter
STZ $0110,x             ; $03:CE06: 9E 10 01
...
CODE_03CE1A:
LDA $07A2,x             ; $03:CE1A: BD A2 07
BNE CODE_03CE35         ; $03:CE1D: D0 16

; If jump timer not expired
;
; Init podoboo again
JSR CODE_03C6A4         ; $03:CE1F: 20 A4 C6
LDA $07B8,x             ; $03:CE22: BD B8 07
ORA #$80                ; $03:CE25: 09 80

; Set movement force according to LSFR random number generator output
STA $043D,x             ; $03:CE27: 9D 3D 04
AND #$0F                ; $03:CE2A: 29 0F
ORA #$06                ; $03:CE2C: 09 06

; Set interval timer again
STA $07A2,x             ; $03:CE2E: 9D A2 07
LDA #$F9                ; $03:CE31: A9 F9

; Set jumping speed of podoboo
STA $A1,x               ; $03:CE33: 95 A1

CODE_03CE35:
LDA $00A1,x             ; $03:CE35: BD A1 00

; If speed is upwards, skip
BMI CODE_03CE53         ; $03:CE38: 30 19
LDA $0238,x             ; $03:CE3A: BD 38 02
CMP #$C0                ; $03:CE3D: C9 C0

; if Y position is less than $C0, skip
...
; Reset some timer
STZ $0B09,x             ; $03:CE4B: 9E 09 0B
LDA #$27                ; $03:CE4E: A9 27
...
CODE_03CE53:
INC $0B09,x             ; $03:CE53: FE 09 0B

; Leave behind fire and lava particles
JSL CODE_05D614         ; $03:CE56: 22 14 D6 05
...
CODE_03CE67:
LDA $29,x               ; $03:CE67: B5 29
AND #$20                ; $03:CE69: 29 20

; MAIN routine of hammer bros
; If not dying, branch
BEQ CODE_03CE70         ; $03:CE6B: F0 03

; Else, jump if dying.
JMP CODE_03CF76         ; $03:CE6D: 4C 76 CF

CODE_03CE70:
LDA $020F,x             ; $03:CE70: BD 0F 02
BEQ CODE_03CEA3         ; $03:CE73: F0 2E

; Hammer bros jump timer
; If jump timer reached 00, make hammer bros jump
; else, decrease timer and continue.
DEC $020F,x             ; $03:CE75: DE 0F 02
LDA $03D1               ; $03:CE78: AD D1 03
AND #$0C                ; $03:CE7B: 29 0C

; If $04,$08 or $0C
; Branch
BNE CODE_03CEE9         ; $03:CE7D: D0 6A
LDA $03A2,x             ; $03:CE7F: BD A2 03

; If hammer throw timer ($03A2) is not 00
; Branch
BNE CODE_03CE9B         ; $03:CE82: D0 17
LDY $06CC               ; $03:CE84: AC CC 06
LDA DATA_03CE5D,y       ; $03:CE87: B9 5D CE

; Set hammer throwing timer according to hard world (post 5-3) flag
...
CODE_03CEDB:
LDA DATA_03CEA1,y       ; $03:CEDB: B9 A1 CE
STA $0792,x             ; $03:CEDE: 9D 92 07
LDA $07B8,x             ; $03:CEE1: BD B8 07
ORA #$C0                ; $03:CEE4: 09 C0
STA $020F,x             ; $03:CEE6: 9D 0F 02

; Hammer bros forwards walking speed

CODE_03CEE9:
LDY #$FC                ; $03:CEE9: A0 FC
LDA $09                 ; $03:CEEB: A5 09
AND #$40                ; $03:CEED: 29 40
BNE CODE_03CEF3         ; $03:CEEF: D0 02
LDY #$04                ; $03:CEF1: A0 04

; Hammer bros backwards walking speed

CODE_03CEF3:
STY $5E,x               ; $03:CEF3: 94 5E

; Set sprite direction to right
LDY #$01                ; $03:CEF5: A0 01
JSR CODE_03E8BC         ; $03:CEF7: 20 BC E8

; SUB_HORZ_POS
; Branch if player is to the right of the hammer bros.
BMI CODE_03CF06         ; $03:CEFA: 30 0A

; Set sprite direction to left
INY                     ; $03:CEFC: C8
LDA $07A2,x             ; $03:CEFD: BD A2 07

; Check if Hammer Bros. has waited long enough
BNE CODE_03CF06         ; $03:CF00: D0 04
LDA #$F8                ; $03:CF02: A9 F8

; Hammer bros forwards walking speed after he has waited long enough.
STA $5E,x               ; $03:CF04: 95 5E

; Set sprite direction

CODE_03CF06:
STY $47,x               ; $03:CF06: 94 47

CODE_03CF08:
LDY #$00                ; $03:CF08: A0 00
...
CODE_03CF29:
JSR CODE_03C113         ; $03:CF29: 20 13 C1
...
CODE_03CF40:
JMP CODE_03C0AD         ; $03:CF40: 4C AD C0

CODE_03CF43:
LDY #$01                ; $03:CF43: A0 01

CODE_03CF45:
LDA $5E,x               ; $03:CF45: B5 5E
PHA                     ; $03:CF47: 48
...
; "More difficult quest" flag
LDA $076A               ; $03:CF69: AD 6A 07
BEQ CODE_03CF70         ; $03:CF6C: F0 02
INY                     ; $03:CF6E: C8
INY                     ; $03:CF6F: C8

CODE_03CF70:
LDA DATA_03CE63,y       ; $03:CF70: B9 63 CE
STA $5E,x               ; $03:CF73: 95 5E
RTS                     ; $03:CF75: 60

CODE_03CF76:
JSR CODE_03C113         ; $03:CF76: 20 13 C1
JMP CODE_03C0AD         ; $03:CF79: 4C AD C0

CODE_03CF7C:
CMP #$0E                ; $03:CF7C: C9 0E
...
; Erase sprite
JSR CODE_03CDE2         ; $03:CF86: 20 E2 CD

CODE_03CF89:
RTS                     ; $03:CF89: 60

CODE_03CF8A:
JSR CODE_03C140         ; $03:CF8A: 20 40 C1
JMP CODE_03C0AD         ; $03:CF8D: 4C AD C0

CODE_03CF90:
LDA $A1,x               ; $03:CF90: B5 A1
...
CODE_03CFAC:
LDA $0238,x             ; $03:CFAC: BD 38 02
CMP $5E,x               ; $03:CFAF: D5 5E
BCC CODE_03CFB6         ; $03:CFB1: 90 03
JMP CODE_03C123         ; $03:CFB3: 4C 23 C1

CODE_03CFB6:
JMP CODE_03C11F         ; $03:CFB6: 4C 1F C1

CODE_03CFB9:
JSR CODE_03CFDB         ; $03:CFB9: 20 DB CF
...
CODE_03CFDB:
LDA #$13                ; $03:CFDB: A9 13

CODE_03CFDD:
STA $01                 ; $03:CFDD: 85 01
...
CODE_03D038:
LDY #$02                ; $03:D038: A0 02
JSR CODE_03E8BC         ; $03:D03A: 20 BC E8
BPL CODE_03D040         ; $03:D03D: 10 01
DEY                     ; $03:D03F: 88

CODE_03D040:
STY $47,x               ; $03:D040: 94 47

CODE_03D042:
JSR CODE_03D07B         ; $03:D042: 20 7B D0
...
CODE_03D068:
LDA $021A,x             ; $03:D068: BD 1A 02
SEC                     ; $03:D06B: 38
SBC $5E,x               ; $03:D06C: F5 5E
STA $021A,x             ; $03:D06E: 9D 1A 02
LDA $79,x               ; $03:D071: B5 79
SBC #$00                ; $03:D073: E9 00

; Subtract sprite X position with sprite X speed
STA $79,x               ; $03:D075: 95 79
RTS                     ; $03:D077: 60

CODE_03D078:
JMP CODE_03C13A         ; $03:D078: 4C 3A C1

CODE_03D07B:
LDA $A1,x               ; $03:D07B: B5 A1
...
CODE_03D0B8:
LDA $07A2,x             ; $03:D0B8: BD A2 07
BEQ CODE_03D0C6         ; $03:D0BB: F0 09

CODE_03D0BD:
LDA $09                 ; $03:D0BD: A5 09
LSR A                   ; $03:D0BF: 4A
...
CODE_03D1E1:
JSR CODE_03FDCB         ; $03:D1E1: 20 CB FD
...
CODE_03D23B:
STY $ED                 ; $03:D23B: 84 ED
STZ $00                 ; $03:D23D: 64 00

CODE_03D23F:
LDA $EF                 ; $03:D23F: A5 EF
...
CODE_03D2B6:
LDA #$00                ; $03:D2B6: A9 00
STA $0D00,y             ; $03:D2B8: 99 00 0D

CODE_03D2BB:
JSR CODE_03F48B         ; $03:D2BB: 20 8B F4
TYA                     ; $03:D2BE: 98
PHA                     ; $03:D2BF: 48
...
CODE_03D30C:
CMP #$08                ; $03:D30C: C9 08
BCC CODE_03D324         ; $03:D30E: 90 14

CODE_03D310:
LDA $05                 ; $03:D310: A5 05
...
CODE_03D3A5:
JSR CODE_03C0AD         ; $03:D3A5: 20 AD C0
...
CODE_03D3D4:
LDA DATA_03D392,y       ; $03:D3D4: B9 92 D3
...
CODE_03D3DE:
LDA $29,x               ; $03:D3DE: B5 29
AND #$20                ; $03:D3E0: 29 20

; If not dying, branch
BEQ CODE_03D3E7         ; $03:D3E2: F0 03

; Otherwise, take jump when dying.
JMP CODE_03C113         ; $03:D3E4: 4C 13 C1

CODE_03D3E7:
LDA $29,x               ; $03:D3E7: B5 29
...
CODE_03D3F4:
LDA #$12                ; $03:D3F4: A9 12
STA $06CB               ; $03:D3F6: 8D CB 06
LDY #$02                ; $03:D3F9: A0 02

CODE_03D3FB:
LDA DATA_03D3DB,y       ; $03:D3FB: B9 DB D3
...
CODE_03D41B:
STY $47,x               ; $03:D41B: 94 47
JMP CODE_03C0AD         ; $03:D41D: 4C AD C0

CODE_03D420:
LDY #$00                ; $03:D420: A0 00
...
CODE_03D47F:
LDA $A1,x               ; $03:D47F: B5 A1
BNE CODE_03D485         ; $03:D481: D0 02
LDY #$00                ; $03:D483: A0 00

CODE_03D485:
LDA $0001,y             ; $03:D485: B9 01 00
...
CODE_03D4D0:
LDX $0368               ; $03:D4D0: AE 68 03
LDA $1C,x               ; $03:D4D3: B5 1C
CMP #$2D                ; $03:D4D5: C9 2D

; Check if Bowser still exists
; (He could've been killed by fireballs)
; If he doesn't exist, skip bridge destruction
BNE CODE_03D4EA         ; $03:D4D7: D0 11

; Store bowser's sprite index into current sprite index
STX $9E                 ; $03:D4D9: 86 9E
LDA $29,x               ; $03:D4DB: B5 29

; If Bowser is in a normal state, destroy bridge
BEQ CODE_03D503         ; $03:D4DD: F0 24
AND #$40                ; $03:D4DF: 29 40

; If Bowser isn't in a defeated state (fall down bridge), remove bridge
BEQ CODE_03D4EA         ; $03:D4E1: F0 07
LDA $0238,x             ; $03:D4E3: BD 38 02
CMP #$E0                ; $03:D4E6: C9 E0

; If Bowser sprite is not low enough yet, branch
BCC CODE_03D4FD         ; $03:D4E8: 90 13

CODE_03D4EA:
LDA $0095               ; $03:D4EA: AD 95 00

; Flag to not play music?
BNE CODE_03D4F7         ; $03:D4ED: D0 08
LDA #$0B                ; $03:D4EF: A9 0B

; Toad/Princess saved fanfare music
...
CODE_03D4F7:
INC $0772               ; $03:D4F7: EE 72 07
JMP CODE_03D56B         ; $03:D4FA: 4C 6B D5

CODE_03D4FD:
JSR CODE_03C13A         ; $03:D4FD: 20 3A C1
JMP CODE_03D679         ; $03:D500: 4C 79 D6

CODE_03D503:
DEC $0364               ; $03:D503: CE 64 03
BNE CODE_03D557         ; $03:D506: D0 4F
LDA #$04                ; $03:D508: A9 04

; Amount of delay until the next bridge tile collapses
...
; Bowser bridge crumbling sound
STA $1603               ; $03:D534: 8D 03 16
...
CODE_03D557:
JMP CODE_03D679         ; $03:D557: 4C 79 D6

DATA_03D55A:
db $21,$41,$11,$31

CODE_03D55E:
LDA $29,x               ; $03:D55E: B5 29
...
BCC CODE_03D4FD         ; $03:D569: 90 92

CODE_03D56B:
LDX #$08                ; $03:D56B: A2 08

CODE_03D56D:
JSR CODE_03CDE2         ; $03:D56D: 20 E2 CD
DEX                     ; $03:D570: CA

; If Bowser reaches a certain Y position
;
; Erase all current sprites
BPL CODE_03D56D         ; $03:D571: 10 FA

; Routine always returns #$00, so zero out generators
STA $06CB               ; $03:D573: 8D CB 06

; Restore current sprite index
LDX $9E                 ; $03:D576: A6 9E
RTS                     ; $03:D578: 60

; Zero out generators

CODE_03D579:
STZ $06CB               ; $03:D579: 9C CB 06
LDA $0747               ; $03:D57C: AD 47 07
BEQ CODE_03D584         ; $03:D57F: F0 03
JMP CODE_03D636         ; $03:D581: 4C 36 D6

CODE_03D584:
LDA $0363               ; $03:D584: AD 63 03
BPL CODE_03D58C         ; $03:D587: 10 03
JMP CODE_03D60B         ; $03:D589: 4C 0B D6

CODE_03D58C:
DEC $0364               ; $03:D58C: CE 64 03
...
CODE_03D636:
JMP CODE_03D647         ; $03:D636: 4C 47 D6

CODE_03D639:
CMP #$01                ; $03:D639: C9 01
BNE CODE_03D647         ; $03:D63B: D0 0A
DEC $0238,x             ; $03:D63D: DE 38 02
JSR CODE_03C726         ; $03:D640: 20 26 C7
LDA #$FE                ; $03:D643: A9 FE

; Bowser jumps
STA $A1,x               ; $03:D645: 95 A1

CODE_03D647:
LDA $075F               ; $03:D647: AD 5F 07
CMP #$07                ; $03:D64A: C9 07

; Check if world 8
; Branch, in order for real Bowser to throw hammers and fireballs simultaneously
BEQ CODE_03D652         ; $03:D64C: F0 04
CMP #$05                ; $03:D64E: C9 05

; Check if world 6 or 7
; Branch, in order for Bowser to throw hammers only
BCS CODE_03D679         ; $03:D650: B0 27

CODE_03D652:
LDA $079C               ; $03:D652: AD 9C 07
BNE CODE_03D679         ; $03:D655: D0 22
LDA #$20                ; $03:D657: A9 20
STA $079C               ; $03:D659: 8D 9C 07
LDA $0363               ; $03:D65C: AD 63 03

; Bit 7 stops horizontal movement?
EOR #$80                ; $03:D65F: 49 80
STA $0363               ; $03:D661: 8D 63 03
BMI CODE_03D647         ; $03:D664: 30 E1

; Get timer delay of next fireball spawn
JSR CODE_03D6F9         ; $03:D666: 20 F9 D6
LDY $06CC               ; $03:D669: AC CC 06
BEQ CODE_03D671         ; $03:D66C: F0 03
SEC                     ; $03:D66E: 38
SBC #$10                ; $03:D66F: E9 10

; If hard mode (after 5-3)
;
; Timer -= #$10;
;
; store into time until next fireball gets generated

CODE_03D671:
STA $079C               ; $03:D671: 8D 9C 07
LDA #$15                ; $03:D674: A9 15

; Activate bowser fire generator
STA $06CB               ; $03:D676: 8D CB 06

CODE_03D679:
JSR CODE_03D6D4         ; $03:D679: 20 D4 D6
...
CMP #$07                ; $03:D6B2: C9 07

; Play real bowser battle music at world 8
BNE CODE_03D6BA         ; $03:D6B4: D0 04

; "real" bowser battle music
LDA #$05                ; $03:D6B6: A9 05
BRA CODE_03D6BC         ; $03:D6B8: 80 02

CODE_03D6BA:
LDA #$04                ; $03:D6BA: A9 04

; "fake" bowser battle music

CODE_03D6BC:
STA $1602               ; $03:D6BC: 8D 02 16
STA $97                 ; $03:D6BF: 85 97

CODE_03D6C1:
LDA #$2D                ; $03:D6C1: A9 2D

; Sprite to generate: Bowser
STA $1C,x               ; $03:D6C3: 95 1C
...
; How fast the bowser fire generates.

DATA_03D6F1:
db $BF,$40,$BF,$BF,$BF,$40,$40,$BF

CODE_03D6F9:
LDY $0367               ; $03:D6F9: AC 67 03
INC $0367               ; $03:D6FC: EE 67 03
LDA $0367               ; $03:D6FF: AD 67 03
AND #$07                ; $03:D702: 29 07
STA $0367               ; $03:D704: 8D 67 03
LDA DATA_03D6F1,y       ; $03:D707: B9 F1 D6

; Get timer delay of next fireball spawn
; Returns: Timer delay in A

CODE_03D70A:
RTS                     ; $03:D70A: 60

CODE_03D70B:
CPX $0F4D               ; $03:D70B: EC 4D 0F
...
; Compare with possible bowser fireball Y positions
CMP DATA_03C970,y       ; $03:D749: D9 70 C9
...
CODE_03D755:
JSR CODE_03FD39         ; $03:D755: 20 39 FD
...
; Firework sound
STA $1600               ; $03:D773: 8D 00 16
BRA CODE_03D77C         ; $03:D776: 80 04

CODE_03D778:
CMP #$04                ; $03:D778: C9 04
BCS CODE_03D794         ; $03:D77A: B0 18

CODE_03D77C:
JSR CODE_03FD39         ; $03:D77C: 20 39 FD
...
CODE_03D7AF:
STZ $06CB               ; $03:D7AF: 9C CB 06
LDA $0746               ; $03:D7B2: AD 46 07
CMP #$05                ; $03:D7B5: C9 05

; Flagpole task control
BCS CODE_03D7F3         ; $03:D7B7: B0 3A

; ExecutePtrShort
JSR CODE_039B08         ; $03:D7B9: 20 08 9B
...
; Timer to score conversion sound
STA $1603               ; $03:D7ED: 8D 03 16

CODE_03D7F0:
INC $0746               ; $03:D7F0: EE 46 07

CODE_03D7F3:
RTS                     ; $03:D7F3: 60

CODE_03D7F4:
LDA #$4C                ; $03:D7F4: A9 4C

; SMAS menu 'zooming' sound (after pressing start in hall screen)
STA $1603               ; $03:D7F6: 8D 03 16
STA $0E1A               ; $03:D7F9: 8D 1A 0E
BRA CODE_03D7F0         ; $03:D7FC: 80 F2

CODE_03D7FE:
LDA $07E9               ; $03:D7FE: AD E9 07
ORA $07EA               ; $03:D801: 0D EA 07
ORA $07EB               ; $03:D804: 0D EB 07
BEQ CODE_03D7F4         ; $03:D807: F0 EB

CODE_03D809:
LDY #$23                ; $03:D809: A0 23
...
CODE_03D84C:
JSR CODE_03FD39         ; $03:D84C: 20 39 FD
...
CODE_03D877:
JSR CODE_03D84C         ; $03:D877: 20 4C D8
LDA #$06                ; $03:D87A: A9 06
STA $07A2,x             ; $03:D87C: 9D A2 07

CODE_03D87F:
INC $0746               ; $03:D87F: EE 46 07
RTS                     ; $03:D882: 60

CODE_03D883:
JSR CODE_03D84C         ; $03:D883: 20 4C D8
...
CODE_03D8EB:
LDA #$00                ; $03:D8EB: A9 00
STA $0257,x             ; $03:D8ED: 9D 57 02
RTS                     ; $03:D8F0: 60

CODE_03D8F1:
STA $07                 ; $03:D8F1: 85 07
...
; Lift (balance) movement code

CODE_03D914:
LDA $BC,x               ; $03:D914: B5 BC
CMP #$03                ; $03:D916: C9 03
BNE CODE_03D91D         ; $03:D918: D0 03

; Erase sprite
JMP CODE_03CDE2         ; $03:D91A: 4C E2 CD

CODE_03D91D:
LDA $29,x               ; $03:D91D: B5 29
...
CODE_03D943:
JMP CODE_03DB17         ; $03:D943: 4C 17 DB

CODE_03D946:
CMP $0238,y             ; $03:D946: D9 38 02
...
CODE_03D979:
CMP $9E                 ; $03:D979: C5 9E
BEQ CODE_03D989         ; $03:D97B: F0 0C

CODE_03D97D:
JSR CODE_03C167         ; $03:D97D: 20 67 C1
JMP CODE_03D98C         ; $03:D980: 4C 8C D9

CODE_03D983:
JSR CODE_03DB30         ; $03:D983: 20 30 DB
JMP CODE_03D98C         ; $03:D986: 4C 8C D9

CODE_03D989:
JSR CODE_03C163         ; $03:D989: 20 63 C1

CODE_03D98C:
LDY $29,x               ; $03:D98C: B4 29
PLA                     ; $03:D98E: 68
SEC                     ; $03:D98F: 38
...
CODE_03DA96:
LDA #$183F              ; $03:DA96: A9 3F 18
BRA CODE_03DA9E         ; $03:DA99: 80 03

CODE_03DA9B:
LDA #$18A3              ; $03:DA9B: A9 A3 18

CODE_03DA9E:
STA $1708,x             ; $03:DA9E: 9D 08 17
BRA CODE_03DAAC         ; $03:DAA1: 80 09

CODE_03DAA3:
LDA #$0024              ; $03:DAA3: A9 24 00
STA $1706,x             ; $03:DAA6: 9D 06 17
STA $1708,x             ; $03:DAA9: 9D 08 17

CODE_03DAAC:
LDA #$FFFF              ; $03:DAAC: A9 FF FF
...
CODE_03DABD:
SEP #$30                ; $03:DABD: E2 30

CODE_03DABF:
LDX $9E                 ; $03:DABF: A6 9E
RTS                     ; $03:DAC1: 60
...
CODE_03DB30:
JSR CODE_03C726         ; $03:DB30: 20 26 C7
...
CODE_03DB51:
LDX $9E                 ; $03:DB51: A6 9E
RTS                     ; $03:DB53: 60

CODE_03DB54:
LDA $A1,x               ; $03:DB54: B5 A1
...
CODE_03DB6F:
BRA CODE_03DB81         ; $03:DB6F: 80 10

CODE_03DB71:
LDA $0238,x             ; $03:DB71: BD 38 02
...
CODE_03DB7E:
JSR CODE_03C163         ; $03:DB7E: 20 63 C1

CODE_03DB81:
LDA $03A2,x             ; $03:DB81: BD A2 03
...
CODE_03DBAA:
SBC #$00                ; $03:DBAA: E9 00

CODE_03DBAC:
STA $78                 ; $03:DBAC: 85 78
...
CODE_03DBC1:
JSR CODE_03C0AD         ; $03:DBC1: 20 AD C0
...
CODE_03DBD3:
JSR CODE_03DBDE         ; $03:DBD3: 20 DE DB
JMP CODE_03DB81         ; $03:DBD6: 4C 81 DB

CODE_03DBD9:
JSR CODE_03DBDE         ; $03:DBD9: 20 DE DB
BRA CODE_03DBF6         ; $03:DBDC: 80 18

CODE_03DBDE:
LDA $0747               ; $03:DBDE: AD 47 07
...
CODE_03DC03:
LDA $1C,x               ; $03:DC03: B5 1C
CMP #$14                ; $03:DC05: C9 14

; If cheep-cheep, exit
BEQ CODE_03DC61         ; $03:DC07: F0 58

; Get horizontal coordinate of left side of screen
LDA $071C               ; $03:DC09: AD 1C 07
LDY $1C,x               ; $03:DC0C: B4 1C
CPY #$05                ; $03:DC0E: C0 05
BEQ CODE_03DC16         ; $03:DC10: F0 04
CPY #$0D                ; $03:DC12: C0 0D
BNE CODE_03DC18         ; $03:DC14: D0 02

; Check for hammer bros and piranha plant sprites
; Add 56 pixels if those.

CODE_03DC16:
ADC #$38                ; $03:DC16: 69 38

CODE_03DC18:
SBC #$48                ; $03:DC18: E9 48

; Subtract 72 pixels from horizontal coordinate earlier
; Regardless of the sprite
STA $01                 ; $03:DC1A: 85 01
LDA $071A               ; $03:DC1C: AD 1A 07
SBC #$00                ; $03:DC1F: E9 00
STA $00                 ; $03:DC21: 85 00
LDA $071D               ; $03:DC23: AD 1D 07
CLC                     ; $03:DC26: 18
ADC #$48                ; $03:DC27: 69 48
...
BEQ CODE_03DC61         ; $03:DC4C: F0 13
CPY #$0D                ; $03:DC4E: C0 0D
...
; Do not erase the following sprites when too far to the right:
; Hammer bros, piranha plant, flagpole, star flag (at end of level), springboard
BEQ CODE_03DC61         ; $03:DC5C: F0 03

; Erase sprite if necessary

CODE_03DC5E:
JSR CODE_03CDE2         ; $03:DC5E: 20 E2 CD
...
CODE_03DCCA:
LDX $9E                 ; $03:DCCA: A6 9E
RTS                     ; $03:DCCC: 60

DATA_03DCCD:
db $06,$00,$02,$12,$11,$07,$05,$2D

; Sprite to generate when you kill Bowser with fireballs.
; In this order: Goomba, Green Koopa, Red Koopa, Spiny, Lakitu, Blooper, Hammer Bros., Bowser

CODE_03DCD5:
JSR CODE_03FD39         ; $03:DCD5: 20 39 FD
LDA #$03                ; $03:DCD8: A9 03

; Shell kick sound
STA $1600               ; $03:DCDA: 8D 00 16
...
; Sprite to generate: Bowser killed sprite depending on the world
STA $1C,x               ; $03:DD3B: 95 1C
...
CODE_03DD8F:
JSR CODE_03E789         ; $03:DD8F: 20 89 E7
...
CODE_03DDFA:
JSR CODE_03CDE5         ; $03:DDFA: 20 E5 CD
LDA #$06                ; $03:DDFD: A9 06
JSR CODE_03E04F         ; $03:DDFF: 20 4F E0
LDA #$0A                ; $03:DE02: A9 0A

; You got powerup sound
STA $1600               ; $03:DE04: 8D 00 16
LDA $020C               ; $03:DE07: AD 0C 02
CMP #$02                ; $03:DE0A: C9 02
BCC CODE_03DE1D         ; $03:DE0C: 90 0F
CMP #$03                ; $03:DE0E: C9 03

; If you obtained a mushroom or fireflower
; skip this code (to another code)
;
; If you got a 1-up
; skip this code (to another code)
BEQ CODE_03DE37         ; $03:DE10: F0 25
LDA #$23                ; $03:DE12: A9 23
STA $07AF               ; $03:DE14: 8D AF 07
LDA #$0D                ; $03:DE17: A9 0D

; Otherwise enable starman stuff.
; You got the star powerup music.
; Set the timer of the starman powerup.
...
CODE_03DE1D:
LDA $0756               ; $03:DE1D: AD 56 07

; If you got a mushroom go branch
BEQ CODE_03DE42         ; $03:DE20: F0 20

; If it is NOT fireflower, return
CMP #$01                ; $03:DE22: C9 01

; useless check, the only values can be $00-$01
BNE CODE_03DE4E         ; $03:DE24: D0 28
LDX $9E                 ; $03:DE26: A6 9E
LDA #$02                ; $03:DE28: A9 02
...
; Get sprite index
LDX $9E                 ; $03:DE31: A6 9E

; Set "fireflower get" animation
LDA #$0C                ; $03:DE33: A9 0C
BRA CODE_03DE49         ; $03:DE35: 80 12

CODE_03DE37:
LDA #$0B                ; $03:DE37: A9 0B

; Get 1up
STA $0110,x             ; $03:DE39: 9D 10 01
LDA #$00                ; $03:DE3C: A9 00

; No SFX?
STA $1600               ; $03:DE3E: 8D 00 16
RTS                     ; $03:DE41: 60

CODE_03DE42:
LDA #$01                ; $03:DE42: A9 01

; Set Mario's powerup
STA $0756               ; $03:DE44: 8D 56 07

; Set "Mario got a powerup" flag
LDA #$09                ; $03:DE47: A9 09

; Mario standing still flag?

CODE_03DE49:
LDY #$00                ; $03:DE49: A0 00

; Goto powerup stuff
JSR CODE_03DF6C         ; $03:DE4B: 20 6C DF
...
CODE_03DE6C:
LDA $03D9,x             ; $03:DE6C: BD D9 03
BNE CODE_03DE8F         ; $03:DE6F: D0 1E

CODE_03DE71:
LDA $0F                 ; $03:DE71: A5 0F
...
; Enter pipe/hurt sound
STA $1600               ; $03:DF61: 8D 00 16
JSL CODE_049A88         ; $03:DF64: 22 88 9A 04
LDA #$0A                ; $03:DF68: A9 0A

CODE_03DF6A:
LDY #$01                ; $03:DF6A: A0 01

CODE_03DF6C:
STA $0F                 ; $03:DF6C: 85 0F
STY $28                 ; $03:DF6E: 84 28
LDY #$FF                ; $03:DF70: A0 FF

; Everything freezes timer
STY $0747               ; $03:DF72: 8C 47 07
INY                     ; $03:DF75: C8
STY $0775               ; $03:DF76: 8C 75 07

CODE_03DF79:
LDX $9E                 ; $03:DF79: A6 9E
RTS                     ; $03:DF7B: 60

CODE_03DF7C:
STX $5D                 ; $03:DF7C: 86 5D
PHX                     ; $03:DF7E: DA
...
CODE_03DFD4:
LDA DATA_03DFA2,y       ; $03:DFD4: B9 A2 DF
...
CODE_03E12B:
JSR CODE_03E143         ; $03:E12B: 20 43 E1
BRA CODE_03E139         ; $03:E12E: 80 09

CODE_03E130:
LDA $0481,y             ; $03:E130: B9 81 04
...
CODE_03E140:
LDX $9E                 ; $03:E140: A6 9E
RTS                     ; $03:E142: 60

CODE_03E143:
LDA $0029,y             ; $03:E143: B9 29 00
...
CODE_03E201:
JSR CODE_03E2ED         ; $03:E201: 20 ED E2
...
CODE_03E21B:
LDX $9E                 ; $03:E21B: A6 9E
RTS                     ; $03:E21D: 60

CODE_03E21E:
LDA $0747               ; $03:E21E: AD 47 07
...
CODE_03E25A:
LDX $9E                 ; $03:E25A: A6 9E
RTS                     ; $03:E25C: 60

CODE_03E25D:
LDX $9E                 ; $03:E25D: A6 9E

CODE_03E25F:
LDA $0F9F,y             ; $03:E25F: B9 9F 0F
SEC                     ; $03:E262: 38
...
CODE_03E2B4:
JSR CODE_03E6B5         ; $03:E2B4: 20 B5 E6

CODE_03E2B7:
LDX $9E                 ; $03:E2B7: A6 9E
RTS                     ; $03:E2B9: 60
...
CODE_03E2CB:
LDA $0238,x             ; $03:E2CB: BD 38 02

CODE_03E2CE:
LDY $0F                 ; $03:E2CE: A4 0F
...
CODE_03E309:
LDA $0716               ; $03:E309: AD 16 07

; When "pass through blocks" flag is enabled
; return.
BNE CODE_03E33D         ; $03:E30C: D0 2F
LDA $0F                 ; $03:E30E: A5 0F
CMP #$0B                ; $03:E310: C9 0B

; When in death animation
; return
BEQ CODE_03E33D         ; $03:E312: F0 29
CMP #$04                ; $03:E314: C9 04

; When on flagpole?
; return
BCC CODE_03E33D         ; $03:E316: 90 25
LDA #$01                ; $03:E318: A9 01
LDY $0704               ; $03:E31A: AC 04 07

; If swimming, make player state 01
; branch
BNE CODE_03E329         ; $03:E31D: D0 0A
LDA $28                 ; $03:E31F: A5 28
BEQ CODE_03E327         ; $03:E321: F0 04
CMP #$03                ; $03:E323: C9 03
BNE CODE_03E32B         ; $03:E325: D0 04

CODE_03E327:
LDA #$02                ; $03:E327: A9 02

; If player state is 00, make it 02
; If player state is 03, keep it 03

CODE_03E329:
STA $28                 ; $03:E329: 85 28

CODE_03E32B:
LDA $BB                 ; $03:E32B: A5 BB
CMP #$01                ; $03:E32D: C9 01

; If position of Mario is NOT inside range --
; (AKA anywhere he can shoot fireballs), return
BNE CODE_03E33D         ; $03:E32F: D0 0C
LDA #$FF                ; $03:E331: A9 FF

; note to self: look at what this store does.
STA $0480               ; $03:E333: 8D 80 04
LDA $0237               ; $03:E336: AD 37 02
CMP #$CF                ; $03:E339: C9 CF

; Branch if player Y-position is inside the game
; (AKA not in a bottomless pit)
BCC CODE_03E33E         ; $03:E33B: 90 01
...
; Block interaction routine

CODE_03E33E:
LDY #$02                ; $03:E33E: A0 02
LDA $0714               ; $03:E340: AD 14 07

; If Mario is ducking, branch, #$0E into $EB
BNE CODE_03E351         ; $03:E343: D0 0C
LDA $0754               ; $03:E345: AD 54 07

; If Small Mario, branch, #$0E into $EB
BNE CODE_03E351         ; $03:E348: D0 07
DEY                     ; $03:E34A: 88
LDA $0704               ; $03:E34B: AD 04 07
BNE CODE_03E351         ; $03:E34E: D0 01

; If swimming, branch, #$07 into $EB
;
; If anything else (normal non-small mario), #$00 into $EB
DEY                     ; $03:E350: 88

CODE_03E351:
LDA DATA_03EB3F,y       ; $03:E351: B9 3F EB

; Set Mario's hitbox for object interaction?
STA $EB                 ; $03:E354: 85 EB

; Hitbox size to Y
TAY                     ; $03:E356: A8

; Mario Small Flag to X
LDX $0754               ; $03:E357: AE 54 07
LDA $0714               ; $03:E35A: AD 14 07

; If Mario isn't ducking
; branch
BEQ CODE_03E360         ; $03:E35D: F0 01

; Mario is ducking, so Mario Small flag index indicates mario = small
INX                     ; $03:E35F: E8

CODE_03E360:
LDA $0237               ; $03:E360: AD 37 02
CMP DATA_03E307,x       ; $03:E363: DD 07 E3

; Mario's Y position
; If it's smaller than $20 (Small mario), or $10 (big mario)
; branch (to make mario not interact w/ objects on/above statusbar)
BCC CODE_03E3A0         ; $03:E366: 90 38
LDA #$01                ; $03:E368: A9 01
STA $E4                 ; $03:E36A: 85 E4
JSR CODE_03EB7B         ; $03:E36C: 20 7B EB
BEQ CODE_03E3A0         ; $03:E36F: F0 2F
JSR CODE_03E70D         ; $03:E371: 20 0D E7

; Check if tile is E9 or EA, and if so, play the coin sound
; Then branch. If not tile E9 or EA, continue.
BCS CODE_03E3C9         ; $03:E374: B0 53
LDY $A0                 ; $03:E376: A4 A0

; If Mario y-speed is that of falling, branch
...
CODE_03E393:
CMP #$2A                ; $03:E393: C9 2A
BEQ CODE_03E39C         ; $03:E395: F0 05
LDA #$01                ; $03:E397: A9 01
STA $1600               ; $03:E399: 8D 00 16

CODE_03E39C:
LDA #$01                ; $03:E39C: A9 01

; Executes as soon as mario hits a cement-block solid
; block from below which does NOT spawn ANYTHING
; Play sound, set Y-speed to downwards
STA $A0                 ; $03:E39E: 85 A0

CODE_03E3A0:
LDY $EB                 ; $03:E3A0: A4 EB
...
CODE_03E3C9:
JMP CODE_03E4C8         ; $03:E3C9: 4C C8 E4

CODE_03E3CC:
JSR CODE_03E706         ; $03:E3CC: 20 06 E7
...
CODE_03E3DC:
JSR CODE_03E5FA         ; $03:E3DC: 20 FA E5
...
CODE_03E3F3:
JSR CODE_03E601         ; $03:E3F3: 20 01 E6
...
CODE_03E40B:
LDA #$00                ; $03:E40B: A9 00
STA $28                 ; $03:E40D: 85 28

CODE_03E40F:
LDY $EB                 ; $03:E40F: A4 EB
INY                     ; $03:E411: C8
INY                     ; $03:E412: C8
...
CODE_03E453:
JSR CODE_03E5FA         ; $03:E453: 20 FA E5
...
CODE_03E460:
JSR CODE_03E70D         ; $03:E460: 20 0D E7
...
; Enter pipe/hurt sound
STY $1600               ; $03:E491: 8C 00 16
STY $9A                 ; $03:E494: 84 9A
STZ $07AF               ; $03:E496: 9C AF 07

CODE_03E499:
AND #$CF                ; $03:E499: 29 CF
...
CODE_03E4AD:
LDA DATA_03E4C6,y       ; $03:E4AD: B9 C6 E4
STA $06DE               ; $03:E4B0: 8D DE 06

CODE_03E4B3:
LDA $0F                 ; $03:E4B3: A5 0F
...
CODE_03E4C2:
JSR CODE_03E6B5         ; $03:E4C2: 20 B5 E6
...
CODE_03E4E7:
LDA #$18                ; $03:E4E7: A9 18
STA $5D                 ; $03:E4E9: 85 5D
PHX                     ; $03:E4EB: DA
LDX #$00                ; $03:E4EC: A2 00

CODE_03E4EE:
LDA $1C,x               ; $03:E4EE: B5 1C
...
LDA #$F0                ; $03:E556: A9 F0

; Music Fade Out
STA $1602               ; $03:E558: 8D 02 16
LDA #$46                ; $03:E55B: A9 46
...
CODE_03E56C:
CMP DATA_03E521,x       ; $03:E56C: DD 21 E5
BCS CODE_03E574         ; $03:E56F: B0 03
DEX                     ; $03:E571: CA
BNE CODE_03E56C         ; $03:E572: D0 F8

CODE_03E574:
STX $010F               ; $03:E574: 8E 0F 01

CODE_03E577:
LDA #$04                ; $03:E577: A9 04
STA $0F                 ; $03:E579: 85 0F
JMP CODE_03E58D         ; $03:E57B: 4C 8D E5

CODE_03E57E:
CMP #$2A                ; $03:E57E: C9 2A
BNE CODE_03E58D         ; $03:E580: D0 0B
LDA $0237               ; $03:E582: AD 37 02
CMP #$20                ; $03:E585: C9 20

; If player Y-position is not above the flagpole
BCS CODE_03E58D         ; $03:E587: B0 04
LDA #$01                ; $03:E589: A9 01

; Player action: climb a vine/slide down the flagpole
STA $0F                 ; $03:E58B: 85 0F

CODE_03E58D:
LDA #$03                ; $03:E58D: A9 03

; Player state: slide down the flagpole
STA $28                 ; $03:E58F: 85 28
LDA #$00                ; $03:E591: A9 00
STA $5D                 ; $03:E593: 85 5D

; Store zero to player X-speed
;
; Full left movement, minimal right movement flag?
STA $0705               ; $03:E595: 8D 05 07
...
CODE_03E601:
JSR CODE_03E61A         ; $03:E601: 20 1A E6
...
CODE_03E67A:
LDY DATA_0390D8,x       ; $03:E67A: BC D8 90
DEY                     ; $03:E67D: 88
...
CODE_03E6AF:
LDA #$04                ; $03:E6AF: A9 04

; Enter pipe/hurt sound
STA $1600               ; $03:E6B1: 8D 00 16
RTS                     ; $03:E6B4: 60

CODE_03E6B5:
LDA #$00                ; $03:E6B5: A9 00
...
CODE_03E6FB:
JSR CODE_03E71D         ; $03:E6FB: 20 1D E7
...
CODE_03E706:
JSR CODE_03E71D         ; $03:E706: 20 1D E7
CMP DATA_03E702,x       ; $03:E709: DD 02 E7
RTS                     ; $03:E70C: 60

CODE_03E70D:
CMP #$E9                ; $03:E70D: C9 E9
...
CODE_03E717:
LDA #$01                ; $03:E717: A9 01

; Coin sound
STA $1603               ; $03:E719: 8D 03 16
...
CODE_03E760:
JSR CODE_03E92C         ; $03:E760: 20 2C E9
BNE CODE_03E768         ; $03:E763: D0 03

CODE_03E765:
JMP CODE_03E851         ; $03:E765: 4C 51 E8

CODE_03E768:
JSR CODE_03E933         ; $03:E768: 20 33 E9
...
CODE_03E784:
LDA #$01                ; $03:E784: A9 01
JSR CODE_03E07E         ; $03:E786: 20 7E E0

CODE_03E789:
CMP #$09                ; $03:E789: C9 09
...
CODE_03E799:
AND #$01                ; $03:E799: 29 01
STA $1C,x               ; $03:E79B: 95 1C

CODE_03E79D:
LDA $29,x               ; $03:E79D: B5 29
...
CODE_03E7B7:
LDA #$FF                ; $03:E7B7: A9 FF

CODE_03E7B9:
STA $A1,x               ; $03:E7B9: 95 A1
...
CODE_03E7EA:
JMP CODE_03E86D         ; $03:E7EA: 4C 6D E8

CODE_03E7ED:
LDA $29,x               ; $03:E7ED: B5 29
...
CODE_03E83C:
JSR CODE_03E8CA         ; $03:E83C: 20 CA E8
...
CODE_03E84A:
LDA $29,x               ; $03:E84A: B5 29
AND #$BF                ; $03:E84C: 29 BF
STA $29,x               ; $03:E84E: 95 29
RTS                     ; $03:E850: 60

CODE_03E851:
LDA $1C,x               ; $03:E851: B5 1C
...
CODE_03E868:
LDA DATA_03E726,y       ; $03:E868: B9 26 E7

CODE_03E86B:
STA $29,x               ; $03:E86B: 95 29

CODE_03E86D:
LDA $0238,x             ; $03:E86D: BD 38 02
...
LDA #$01                ; $03:E8A1: A9 01

; Hit hard block sound
STA $1600               ; $03:E8A3: 8D 00 16
JSL CODE_05D2B1         ; $03:E8A6: 22 B1 D2 05

CODE_03E8AA:
LDA $1C,x               ; $03:E8AA: B5 1C
...
CODE_03E8B9:
JMP CODE_03E1D8         ; $03:E8B9: 4C D8 E1

; SUB_HORZ_POS. Distance between Player and the sprite gets stored in $00

CODE_03E8BC:
LDA $021A,x             ; $03:E8BC: BD 1A 02
SEC                     ; $03:E8BF: 38
...
CODE_03E8CA:
JSR CODE_03C726         ; $03:E8CA: 20 26 C7
...
CODE_03E8E1:
JSR CODE_03E8D8         ; $03:E8E1: 20 D8 E8
...
CODE_03E900:
JMP CODE_03E86D         ; $03:E900: 4C 6D E8

CODE_03E903:
JSR CODE_03E92C         ; $03:E903: 20 2C E9
BEQ CODE_03E925         ; $03:E906: F0 1D
CMP #$27                ; $03:E908: C9 27
BNE CODE_03E914         ; $03:E90A: D0 08

CODE_03E90C:
JSR CODE_03DD81         ; $03:E90C: 20 81 DD
LDA #$FC                ; $03:E90F: A9 FC
STA $A1,x               ; $03:E911: 95 A1
RTS                     ; $03:E913: 60

CODE_03E914:
LDA $0792,x             ; $03:E914: BD 92 07
...
CODE_03EA98:
STA $0F9E,y             ; $03:EA98: 99 9E 0F

CODE_03EA9B:
LDX $9E                 ; $03:EA9B: A6 9E
RTS                     ; $03:EA9D: 60

CODE_03EA9E:
LDA $0F9C,y             ; $03:EA9E: B9 9C 0F
...
CODE_03EAB1:
STA $0F9C,y             ; $03:EAB1: 99 9C 0F

CODE_03EAB4:
LDX $9E                 ; $03:EAB4: A6 9E
RTS                     ; $03:EAB6: 60

CODE_03EAB7:
LDX #$00                ; $03:EAB7: A2 00

CODE_03EAB9:
STY $06                 ; $03:EAB9: 84 06
LDA #$01                ; $03:EABB: A9 01
STA $07                 ; $03:EABD: 85 07

CODE_03EABF:
LDA $0F9C,y             ; $03:EABF: B9 9C 0F
...
CODE_03EB35:
LDA #$00                ; $03:EB35: A9 00

CODE_03EB37:
JSR CODE_03EB83         ; $03:EB37: 20 83 EB
LDX $9E                 ; $03:EB3A: A6 9E
CMP #$00                ; $03:EB3C: C9 00
RTS                     ; $03:EB3E: 60

; Mario<->block interaction "offset". Big Mario, Big swimming mario, Small mario
...
CODE_03EB7B:
LDA #$00                ; $03:EB7B: A9 00
BRA CODE_03EB81         ; $03:EB7D: 80 02

CODE_03EB7F:
LDA #$01                ; $03:EB7F: A9 01

CODE_03EB81:
LDX #$00                ; $03:EB81: A2 00
...
CLC                     ; $03:EB89: 18
ADC $0219,x             ; $03:EB8A: 7D 19 02
STA $05                 ; $03:EB8D: 85 05
LDA $78,x               ; $03:EB8F: B5 78
...
; X-position of Mario sprite
;
; Screen of player
LSR A                   ; $03:EB9B: 4A

; put values in $06 for indirect pointer?
JSR CODE_03ACF6         ; $03:EB9C: 20 F6 AC
LDY $04                 ; $03:EB9F: A4 04
LDA $0237,x             ; $03:EBA1: BD 37 02
CLC                     ; $03:EBA4: 18
ADC DATA_03EB5E,y       ; $03:EBA5: 79 5E EB
AND #$F0                ; $03:EBA8: 29 F0
SEC                     ; $03:EBAA: 38
SBC #$20                ; $03:EBAB: E9 20

; Y-position of Mario sprite
;
; $7E0002 = (((MarioYpos+TableEB5E)&0xF0)-0x20)
; Is that 0x20 here to compensate for that statusbar area?
STA $02                 ; $03:EBAD: 85 02
TAY                     ; $03:EBAF: A8
LDA ($06),y             ; $03:EBB0: B1 06

; Store block Mario is interacting with in $03
; This indirect pointer points to the $0500 area. It contains
STA $03                 ; $03:EBB2: 85 03

; the object layout of the current screen.
LDY $04                 ; $03:EBB4: A4 04
PLA                     ; $03:EBB6: 68
...
CODE_03EBBF:
LDA $0219,x             ; $03:EBBF: BD 19 02

CODE_03EBC2:
AND #$0F                ; $03:EBC2: 29 0F
...
CODE_03EBD4:
STZ $03                 ; $03:EBD4: 64 03

CODE_03EBD6:
LDA $03                 ; $03:EBD6: A5 03
RTS                     ; $03:EBD8: 60
...
CODE_03EC22:
LDX #$06                ; $03:EC22: A2 06

CODE_03EC24:
STA $0900,y             ; $03:EC24: 99 00 09
CLC                     ; $03:EC27: 18
...
CODE_03EC3C:
STA $080D,y             ; $03:EC3C: 99 0D 08
STA $0809,y             ; $03:EC3F: 99 09 08

CODE_03EC42:
STA $0805,y             ; $03:EC42: 99 05 08
STA $0801,y             ; $03:EC45: 99 01 08
RTS                     ; $03:EC48: 60

CODE_03EC49:
LDA #$F0                ; $03:EC49: A9 F0

CODE_03EC4B:
STA $0915,y             ; $03:EC4B: 99 15 09
STA $0911,y             ; $03:EC4E: 99 11 09

CODE_03EC51:
STA $090D,y             ; $03:EC51: 99 0D 09

CODE_03EC54:
STA $0909,y             ; $03:EC54: 99 09 09

CODE_03EC57:
STA $0905,y             ; $03:EC57: 99 05 09
STA $0901,y             ; $03:EC5A: 99 01 09
RTS                     ; $03:EC5D: 60

CODE_03EC5E:
LDA $0743               ; $03:EC5E: AD 43 07
BEQ CODE_03EC66         ; $03:EC61: F0 03
JMP CODE_03ECEA         ; $03:EC63: 4C EA EC

CODE_03EC66:
LDY $0B46,x             ; $03:EC66: BC 46 0B
...
CODE_03EC84:
LDA #$F0                ; $03:EC84: A9 F0

CODE_03EC86:
LDY $0B46,x             ; $03:EC86: BC 46 0B
...
CODE_03ECE6:
JSR CODE_03EC49         ; $03:ECE6: 20 49 EC

CODE_03ECE9:
RTS                     ; $03:ECE9: 60

CODE_03ECEA:
JSL CODE_05CDEB         ; $03:ECEA: 22 EB CD 05
RTS                     ; $03:ECEE: 60
...
CODE_03ED2D:
LDA DATA_03ECEF,x       ; $03:ED2D: BD EF EC
...
; Erase sprite
JSR CODE_03CDE2         ; $03:EF04: 20 E2 CD

CODE_03EF07:
JSL CODE_05CB42         ; $03:EF07: 22 42 CB 05
RTS                     ; $03:EF0B: 60

CODE_03EF0C:
CMP #$33                ; $03:EF0C: C9 33
...
; Erase sprite
JSR CODE_03CDE2         ; $03:EF16: 20 E2 CD

CODE_03EF19:
JSL CODE_05CB42         ; $03:EF19: 22 42 CB 05
RTS                     ; $03:EF1D: 60

CODE_03EF1E:
CMP #$2D                ; $03:EF1E: C9 2D
BNE CODE_03EF27         ; $03:EF20: D0 05
JSL CODE_05D055         ; $03:EF22: 22 55 D0 05
RTS                     ; $03:EF26: 60

CODE_03EF27:
CMP #$35                ; $03:EF27: C9 35
...
CODE_03EF37:
JSL CODE_04DCBB         ; $03:EF37: 22 BB DC 04
RTS                     ; $03:EF3B: 60

CODE_03EF3C:
LDA $0238,x             ; $03:EF3C: BD 38 02
...
CODE_03EFBC:
STY $EF                 ; $03:EFBC: 84 EF

CODE_03EFBE:
LDY $EF                 ; $03:EFBE: A4 EF
...
CODE_03EFE1:
LDA DATA_03EEA9,y       ; $03:EFE1: B9 A9 EE
...
CODE_03EFEC:
EOR $04                 ; $03:EFEC: 45 04

CODE_03EFEE:
STA $04                 ; $03:EFEE: 85 04
...
CODE_03F011:
LDA DATA_03EE8E,y       ; $03:F011: B9 8E EE
TAX                     ; $03:F014: AA

CODE_03F015:
LDY $EC                 ; $03:F015: A4 EC
...
CODE_03F05C:
BRA CODE_03F0AE         ; $03:F05C: 80 50

CODE_03F05E:
CPX #$90                ; $03:F05E: E0 90
...
CODE_03F071:
JMP CODE_03F11A         ; $03:F071: 4C 1A F1

CODE_03F074:
LDA $EF                 ; $03:F074: A5 EF
...
CODE_03F188:
LDA $EF                 ; $03:F188: A5 EF
CMP #$08                ; $03:F18A: C9 08
BNE CODE_03F191         ; $03:F18C: D0 03

CODE_03F18E:
JMP CODE_03F295         ; $03:F18E: 4C 95 F2

CODE_03F191:
LDA $F0                 ; $03:F191: A5 F0
...
CODE_03F1F6:
CPX #$02                ; $03:F1F6: E0 02
BCC CODE_03F23A         ; $03:F1F8: 90 40

CODE_03F1FA:
LDA $036A               ; $03:F1FA: AD 6A 03
...
; Erase sprite
JSR CODE_03CDE2         ; $03:F2F3: 20 E2 CD

CODE_03F2F6:
RTS                     ; $03:F2F6: 60

CODE_03F2F7:
LDA DATA_03ED8C,x       ; $03:F2F7: BD 8C ED
STA $00                 ; $03:F2FA: 85 00
LDA DATA_03ED8C+1,x     ; $03:F2FC: BD 8D ED

CODE_03F2FF:
STA $01                 ; $03:F2FF: 85 01
...
CODE_03F30B:
JMP CODE_03FF96         ; $03:F30B: 4C 96 FF

CODE_03F30E:
STA $01                 ; $03:F30E: 85 01
JMP CODE_03FF4E         ; $03:F310: 4C 4E FF

CODE_03F313:
STX $9E                 ; $03:F313: 86 9E
...
CODE_03F37C:
LDA #$02                ; $03:F37C: A9 02

CODE_03F37E:
STA $0802,y             ; $03:F37E: 99 02 08
...
CODE_03F3DE:
JSR CODE_03EC42         ; $03:F3DE: 20 42 EC
...
CODE_03F5E9:
JSR CODE_03EC54         ; $03:F5E9: 20 54 EC
PLA                     ; $03:F5EC: 68
CLC                     ; $03:F5ED: 18
...
CODE_03F72F:
STZ $0E40               ; $03:F72F: 9C 40 0E

CODE_03F732:
JSL CODE_05D7A1         ; $03:F732: 22 A1 D7 05

CODE_03F736:
LDA $0F                 ; $03:F736: A5 0F
...
CODE_03F789:
LDA #$02                ; $03:F789: A9 02

CODE_03F78B:
STA $0C10,y             ; $03:F78B: 99 10 0C
...
CODE_03F7AF:
JSR CODE_03FBA2         ; $03:F7AF: 20 A2 FB
BRA CODE_03F7BE         ; $03:F7B2: 80 0A

CODE_03F7B4:
JSR CODE_03FC60         ; $03:F7B4: 20 60 FC
BRA CODE_03F7BE         ; $03:F7B7: 80 05

CODE_03F7B9:
LDY #$0E                ; $03:F7B9: A0 0E
LDA DATA_03F62E,y       ; $03:F7BB: B9 2E F6

CODE_03F7BE:
STA $06D5               ; $03:F7BE: 8D D5 06
...
CODE_03F7D7:
LDA #$E0                ; $03:F7D7: A9 E0

CODE_03F7D9:
STA $06D5               ; $03:F7D9: 8D D5 06

CODE_03F7DC:
LDA #$04                ; $03:F7DC: A9 04
...
CODE_03F861:
JSR CODE_0385F3         ; $03:F861: 20 F3 85
...
CODE_03F86F:
LDA DATA_03F85B,x       ; $03:F86F: BD 5B F8
...
CODE_03F89B:
JSR CODE_03F922         ; $03:F89B: 20 22 F9
...
CODE_03F8CD:
JSR CODE_03FA6E         ; $03:F8CD: 20 6E FA
BRA CODE_03F915         ; $03:F8D0: 80 43

CODE_03F8D2:
LDA $1C,x               ; $03:F8D2: B5 1C
...
CODE_03F915:
LDX #$04                ; $03:F915: A2 04

CODE_03F917:
STZ $1C,x               ; $03:F917: 74 1C
DEX                     ; $03:F919: CA
...
CODE_03F922:
STZ $0E85               ; $03:F922: 9C 85 0E

; "More difficult quest" flag. Sets 076A too and shows star next to world
LDA $07FC               ; $03:F925: AD FC 07
...
CODE_03F965:
LDA #$88                ; $03:F965: A9 88

CODE_03F967:
STA $0238,x             ; $03:F967: 9D 38 02
...
CODE_03FB7E:
LDA DATA_03F63E,x       ; $03:FB7E: BD 3E F6
...
CODE_03FC13:
LDA $070D               ; $03:FC13: AD 0D 07
JMP CODE_03FC80         ; $03:FC16: 4C 80 FC

CODE_03FC19:
LDA #$03                ; $03:FC19: A9 03
BRA CODE_03FC1F         ; $03:FC1B: 80 02

CODE_03FC1D:
LDA #$02                ; $03:FC1D: A9 02

CODE_03FC1F:
STA $00                 ; $03:FC1F: 85 00
...
CODE_03FC73:
STY $070D               ; $03:FC73: 8C 0D 07

CODE_03FC76:
LDA $0754               ; $03:FC76: AD 54 07
...
CODE_03FC95:
LDA DATA_03F62E,y       ; $03:FC95: B9 2E F6
RTS                     ; $03:FC98: 60

CODE_03FC99:
LDY $0B45               ; $03:FC99: AC 45 0B
...
CODE_03FCB6:
LDA $0E4E               ; $03:FCB6: AD 4E 0E
BEQ CODE_03FD12         ; $03:FCB9: F0 57
LDX #$00                ; $03:FCBB: A2 00

CODE_03FCBD:
LDA $08D0,x             ; $03:FCBD: BD D0 08
...
CODE_03FDCB:
LDA #$01                ; $03:FDCB: A9 01
LDY #$01                ; $03:FDCD: A0 01
BRA CODE_03FDD5         ; $03:FDCF: 80 04

CODE_03FDD1:
LDA #$0D                ; $03:FDD1: A9 0D
LDY #$04                ; $03:FDD3: A0 04

; Store current sprite index into $00

CODE_03FDD5:
STX $00                 ; $03:FDD5: 86 00
CLC                     ; $03:FDD7: 18
...
CODE_03FDFF:
JSR CODE_03FE1E         ; $03:FDFF: 20 1E FE
LSR A                   ; $03:FE02: 4A
LSR A                   ; $03:FE03: 4A
LSR A                   ; $03:FE04: 4A
LSR A                   ; $03:FE05: 4A
...
CODE_03FE1E:
STX $04                 ; $03:FE1E: 86 04
LDY #$01                ; $03:FE20: A0 01

CODE_03FE22:
LDA $071C,y             ; $03:FE22: B9 1C 07
SEC                     ; $03:FE25: 38
...
CODE_03FE47:
LDA DATA_03FE0B,x       ; $03:FE47: BD 0B FE
...
CODE_03FE62:
STX $04                 ; $03:FE62: 86 04
LDY #$01                ; $03:FE64: A0 01

CODE_03FE66:
LDA DATA_03FE60,y       ; $03:FE66: B9 60 FE
SEC                     ; $03:FE69: 38
...
CODE_03FE8A:
LDA DATA_03FE54,x       ; $03:FE8A: BD 54 FE
...
CODE_03FEDF:
LDA #$01                ; $03:FEDF: A9 01

CODE_03FEE1:
STA $0D00,y             ; $03:FEE1: 99 00 0D
RTS                     ; $03:FEE4: 60

CODE_03FEE5:
REP #$20                ; $03:FEE5: C2 20
...
CODE_03FEFB:
LDA #$01                ; $03:FEFB: A9 01

CODE_03FEFD:
STA $0D10,y             ; $03:FEFD: 99 10 0D
...
CODE_03FF1D:
LDA #$01                ; $03:FF1D: A9 01

CODE_03FF1F:
STA $0C08,y             ; $03:FF1F: 99 08 0C

...
CODE_058000:
STZ $0EDC               ; $05:8000: 9C DC 0E
PHB                     ; $05:8003: 8B
LDA #$7E                ; $05:8004: A9 7E
PHA                     ; $05:8006: 48
PLB                     ; $05:8007: AB
REP #$30                ; $05:8008: C2 30
LDX #$0000              ; $05:800A: A2 00 00
LDA #$0000              ; $05:800D: A9 00 00

CODE_058010:
STA $D000,x             ; $05:8010: 9D 00 D0
...
; Clear Background map16 tilemap
PLB                     ; $05:803E: AB
PHB                     ; $05:803F: 8B
PHK                     ; $05:8040: 4B
PLB                     ; $05:8041: AB
STZ $0EC0               ; $05:8042: 9C C0 0E
LDA $0E65               ; $05:8045: AD 65 0E
AND #$00FF              ; $05:8048: 29 FF 00
BEQ CODE_05804F         ; $05:804B: F0 02
STA $DB                 ; $05:804D: 85 DB

CODE_05804F:
LDA $DB                 ; $05:804F: A5 DB
AND #$00FF              ; $05:8051: 29 FF 00
ASL A                   ; $05:8054: 0A
TAX                     ; $05:8055: AA
LDA DATA_05AD04,x       ; $05:8056: BD 04 AD

; Data bank = current bank
;
; Index to the current BG2 map16 page being written to
;
; Store current background to $DB. $0E65 is clone
; of $DB (or the opposite......)
;
;
; Layer 2 background
; to 16-bit index
;
; Get relative index to backgrounds properties table, transfer it to X
STA $02                 ; $05:8059: 85 02

CODE_05805B:
REP #$30                ; $05:805B: C2 30
LDX $02                 ; $05:805D: A6 02
LDA DATA_05B557,x       ; $05:805F: BD 57 B5
STA $04                 ; $05:8062: 85 04
INC $02                 ; $05:8064: E6 02

; This table seems to be the group of commands associated with the background scene to load. Format: HHH LLL CCCCCC VVVV?
; ("Background scene" being the graphics tileset, layer 3 flag, HDMA gradient and the BG2 tilemap).
;
; increase pointer to read next word of background commands next time.
INC $02                 ; $05:8066: E6 02
...
; / Store CCCCCC into $EF. It seems to be the "command"
STA $EF                 ; $05:806F: 85 EF
LDA $04                 ; $05:8071: A5 04
AND #$000F              ; $05:8073: 29 0F 00

; Store VVVV data. It seems to be the value associated with the command
STA $F1                 ; $05:8076: 85 F1
LDA $04                 ; $05:8078: A5 04
AND #$E000              ; $05:807A: 29 00 E0

; Load HHH data
; HHH = high nibble of background data starting index of map16 tilemap
STA $ED                 ; $05:807D: 85 ED
LDA $04                 ; $05:807F: A5 04
LSR A                   ; $05:8081: 4A
AND #$0E00              ; $05:8082: 29 00 0E
ORA $ED                 ; $05:8085: 05 ED
XBA                     ; $05:8087: EB
STA $ED                 ; $05:8088: 85 ED
AND #$00F0              ; $05:808A: 29 F0 00
CMP #$00E0              ; $05:808D: C9 E0 00

; Load LLL data
; LLL = Low nibble of background data starting index of map16 tilemap * 2
; This code basically turns HHH LLL into HHH0 LLL0
;
;
;
;
; If #$E0, then $EF is a command to be processed
BNE CODE_0580B3         ; $05:8090: D0 21
LDA $EF                 ; $05:8092: A5 EF
CMP #$003F              ; $05:8094: C9 3F 00

; if not end of background data marker, process background generation
; (Otherwise just skip branch and finish up routine)
BNE CODE_0580AE         ; $05:8097: D0 15
INC $0EC0               ; $05:8099: EE C0 0E
INC $0EC0               ; $05:809C: EE C0 0E
LDA $0EC0               ; $05:809F: AD C0 0E
XBA                     ; $05:80A2: EB
TAX                     ; $05:80A3: AA
LDA #$FFFF              ; $05:80A4: A9 FF FF

; Skip 2 pages and place a BG2 map16 marker word
;
;
; And make sure the final bytes are FFFF to mark the end of the map16 tilemap.
STA $7ED000,x           ; $05:80A7: 9F 00 D0 7E
JMP CODE_059166         ; $05:80AB: 4C 66 91

; Process the background generation command

CODE_0580AE:
JSR CODE_058F19         ; $05:80AE: 20 19 8F

; back to the command processing loop
BRA CODE_05805B         ; $05:80B1: 80 A8

CODE_0580B3:
LDA $0EC0               ; $05:80B3: AD C0 0E
XBA                     ; $05:80B6: EB
CLC                     ; $05:80B7: 18
ADC $ED                 ; $05:80B8: 65 ED
STA $EB                 ; $05:80BA: 85 EB
LDA $EF                 ; $05:80BC: A5 EF
CMP #$0010              ; $05:80BE: C9 10 00
BCC CODE_0580C9         ; $05:80C1: 90 06
JSR CODE_058E85         ; $05:80C3: 20 85 8E

; This part of the routine deals with the background generation command
; (as in, place the tiles in the background)
; $EB stores the starting index of the background objects to be placed at
; At this point $EF isn't a command but rather serves another purpose. TODO
;
;
;
; If EF is #$10 or higher, then ???
...
; background to index. Up to $21 is valid.
TAY                     ; $05:80CE: A8
LDA PNTR_0580E1,y       ; $05:80CF: B9 E1 80

; Setup indirect pointer based on background.
STA $04                 ; $05:80D2: 85 04
LDA #$0005              ; $05:80D4: A9 05 00
...
; Jump to background generation/background loading routines
JMP ($0000)             ; $05:80DE: 6C 00 00
...
; $0A - this points to INSTRUCTION POINTER #0
dw $8125
...
; INSTRUCTION POINTER #0: this points to opcode | Related to generating the goomba pillar background
dw $823F

; Related to generating the goomba pillar background
dw $8244
dw $81F5
dw $82E2
dw $9004
...
CODE_0582AD:
LDA DATA_058223+8,y     ; $05:82AD: B9 2B 82
...
CODE_0582F3:
LDA DATA_05B494,y       ; $05:82F3: B9 94 B4
...
CODE_0583FF:
LDA DATA_0583BA,y       ; $05:83FF: B9 BA 83
...
CODE_058445:
LDY #$0000              ; $05:8445: A0 00 00

CODE_058448:
LDX $EB                 ; $05:8448: A6 EB
...
CODE_05845F:
LDA $7ED000,x           ; $05:845F: BF 00 D0 7E
...
CODE_05846B:
CMP #$02                ; $05:846B: C9 02
BNE CODE_058473         ; $05:846D: D0 04
LDA #$0B                ; $05:846F: A9 0B
BRA CODE_058476         ; $05:8471: 80 03

CODE_058473:
LDA DATA_058422,y       ; $05:8473: B9 22 84

CODE_058476:
STA $7ED000,x           ; $05:8476: 9F 00 D0 7E
...
CODE_0584A6:
LDA #$34                ; $05:84A6: A9 34

CODE_0584A8:
STA $7ED000,x           ; $05:84A8: 9F 00 D0 7E
...
CODE_0584BA:
LDA #$40                ; $05:84BA: A9 40

CODE_0584BC:
STA $7ED010,x           ; $05:84BC: 9F 10 D0 7E
...
CODE_0584CE:
LDA #$4C                ; $05:84CE: A9 4C

CODE_0584D0:
STA $7ED020,x           ; $05:84D0: 9F 20 D0 7E
INX                     ; $05:84D4: E8
...
CODE_0584EF:
LDA #$3D                ; $05:84EF: A9 3D
BRA CODE_0584F9         ; $05:84F1: 80 06

CODE_0584F3:
LDA #$36                ; $05:84F3: A9 36
BRA CODE_0584F9         ; $05:84F5: 80 02

CODE_0584F7:
LDA #$35                ; $05:84F7: A9 35

CODE_0584F9:
STA $7ED000,x           ; $05:84F9: 9F 00 D0 7E
...
CODE_0585FC:
LDA DATA_05B323,y       ; $05:85FC: B9 23 B3
...
CODE_058620:
SEP #$20                ; $05:8620: E2 20
BRA CODE_0585FC         ; $05:8622: 80 D8

CODE_058624:
REP #$20                ; $05:8624: C2 20
...
CODE_05865C:
LDA DATA_05B3E2,y       ; $05:865C: B9 E2 B3
...
CODE_058680:
SEP #$20                ; $05:8680: E2 20
BRA CODE_05865C         ; $05:8682: 80 D8

CODE_058684:
REP #$20                ; $05:8684: C2 20
...
CODE_0586A7:
LDA DATA_05AD48,y       ; $05:86A7: B9 48 AD
...
CODE_0586C8:
SEP #$20                ; $05:86C8: E2 20
BRA CODE_0586A7         ; $05:86CA: 80 DB

CODE_0586CC:
REP #$20                ; $05:86CC: C2 20
...
CODE_058847:
LDY #$0000              ; $05:8847: A0 00 00

CODE_05884A:
STX $E2                 ; $05:884A: 86 E2
SEP #$20                ; $05:884C: E2 20

CODE_05884E:
LDA DATA_05B1A5,y       ; $05:884E: B9 A5 B1
...
CODE_058913:
LDA #$37                ; $05:8913: A9 37
BRA CODE_058919         ; $05:8915: 80 02

CODE_058917:
LDA $E4                 ; $05:8917: A5 E4

CODE_058919:
RTS                     ; $05:8919: 60

CODE_05891A:
LDA $7ED000,x           ; $05:891A: BF 00 D0 7E
...
CODE_058928:
LDA #$38                ; $05:8928: A9 38
BRA CODE_05892E         ; $05:892A: 80 02

CODE_05892C:
LDA $E4                 ; $05:892C: A5 E4

CODE_05892E:
RTS                     ; $05:892E: 60

CODE_05892F:
LDA $7ED000,x           ; $05:892F: BF 00 D0 7E
...
CODE_05899C:
LDA DATA_058971,y       ; $05:899C: B9 71 89
...
CODE_058A01:
LDA $7ED020,x           ; $05:8A01: BF 20 D0 7E
...
CODE_058A0D:
LDA #$04                ; $05:8A0D: A9 04

CODE_058A0F:
STA $7ED020,x           ; $05:8A0F: 9F 20 D0 7E
...
CODE_058A32:
LDA $7ED021,x           ; $05:8A32: BF 21 D0 7E
...
CODE_058A3E:
LDA #$04                ; $05:8A3E: A9 04

CODE_058A40:
STA $7ED021,x           ; $05:8A40: 9F 21 D0 7E
...
CODE_058A7F:
LDA DATA_058A5F+2       ; $05:8A7F: AD 61 8A
...
CODE_058ADF:
LDA DATA_058AC5,y       ; $05:8ADF: B9 C5 8A
...
CODE_058B29:
LDA DATA_058B1A,y       ; $05:8B29: B9 1A 8B
...
CODE_058B51:
SEP #$20                ; $05:8B51: E2 20
LDX $EB                 ; $05:8B53: A6 EB
LDY $EF                 ; $05:8B55: A4 EF

CODE_058B57:
LDA DATA_058B4D,y       ; $05:8B57: B9 4D 8B
...
CODE_058BB7:
LDA DATA_058B7A+6       ; $05:8BB7: AD 80 8B
...
CODE_058BCC:
LDA DATA_058B7A+3       ; $05:8BCC: AD 7D 8B
...
CODE_058BE3:
LDA DATA_058B7A+9       ; $05:8BE3: AD 83 8B
...
CODE_058C6A:
LDA $7ED020,x           ; $05:8C6A: BF 20 D0 7E
...
CODE_058CAF:
LDA DATA_058C5B         ; $05:8CAF: AD 5B 8C
...
CODE_058D26:
LDA DATA_058CD0         ; $05:8D26: AD D0 8C
...
CODE_058D91:
LDA DATA_058D76,y       ; $05:8D91: B9 76 8D
...
CODE_058EA9:
LDX $EB                 ; $05:8EA9: A6 EB

CODE_058EAB:
LDA $EF                 ; $05:8EAB: A5 EF
SEC                     ; $05:8EAD: 38
...
CODE_058EDC:
STA $7ED000,x           ; $05:8EDC: 9F 00 D0 7E
...
; $00 - Increase BG2 map16 tilemap's latest written page number by 1
...
; $09 - Enable Layer 3 image processing
dw CODE_058FFA
...
CODE_058F6F:
LDX #$00D0              ; $05:8F6F: A2 D0 00
LDY #$0000              ; $05:8F72: A0 00 00
SEP #$20                ; $05:8F75: E2 20

CODE_058F77:
LDA DATA_058F3F,y       ; $05:8F77: B9 3F 8F
...
CODE_058F97:
STZ $E4                 ; $05:8F97: 64 E4

CODE_058F99:
LDA $E4                 ; $05:8F99: A5 E4
...
CODE_058FA9:
LDA DATA_058F87,y       ; $05:8FA9: B9 87 8F
...
CODE_058FFA:
SEP #$20                ; $05:8FFA: E2 20
LDA $F1                 ; $05:8FFC: A5 F1

; Process layer 3 image display flag
STA $0EDC               ; $05:8FFE: 8D DC 0E
REP #$20                ; $05:9001: C2 20
RTS                     ; $05:9003: 60

CODE_059004:
LDX #$0080              ; $05:9004: A2 80 00

CODE_059007:
LDA #$0403              ; $05:9007: A9 03 04
...
CODE_05903D:
LDX #$0000              ; $05:903D: A2 00 00
LDA #$0202              ; $05:9040: A9 02 02

CODE_059043:
JSR CODE_058EDC         ; $05:9043: 20 DC 8E
INX                     ; $05:9046: E8
INX                     ; $05:9047: E8
CPX #$0020              ; $05:9048: E0 20 00

; Fill the top 2 rows with blank tiles
BNE CODE_059043         ; $05:904B: D0 F6
LDA #$0101              ; $05:904D: A9 01 01

CODE_059050:
JSR CODE_058EDC         ; $05:9050: 20 DC 8E
INX                     ; $05:9053: E8
INX                     ; $05:9054: E8
CPX #$0030              ; $05:9055: E0 30 00

; Fill the 3rd row with some transition tile into the water.
BNE CODE_059050         ; $05:9058: D0 F6
RTS                     ; $05:905A: 60

; BG map16 tiles to fill the entire cave BG with (the repetitive rocks)
...
CODE_05905F:
LDX #$0000              ; $05:905F: A2 00 00
SEP #$20                ; $05:9062: E2 20

CODE_059064:
LDA DATA_05905B         ; $05:9064: AD 5B 90
...
CODE_059099:
SEP #$20                ; $05:9099: E2 20
LDA $5C                 ; $05:909B: A5 5C
CMP #$02                ; $05:909D: C9 02

; If map type is underground, branch
BEQ CODE_0590A5         ; $05:909F: F0 04
LDA #$5F                ; $05:90A1: A9 5F

; Executes when castle level
; Fill the background's top area with black map16 tiles.
BRA CODE_0590A7         ; $05:90A3: 80 02

; Load background black map16 tile.

CODE_0590A5:
LDA #$00                ; $05:90A5: A9 00

CODE_0590A7:
LDX #$0000              ; $05:90A7: A2 00 00

CODE_0590AA:
JSR CODE_058EDC         ; $05:90AA: 20 DC 8E
INX                     ; $05:90AD: E8
CPX #$0020              ; $05:90AE: E0 20 00
BNE CODE_0590AA         ; $05:90B1: D0 F7
REP #$20                ; $05:90B3: C2 20

; Fill the background's top 2 rows with black tiles loaded in accumulator
RTS                     ; $05:90B5: 60

CODE_0590B6:
INC $0EC0               ; $05:90B6: EE C0 0E
...
CODE_0590C4:
CMP #$01                ; $05:90C4: C9 01
BNE CODE_0590CB         ; $05:90C6: D0 03
STA $0ED1               ; $05:90C8: 8D D1 0E

CODE_0590CB:
JSL CODE_04825E         ; $05:90CB: 22 5E 82 04
REP #$30                ; $05:90CF: C2 30

; Enable HDMA depending on $F1: HDMA gradient type
;
;
;
;
...
CODE_0590D2:
SEP #$30                ; $05:90D2: E2 30
LDA #$02                ; $05:90D4: A9 02
JSL CODE_04825E         ; $05:90D6: 22 5E 82 04
REP #$30                ; $05:90DA: C2 30

; Enable underwater HDMA gradient (without the up/down movement, just the color definition)
RTS                     ; $05:90DC: 60
...
CODE_0590E9:
LDX #$00D0              ; $05:90E9: A2 D0 00
LDY $F1                 ; $05:90EC: A4 F1
LDA DATA_0590DD,y       ; $05:90EE: B9 DD 90

CODE_0590F1:
JSR CODE_058EDC         ; $05:90F1: 20 DC 8E
INX                     ; $05:90F4: E8
...
CODE_0590FD:
JSR CODE_058EDC         ; $05:90FD: 20 DC 8E
INX                     ; $05:9100: E8
...
CODE_05910D:
LDA $F1                 ; $05:910D: A5 F1
STA $99                 ; $05:910F: 85 99

; Upload tileset graphics
JSL CODE_05E6B1         ; $05:9111: 22 B1 E6 05
RTS                     ; $05:9115: 60

CODE_059116:
LDA $F1                 ; $05:9116: A5 F1
ORA #$0010              ; $05:9118: 09 10 00
STA $99                 ; $05:911B: 85 99

; Upload tileset graphics
JSL CODE_05E6B1         ; $05:911D: 22 B1 E6 05
RTS                     ; $05:9121: 60

; index to layer 2 background pointers

DATA_059122:
dw $0000,$0000,$0009,$0004

; this is indexed by $7E00DB
dw $0006,$0001,$0004,$0004
dw $0001,$0008,$000A,$0001

; See the RAM map for the list of backgrounds
...
CODE_059166:
LDA $DB                 ; $05:9166: A5 DB
ASL A                   ; $05:9168: 0A
TAX                     ; $05:9169: AA
LDA DATA_059122,x       ; $05:916A: BD 22 91
ASL A                   ; $05:916D: 0A
TAX                     ; $05:916E: AA
LDA PNTR_0591CE,x       ; $05:916F: BD CE 91
...
; to index
;
; Load index to layer 2 background pointer
;
;
; Store pointer to $00-$02. $02 is hardcoded to bank 5
STA $02                 ; $05:9177: 85 02

; Index to RAM?
LDX #$0000              ; $05:9179: A2 00 00

; Loop count
LDY #$0000              ; $05:917C: A0 00 00

CODE_05917F:
STX $E4                 ; $05:917F: 86 E4
TYX                     ; $05:9181: BB
LDA $7ED000,x           ; $05:9182: BF 00 D0 7E
CMP #$FFFF              ; $05:9186: C9 FF FF

; Start loop
;
;
; If the background map16 tilemap is $FFFF
; Stop uploading tiles.
BEQ CODE_0591CA         ; $05:9189: F0 3F
PHY                     ; $05:918B: 5A
...
; Upload background data by copying -MAP16- data
; to the -VRAM tilemap- located in the -RAM-
...
INX                     ; $05:91C3: E8
INX                     ; $05:91C4: E8
CPY #$1000              ; $05:91C5: C0 00 10

; read full map16 (?) ($0FFF bytes). If not done yet, loop
BNE CODE_05917F         ; $05:91C8: D0 B5

CODE_0591CA:
SEP #$30                ; $05:91CA: E2 30
PLB                     ; $05:91CC: AB
RTL                     ; $05:91CD: 6B

; pointers to various background map16 data
...
; Mario bonus room background commands
...
DATA_05B61D:
dw $E091,$E04F,$E020,$8000
dw $E000,$8001,$E000,$8002
dw $E3F0

DATA_06B62F:
dw $E04B,$E011,$0010,$0C11
...
DATA_05BBD1:
dw $E091,$E042,$E010,$2470
dw $24E0,$E3F0

DATA_05BBDD:
dw $E091,$E045,$E010,$8820
...
DATA_05BC0F:
dw $E010,$E041,$2800,$E000
dw $E000,$2800,$E3F0

DATA_05BC1D:
dw $E091,$E042,$E013,$E04D
...
DATA_05BF0B:
dw $E091,$E013,$E042,$E04D
dw $C080,$C422,$B0A0,$D403
dw $9482,$9476,$E000,$80A0
dw $A405,$E3F0

DATA_05BF27:
dw $E091,$E042,$E010,$6C70
...
CODE_05C95B:
LDA $7FFB00             ; $05:C95B: AF 00 FB 7F
...
CODE_05C997:
LDA $701FE0,x           ; $05:C997: BF E0 1F 70
...
CODE_05CA13:
LDX #$00                ; $05:CA13: A2 00
STZ $E7                 ; $05:CA15: 64 E7

CODE_05CA17:
LDA $0300,x             ; $05:CA17: BD 00 03
...
CODE_05CA76:
LDA #$23                ; $05:CA76: A9 23

CODE_05CA78:
LDY $E7                 ; $05:CA78: A4 E7
INY                     ; $05:CA7A: C8
INY                     ; $05:CA7B: C8
INY                     ; $05:CA7C: C8
PHA                     ; $05:CA7D: 48
...
CODE_05CA90:
LDA DATA_05CAE5,x       ; $05:CA90: BD E5 CA
...
CODE_05CB84:
STZ $0F4F,x             ; $05:CB84: 9E 4F 0F

CODE_05CB87:
LDA $0F                 ; $05:CB87: A5 0F
...
CODE_05CC23:
LDA #$F0                ; $05:CC23: A9 F0

CODE_05CC25:
STA $03CE               ; $05:CC25: 8D CE 03
...
CODE_05CCE4:
LDA DATA_05CCAF,y       ; $05:CCE4: B9 AF CC
STA $043D,x             ; $05:CCE7: 9D 3D 04
STZ $06CB               ; $05:CCEA: 9C CB 06
LDA #$08                ; $05:CCED: A9 08
...
CODE_05CDD6:
ORA $E9                 ; $05:CDD6: 05 E9
STA $0D08,y             ; $05:CDD8: 99 08 0D

CODE_05CDDB:
LDX $9E                 ; $05:CDDB: A6 9E
...
CODE_05CDFD:
LDA DATA_05CF1B,x       ; $05:CDFD: BD 1B CF
CLC                     ; $05:CE00: 18
...
; Bowser consists of 6 tiles: [3][6]
...
; Where 3 and 6 are his 8x8 hair tiles
db $C0,$E0,$C4,$EE,$E2,$C5,$CC,$E0

; This table is indexed by $DD
...
CODE_05D089:
LDA DATA_05CF74,y       ; $05:D089: B9 74 CF
...
CODE_05D0D5:
LDA #$21                ; $05:D0D5: A9 21

CODE_05D0D7:
STA $0903,y             ; $05:D0D7: 99 03 09
...
; Index to Bowser's OAM tile data
LDA $DD                 ; $05:D101: A5 DD
CLC                     ; $05:D103: 18
...
CODE_05D10C:
LDA DATA_05CF20,x       ; $05:D10C: BD 20 CF
...
LDA #$28                ; $05:D19B: A9 28

; Freeze bowser's animation
STA $0F4C               ; $05:D19D: 8D 4C 0F
LDA #$3A                ; $05:D1A0: A9 3A

; Bowser breathe fire sound
STA $1600               ; $05:D1A2: 8D 00 16
JSR CODE_05CCB1         ; $05:D1A5: 20 B1 CC
BRA CODE_05D1AD         ; $05:D1A8: 80 03

CODE_05D1AA:
STZ $014B               ; $05:D1AA: 9C 4B 01

CODE_05D1AD:
LDA $0283               ; $05:D1AD: AD 83 02
...
CODE_05D1F1:
JSR CODE_05C9D3         ; $05:D1F1: 20 D3 C9
PLB                     ; $05:D1F4: AB
RTL                     ; $05:D1F5: 6B
PHA                     ; $05:D1F6: 48
PHB                     ; $05:D1F7: 8B
...
CODE_05D23E:
LDX #$00                ; $05:D23E: A2 00

CODE_05D240:
LDA DATA_05D2A6,x       ; $05:D240: BD A6 D2
...
CODE_05D307:
LDA DATA_05D46D,x       ; $05:D307: BD 6D D4
TAY                     ; $05:D30A: A8
...
CODE_05D339:
JSR CODE_05D458         ; $05:D339: 20 58 D4
...
CODE_05D36D:
STA $0C00,y             ; $05:D36D: 99 00 0C
INC $0B25,x             ; $05:D370: FE 25 0B
RTS                     ; $05:D373: 60

CODE_05D374:
JSR CODE_05D458         ; $05:D374: 20 58 D4
...
CODE_05D3A3:
LDA #$02                ; $05:D3A3: A9 02

CODE_05D3A5:
STA $0C00,y             ; $05:D3A5: 99 00 0C
...
CODE_05D4E1:
LDA #$25                ; $05:D4E1: A9 25

; Bowser lands in lava sound impact
STA $1600               ; $05:D4E3: 8D 00 16
LDA #$20                ; $05:D4E6: A9 20

; Bowser lands in lava sound
STA $1603               ; $05:D4E8: 8D 03 16

CODE_05D4EB:
JSR CODE_05D4F2         ; $05:D4EB: 20 F2 D4

CODE_05D4EE:
JMP CODE_05E3BF         ; $05:D4EE: 4C BF E3
RTL                     ; $05:D4F1: 6B

CODE_05D4F2:
LDA $96                 ; $05:D4F2: A5 96
...
CMP #$02                ; $05:D61C: C9 02

; Check if you left a fire particle mid-air
; If so, return
BEQ CODE_05D657         ; $05:D61E: F0 37
LSR A                   ; $05:D620: 4A

; Check if you landed in lava just now
; If so, return
BCC CODE_05D66C         ; $05:D621: 90 49
...
TAY                     ; $05:D62C: A8
LDA DATA_05D60A,y       ; $05:D62D: B9 0A D6

; Particle tile number in $ED
STA $ED                 ; $05:D630: 85 ED
LDA $021A,x             ; $05:D632: BD 1A 02
STA $00                 ; $05:D635: 85 00
LDA $79,x               ; $05:D637: B5 79

; Store 16-bit sprite x position into $00
...
; Add 8 pixels to it, but subtract the left edge x-coordinate of the screen. store into $00-01
...
CODE_05D657:
BRA CODE_05D69D         ; $05:D657: 80 44

; Executes after a particle has been placed.

CODE_05D659:
INC $0B00,x             ; $05:D659: FE 00 0B
STZ $0B09,x             ; $05:D65C: 9E 09 0B
LDA $0B00,x             ; $05:D65F: BD 00 0B
CMP #$02                ; $05:D662: C9 02

; If the generated particle in question is the mid-air fire particle
BNE CODE_05D66C         ; $05:D664: D0 06
LDA $0238,x             ; $05:D666: BD 38 02

; Set Y position of mid-air particle
...
CODE_05D69D:
LDA $0B09,x             ; $05:D69D: BD 09 0B
LSR A                   ; $05:D6A0: 4A
LSR A                   ; $05:D6A1: 4A
LSR A                   ; $05:D6A2: 4A
CMP #$05                ; $05:D6A3: C9 05

; If less than #$28, branch
BCC CODE_05D6AF         ; $05:D6A5: 90 08
LDA #$30                ; $05:D6A7: A9 30
STA $0B09,x             ; $05:D6A9: 9D 09 0B

; If $28 or higher, just set it to $30
;
; Then return
JMP CODE_05D739         ; $05:D6AC: 4C 39 D7

CODE_05D6AF:
STA $EB                 ; $05:D6AF: 85 EB
ASL A                   ; $05:D6B1: 0A
TAY                     ; $05:D6B2: A8
...
CODE_05D7AE:
JSR CODE_05D7D1         ; $05:D7AE: 20 D1 D7
...
CODE_05D7C2:
LDX #$03                ; $05:D7C2: A2 03

CODE_05D7C4:
LDA DATA_05D79D,x       ; $05:D7C4: BD 9D D7
...
CODE_05D7D1:
LDX #$03                ; $05:D7D1: A2 03
LDY #$30                ; $05:D7D3: A0 30

CODE_05D7D5:
LDA $0E42,x             ; $05:D7D5: BD 42 0E
...
CODE_05D823:
LDA $0219               ; $05:D823: AD 19 02
CLC                     ; $05:D826: 18
ADC #$02                ; $05:D827: 69 02

CODE_05D829:
STA $0E46,x             ; $05:D829: 9D 46 0E
...
CODE_05DAFA:
LDY $0000               ; $05:DAFA: AC 00 00
PLB                     ; $05:DAFD: AB
RTL                     ; $05:DAFE: 6B

CODE_05DAFF:
LDX #$09                ; $05:DAFF: A2 09

CODE_05DB01:
STA $0901,y             ; $05:DB01: 99 01 09
PHA                     ; $05:DB04: 48
...
CODE_05DBE0:
LDA DATA_05DC47,x       ; $05:DBE0: BD 47 DC
...
CODE_05DC03:
JMP CODE_05DCE9         ; $05:DC03: 4C E9 DC

CODE_05DC06:
LDA $09                 ; $05:DC06: A5 09
LSR A                   ; $05:DC08: 4A
BCS CODE_05DC0E         ; $05:DC09: B0 03
DEC $0248,x             ; $05:DC0B: DE 48 02

CODE_05DC0E:
LDA $0248,x             ; $05:DC0E: BD 48 02
...
CODE_05DCCC:
LDA #$03                ; $05:DCCC: A9 03

CODE_05DCCE:
STA $0C00,y             ; $05:DCCE: 99 00 0C
...
LDY $0B62,x             ; $05:DD0A: BC 62 0B
LDA $0747               ; $05:DD0D: AD 47 07

; If screen locked, branch
...
CODE_05DD1A:
LDX #$00                ; $05:DD1A: A2 00
BEQ CODE_05DD25         ; $05:DD1C: F0 07

CODE_05DD1E:
LDA $09                 ; $05:DD1E: A5 09
LSR A                   ; $05:DD20: 4A
LSR A                   ; $05:DD21: 4A
...
CODE_05DD56:
LDA DATA_05DCFB,x       ; $05:DD56: BD FB DC
...
CODE_05DD72:
LDX $9E                 ; $05:DD72: A6 9E
STZ $39,x               ; $05:DD74: 74 39

CODE_05DD76:
LDX $9E                 ; $05:DD76: A6 9E
PLB                     ; $05:DD78: AB
RTL                     ; $05:DD79: 6B
...
; Clear windowing HDMA table
JSR CODE_05E231         ; $05:DE14: 20 31 E2
...
CODE_05DE97:
JSR CODE_05DFFD         ; $05:DE97: 20 FD DF
...
; Save and continue/quit sound
STA $1600               ; $05:DF57: 8D 00 16
...
; Pause sound
STA $1600               ; $05:DF79: 8D 00 16

CODE_05DF7C:
LDA $1680               ; $05:DF7C: AD 80 16
...
; pause flashing arrow tiles

DATA_05DFAC:
db $D0,$DE

; smb1 pause screen tilemap
...
CODE_05E009:
LDA #$54                ; $05:E009: A9 54
STA $00                 ; $05:E00B: 85 00

CODE_05E00D:
LDA DATA_05DFAE,x       ; $05:E00D: BD AE DF
...
CODE_05E04B:
LDA #$40                ; $05:E04B: A9 40
STA $01                 ; $05:E04D: 85 01

CODE_05E04F:
LDA #$40                ; $05:E04F: A9 40
STA $00                 ; $05:E051: 85 00

CODE_05E053:
LDA $00                 ; $05:E053: A5 00
...
CODE_05E0B3:
LDA $0B6B               ; $05:E0B3: AD 6B 0B

; Windowing HDMA routine for pause which actually expands stuff
; Timer for expanding and collapsing
BEQ CODE_05E116         ; $05:E0B6: F0 5E
BMI CODE_05E116         ; $05:E0B8: 30 5C
REP #$30                ; $05:E0BA: C2 30

; Decrease timer. when it hits 0, the expanding ends
DEC $0B6B               ; $05:E0BC: CE 6B 0B
DEC $0B6D               ; $05:E0BF: CE 6D 0B

; Maximum left border set
DEC $0B6D               ; $05:E0C2: CE 6D 0B
INC $0B6F               ; $05:E0C5: EE 6F 0B

; Maximum right border set
INC $0B6F               ; $05:E0C8: EE 6F 0B
DEC $0B71               ; $05:E0CB: CE 71 0B
DEC $0B71               ; $05:E0CE: CE 71 0B
DEC $0B71               ; $05:E0D1: CE 71 0B

; Maximum upper border set
DEC $0B71               ; $05:E0D4: CE 71 0B
INC $0B73               ; $05:E0D7: EE 73 0B
INC $0B73               ; $05:E0DA: EE 73 0B
INC $0B73               ; $05:E0DD: EE 73 0B

; Maximum lower border set
INC $0B73               ; $05:E0E0: EE 73 0B
LDX #$007E              ; $05:E0E3: A2 7E 00
LDA #$00FF              ; $05:E0E6: A9 FF 00

CODE_05E0E9:
STA $7FF000,x           ; $05:E0E9: 9F 00 F0 7F
STA $7FF080,x           ; $05:E0ED: 9F 80 F0 7F
STA $7FF100,x           ; $05:E0F1: 9F 00 F1 7F
STA $7FF180,x           ; $05:E0F5: 9F 80 F1 7F
DEX                     ; $05:E0F9: CA
DEX                     ; $05:E0FA: CA

; Clear windowing RAM?
; NOPing it out doesn't seem to have any side effects
BPL CODE_05E0E9         ; $05:E0FB: 10 EC
LDA $0B6F               ; $05:E0FD: AD 6F 0B
XBA                     ; $05:E100: EB

; Maximum right border
; swap low and high bytes
; OR with maximum left border -> to Windowing HDMA value
ORA $0B6D               ; $05:E101: 0D 6D 0B

; |Load maximum upper border as an index
LDX $0B71               ; $05:E104: AE 71 0B

; / Store windowing HDMA value

CODE_05E107:
STA $7FF000,x           ; $05:E107: 9F 00 F0 7F
INX                     ; $05:E10B: E8
INX                     ; $05:E10C: E8
CPX $0B73               ; $05:E10D: EC 73 0B

; If we didn't reach maximum lower border yet
; Loop
BNE CODE_05E107         ; $05:E110: D0 F5
SEP #$30                ; $05:E112: E2 30
BRA CODE_05E119         ; $05:E114: 80 03

; Increase pause window state?

CODE_05E116:
INC $0B75               ; $05:E116: EE 75 0B

CODE_05E119:
JSR CODE_05DFFD         ; $05:E119: 20 FD DF
RTS                     ; $05:E11C: 60

; COLLAPSE PAUSE WINDOW ROUTINE

CODE_05E11D:
LDA $0B6B               ; $05:E11D: AD 6B 0B
BEQ CODE_05E180         ; $05:E120: F0 5E
BMI CODE_05E180         ; $05:E122: 30 5C
REP #$30                ; $05:E124: C2 30
INC $0B6D               ; $05:E126: EE 6D 0B
...
LDA $0B6F               ; $05:E167: AD 6F 0B
XBA                     ; $05:E16A: EB

; Maximum right border
; Swap low and high bytes
; ORA with maximum left border -> to windowing HDMA value
ORA $0B6D               ; $05:E16B: 0D 6D 0B
LDX $0B71               ; $05:E16E: AE 71 0B

CODE_05E171:
STA $7FF000,x           ; $05:E171: 9F 00 F0 7F
INX                     ; $05:E175: E8

; Load maximum upper border as an index
;
; Store windowing HDMA value to the HDMA RAM
INX                     ; $05:E176: E8
CPX $0B73               ; $05:E177: EC 73 0B

; If not done yet, loop
BNE CODE_05E171         ; $05:E17A: D0 F5
SEP #$30                ; $05:E17C: E2 30
BRA CODE_05E1AB         ; $05:E17E: 80 2B

; Pause window state to zero

CODE_05E180:
STZ $0B75               ; $05:E180: 9C 75 0B
LDA #$0F                ; $05:E183: A9 0F

; Set brightness to max
STA $1201               ; $05:E185: 8D 01 12

; Clear paused flag
STZ $0776               ; $05:E188: 9C 76 07
LDA $1206               ; $05:E18B: AD 06 12
EOR #$03                ; $05:E18E: 49 03

; reverse window mask settings
...
CODE_05E1AB:
JSR CODE_05DFFD         ; $05:E1AB: 20 FD DF
RTS                     ; $05:E1AE: 60

CODE_05E1AF:
DEC $0F8A               ; $05:E1AF: CE 8A 0F
...
CMP #$02                ; $05:E1C2: C9 02
BNE CODE_05E1CA         ; $05:E1C4: D0 04

; If option is not Save and Quit
; Branch
; Go save and quit
JML CODE_0080DE         ; $05:E1C6: 5C DE 80 00

CODE_05E1CA:
JSR CODE_05DFFD         ; $05:E1CA: 20 FD DF
RTS                     ; $05:E1CD: 60
...
LDX #$007E              ; $05:E1D5: A2 7E 00
LDA #$0000              ; $05:E1D8: A9 00 00

CODE_05E1DB:
STA $F000,x             ; $05:E1DB: 9D 00 F0
...
; A = 8-bit. XY = 16-bit
SEP #$20                ; $05:E1EC: E2 20
LDX #$2641              ; $05:E1EE: A2 41 26

; Window 1 left position & window 1 right position
; 2 regs write once. Indirect HDMA
STX $4330               ; $05:E1F1: 8E 30 43
LDX.w #DATA_05E22A      ; $05:E1F4: A2 2A E2
STX $4332               ; $05:E1F7: 8E 32 43
...
; Indirect HDMA address bank byte: $7F:xxxx
STA $4337               ; $05:E201: 8D 37 43
LDA #$00                ; $05:E204: A9 00
STA $1204               ; $05:E206: 8D 04 12

; Window mask settings for BG1, 2, 3, 4.
; No window for BG1, 2, 3, 4.
STZ $1205               ; $05:E209: 9C 05 12
LDA $1206               ; $05:E20C: AD 06 12
ORA #$03                ; $05:E20F: 09 03

; Enable window 1 for OBJ. Window 1 inversion for OBJ
STA $1206               ; $05:E211: 8D 06 12
LDA $1207               ; $05:E214: AD 07 12
ORA #$10                ; $05:E217: 09 10
STA $1207               ; $05:E219: 8D 07 12

; Enable window masking for OBJ on the mainscreen
;
; None on the subscreen
STZ $1208               ; $05:E21C: 9C 08 12

; AXY = 8-bit
SEP #$10                ; $05:E21F: E2 10
LDA $1203               ; $05:E221: AD 03 12
ORA #$08                ; $05:E224: 09 08
...
CODE_05E2D9:
LDA #$2A                ; $05:E2D9: A9 2A

CODE_05E2DB:
STA $0803,y             ; $05:E2DB: 99 03 08
...
CODE_05E439:
JMP CODE_05E4A5         ; $05:E439: 4C A5 E4

CODE_05E43C:
LDA #$01                ; $05:E43C: A9 01
STA $1200               ; $05:E43E: 8D 00 12

CODE_05E441:
LDA $0E74               ; $05:E441: AD 74 0E
...
LDA #$18                ; $05:E5CF: A9 18

; Lightning sound
STA $1603               ; $05:E5D1: 8D 03 16

CODE_05E5D4:
LDA #$11                ; $05:E5D4: A9 11
...
CODE_05E631:
JMP CODE_05E43C         ; $05:E631: 4C 3C E4

CODE_05E634:
LDA $0B76               ; $05:E634: AD 76 0B
BNE CODE_05E6AE         ; $05:E637: D0 75

; Flag to upload animated FG
STZ $028D               ; $05:E639: 9C 8D 02
...
; Set flag to upload animated FG
INC $028D               ; $05:E675: EE 8D 02
BRA CODE_05E6AE         ; $05:E678: 80 34

CODE_05E67A:
LDA $09                 ; $05:E67A: A5 09
CLC                     ; $05:E67C: 18
...
; Bonus Room background number, indexed by player number.
...
CODE_05E6B1:
SEP #$30                ; $05:E6B1: E2 30
PHB                     ; $05:E6B3: 8B
PHK                     ; $05:E6B4: 4B
PLB                     ; $05:E6B5: AB
LDA $99                 ; $05:E6B6: A5 99
CMP #$01                ; $05:E6B8: C9 01

; Branch if not bonus room
BNE CODE_05E6C7         ; $05:E6BA: D0 0B

; Set bonus room flag
STA $02F8               ; $05:E6BC: 8D F8 02
LDX $0753               ; $05:E6BF: AE 53 07
LDA DATA_05E6AF,x       ; $05:E6C2: BD AF E6

; Get Luigi or Mario's bonus room tileset number
STA $99                 ; $05:E6C5: 85 99

; GFX upload routine

CODE_05E6C7:
JSR CODE_05E82A         ; $05:E6C7: 20 2A E8
LDA $99                 ; $05:E6CA: A5 99
ASL A                   ; $05:E6CC: 0A
TAX                     ; $05:E6CD: AA
LDA PNTR_05E6DB,x       ; $05:E6CE: BD DB E6
STA $00                 ; $05:E6D1: 85 00
LDA PNTR_05E6DB+1,x     ; $05:E6D3: BD DC E6
STA $01                 ; $05:E6D6: 85 01

; A routine for each tilemap
...
; Set GFX to upload to none

CODE_05E71B:
STZ $99                 ; $05:E71B: 64 99
PLB                     ; $05:E71D: AB

; Disable flag to upload animated graphics
STZ $028D               ; $05:E71E: 9C 8D 02
LDA #$01                ; $05:E721: A9 01

; Disable graphics animation and stuff
STA $028C               ; $05:E723: 8D 8C 02
RTL                     ; $05:E726: 6B

; Set grass or snow foreground

CODE_05E727:
LDA $DB                 ; $05:E727: A5 DB
CMP #$16                ; $05:E729: C9 16

; If tall slim hills background with snow
; get snow foreground
BEQ CODE_05E748         ; $05:E72B: F0 1B
CMP #$14                ; $05:E72D: C9 14

; Same as above
BEQ CODE_05E748         ; $05:E72F: F0 17
CMP #$0D                ; $05:E731: C9 0D

; Same as above
BEQ CODE_05E748         ; $05:E733: F0 13
LDA #$17                ; $05:E735: A9 17

; Grass foreground
; GFX upload routine
JSR CODE_05E82A         ; $05:E737: 20 2A E8

; finish upload
BRA CODE_05E71B         ; $05:E73A: 80 DF

; Set underground foreground

CODE_05E73C:
LDA #$11                ; $05:E73C: A9 11

; Underground foreground
; GFX upload routine
JSR CODE_05E82A         ; $05:E73E: 20 2A E8

; finish upload
BRA CODE_05E71B         ; $05:E741: 80 D8

; Set background to starry night and foreground to snow

CODE_05E743:
LDA #$16                ; $05:E743: A9 16

; Starry night background
; GFX upload routine
JSR CODE_05E82A         ; $05:E745: 20 2A E8

CODE_05E748:
LDA #$12                ; $05:E748: A9 12

; Snow foreground
; GFX upload routine
JSR CODE_05E82A         ; $05:E74A: 20 2A E8

; finish upload
BRA CODE_05E71B         ; $05:E74D: 80 CC

; Set Bowser's castle background

CODE_05E74F:
LDA #$13                ; $05:E74F: A9 13

; Bowser's castle background (1)
; GFX upload routine
JSR CODE_05E82A         ; $05:E751: 20 2A E8
LDA #$14                ; $05:E754: A9 14

; Bowser's castle background (2)
; GFX upload routine
JSR CODE_05E82A         ; $05:E756: 20 2A E8

; finish upload
BRA CODE_05E71B         ; $05:E759: 80 C0

; Set game over graphics

CODE_05E75B:
LDA #$15                ; $05:E75B: A9 15

; Game over and time up text
; GFX upload routine
JSR CODE_05E82A         ; $05:E75D: 20 2A E8

; finish upload
BRA CODE_05E71B         ; $05:E760: 80 B9
...
; Bank bytes for GFX data addresses.
...
; Low and high bytes for GFX data addresses
...
; Routine enters with $7E0099 loaded in accumulator
; but can also be fixed values. A * 2 to index
TAX                     ; $05:E82B: AA
LDA DATA_05E762,x       ; $05:E82C: BD 62 E7

; Load DMA source bank byte
STA $0287               ; $05:E82F: 8D 87 02

; 16-bit A
REP #$20                ; $05:E832: C2 20
LDA DATA_05E794,x       ; $05:E834: BD 94 E7
...
; DMA VRAM address
STA $028A               ; $05:E83D: 8D 8A 02
LDA DATA_05E7F8,x       ; $05:E840: BD F8 E7

; DMA size
STA $0288               ; $05:E843: 8D 88 02

; 8-bit A
SEP #$20                ; $05:E846: E2 20

; DMA this setup
JSR CODE_05E84C         ; $05:E848: 20 4C E8
RTS                     ; $05:E84B: 60

CODE_05E84C:
LDA #$80                ; $05:E84C: A9 80

; address increment mode
STA $2115               ; $05:E84E: 8D 15 21
REP #$20                ; $05:E851: C2 20
LDA $028A               ; $05:E853: AD 8A 02

; DMA VRAM address
STA $2116               ; $05:E856: 8D 16 21
LDA #$1801              ; $05:E859: A9 01 18

; 2 regs write once for $2118
STA $4300               ; $05:E85C: 8D 00 43
LDA $0285               ; $05:E85F: AD 85 02

; Set DMA source address
STA $4302               ; $05:E862: 8D 02 43
LDX $0287               ; $05:E865: AE 87 02

; DMA source bank
STX $4304               ; $05:E868: 8E 04 43
LDA $0288               ; $05:E86B: AD 88 02

; DMA size
STA $4305               ; $05:E86E: 8D 05 43
LDX #$01                ; $05:E871: A2 01

; Enable DMA on channel 0
STX $420B               ; $05:E873: 8E 0B 42
...
LDX $9E                 ; $05:E880: A6 9E
LDA $1C,x               ; $05:E882: B5 1C
CMP #$04                ; $05:E884: C9 04

; Return if not a regular shelled walking sprite
BCS CODE_05E890         ; $05:E886: B0 08
LDA $29,x               ; $05:E888: B5 29

; If not kicked, return
BMI CODE_05E894         ; $05:E88A: 30 08
AND #$06                ; $05:E88C: 29 06

; If not stunned or upside-down, return
BEQ CODE_05E894         ; $05:E88E: F0 04
...
CODE_05E8A5:
STZ $0F40,x             ; $05:E8A5: 9E 40 0F
BRA CODE_05E8B7         ; $05:E8A8: 80 0D

CODE_05E8AA:
AND #$80                ; $05:E8AA: 29 80
...
CODE_05E8F9:
JSR CODE_05E9BF         ; $05:E8F9: 20 BF E9
...
CODE_05E908:
LDA $03B9               ; $05:E908: AD B9 03
CLC                     ; $05:E90B: 18
ADC #$10                ; $05:E90C: 69 10

CODE_05E90E:
JSR CODE_05E9BF         ; $05:E90E: 20 BF E9
...
; Get current sprite index
LDX $9E                 ; $05:E95E: A6 9E
LDA $1C,x               ; $05:E960: B5 1C
CMP #$35                ; $05:E962: C9 35

; If not princess peach, branch
BNE CODE_05E9AE         ; $05:E964: D0 48
LDA $075F               ; $05:E966: AD 5F 07
CMP #$07                ; $05:E969: C9 07
...
CODE_05EA50:
LDA #$F0                ; $05:EA50: A9 F0

CODE_05EA52:
STA $0901,y             ; $05:EA52: 99 01 09
...
CODE_05EA5E:
LDA #$29                ; $05:EA5E: A9 29

CODE_05EA60:
STA $0903,y             ; $05:EA60: 99 03 09
...
CODE_05EAE8:
LDA DATA_05EA9D,x       ; $05:EAE8: BD 9D EA
...
; Set more difficult quest flag in SRAM
STA $701FF8             ; $05:EB60: 8F F8 1F 70
LDA $075F               ; $05:EB64: AD 5F 07

; Set current world in SRAM
STA $701FF9             ; $05:EB67: 8F F9 1F 70
...
DATA_05EE0A:
db $00,$01,$80,$01,$02,$01,$80,$01
db $80,$01,$02,$80,$02,$80,$01,$80
db $01,$80,$01,$80,$01,$02,$01

DATA_05EE21:
db $00,$A0,$08,$08,$40,$30,$10,$08
...
CODE_05EE49:
LDY #$05                ; $05:EE49: A0 05
SEC                     ; $05:EE4B: 38

CODE_05EE4C:
LDA $CE,x               ; $05:EE4C: B5 CE
